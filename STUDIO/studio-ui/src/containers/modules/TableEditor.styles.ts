...
export const useStyles = createStyles(({ css, token }) => ({
     ...
     *
     * <p>It wears what the cell wears — the same font, the same alignment, the same colour, over the cell's own
     * background — so a value reads while it is being written the way it will read once it is written, which is
     * what the Editor did by putting the cell's own styling on its field.
     */
    input: css`
        width: 100%;
        min-width: 0;
