...
/** One-line summary of which rules fired, capped so a collect-all table does not overflow the panel. */
const firedSummaryText = (firedRules: string[], t: TFunction): string => {
    if (firedRules.length === 0) {
        return t('decision.noneFired')
    }
    if (firedRules.length <= FIRED_PREVIEW) {
        return t('decision.fired', { rules: firedRules.join(', ') })
    }
    return t('decision.firedCount', {
        count: firedRules.length,
        rules: firedRules.slice(0, FIRED_PREVIEW).join(', '),
    })
}
...
const DecisionPanel: React.FC<DecisionPanelProps> = ({ decision, ruleNames, frameUri, frameName }) => {
    ...
    const firedSummary = firedSummaryText(firedRules, t)
