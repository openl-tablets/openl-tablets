...
describe('TraceTree', () => {
    ...
    it('replays a returned node by restarting the trace and running to it', async () => {
        useTraceStore.setState({
            ...
            status: 'completed',
            frames: [],
            tree: { uri: 'uRoot', name: 'ROOT', instance: 3, kind: 'spreadsheet', durationMillis: 42, selfMillis: 42, steps: []},
        })
        render(<TraceTree />)
        await userEvent.click(screen.getByTestId('tree-replay-uRoot'))
        // Replay restarts (terminate) and runs to this exact execution: the breakpoint targets its instance (@3).
        await waitFor(() => expect(traceService.setBreakpoints).toHaveBeenCalledWith('p1', ['uRoot@3']))
                ...
                frames: [frame(0, {
                    ...
                    steps: [
                        step('R0C0', 'executed', '$Term'),
                        {
                            ...
                            children: [
                                { uri: 'u0', name: '$Term', instance: 0, kind: 'stepRef', durationMillis: 0, selfMillis: 0,
                                    steps: [], refStep: 'R0C0' },
                                { uri: 'u9', name: 'SubPremium', instance: 0, kind: 'spreadsheet', durationMillis: 3,
                                    selfMillis: 3, steps: []},
                            ],
                        },
                    ],
