...
describe('parseChangePing', () => {
    it('reads the files and the origins the backend named', () => {
        expect(parseChangePing(JSON.stringify({ files: ['rules/A.xlsx'], origins: ['tab-1']})))
            .toEqual({ files: ['rules/A.xlsx'], origins: ['tab-1']})
    })

    it('reads a ping that carries origins alone — the workspace ping and the broadcast', () => {
        expect(parseChangePing(JSON.stringify({ origins: ['tab-1']})))
            .toEqual({ files: [], origins: ['tab-1']})
    })
    ...
    it('reads a malformed body as a change of unknown files and unknown origin', () => {
        ...
        expect(parseChangePing('not json')).toEqual({ files: [], origins: []})
        expect(parseChangePing(JSON.stringify({ files: 'all', origins: 7 }))).toEqual({ files: [], origins: []})
    })
})

describe('isOwnEcho', () => {
    it('recognises a ping this tab caused on its own', () => {
        expect(isOwnEcho({ files: [], origins: [CLIENT_ID]})).toBe(true)
    })
    ...
    it('keeps a ping that stands for another session as well', () => {
        ...
        expect(isOwnEcho({ files: [], origins: [CLIENT_ID, 'another-tab']})).toBe(false)
    })

    it('keeps a ping made outside a request, which belongs to nobody', () => {
        expect(isOwnEcho({ files: [], origins: []})).toBe(false)
    })
})
