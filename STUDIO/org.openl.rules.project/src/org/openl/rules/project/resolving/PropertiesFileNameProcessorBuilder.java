...
public final class PropertiesFileNameProcessorBuilder {
        ...
        if (StringUtils.isBlank(prcClass) || prcClass
            ...
            try {
                ...
                try {
                    ...
                    if (CollectionUtils.isEmpty(patterns)) {
                        PropertiesFileNameProcessor processor = newInstance(declaredConstructor);
                        this.processor = processor;
                    } else {
                        this.processor = new PropertiesFileNameProcessor() {
                            @Override
                            public ITableProperties process(String modulePath) throws NoMatchFileNameException {
                                try {
                                    ...
                                    return processor.process(module, patterns);
                                } catch (InvalidFileNamePatternException e1) {
    ...
    private PropertiesFileNameProcessor buildDefault(String... patterns) throws InvalidFileNamePatternException {
    ...
    private PropertiesFileNameProcessor wrapProcessors(PropertiesFileNameProcessor[] prcsrs) {
        if (prcsrs.length == 1) {
            return prcsrs[0];
        } else {
            return new PropertiesFileNameProcessor() {
                @Override
                public ITableProperties process(String modulePath) throws NoMatchFileNameException {
