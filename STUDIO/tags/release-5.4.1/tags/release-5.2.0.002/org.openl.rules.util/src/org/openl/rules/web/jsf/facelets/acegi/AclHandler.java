...
public class AclHandler extends TagHandler {
        ...
        throws IOException, FacesException, FaceletException, ELException
    {
        ...
        if (beans.length == 0) {
            log.warn("No AclManager would found the application context: "
                + context.toString());
        ...
        if (log.isDebugEnabled()) {
            log.debug("Authentication: '" + auth + "' has: "
                + ((acls == null) ? 0 : acls.length) + " AclEntrys for domain object: '"
                + resolvedDomainObject + "' from AclManager: '" + aclManager.toString()
                + "'");
        }
            ...
            if (acls[i] instanceof BasicAclEntry) {
                ...
                for (int j = 0; j < requiredIntegers.length; j++) {
                    if (processableAcl.isPermitted(requiredIntegers[j].intValue())) {
                        if (log.isDebugEnabled()) {
                            log.debug("Including tag body as found permission: "
                                + requiredIntegers[j] + " due to AclEntry: '"
                                + processableAcl + "'");
                        }
                        nextHandler.apply(faceletContext, parent);
                        return;
    ...
    private ApplicationContext getAppCtx(FaceletContext ctx) {
        HttpServletRequest request = (HttpServletRequest) ctx.getFacesContext()
