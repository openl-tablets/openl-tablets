...
YAHOO.widget.DateMath = new function() {
  ...
  this.add = function(date, field, amount) {
    ...
    switch (field)
      {case this.MONTH:var newMonth = date.getMonth() + amount;var years = 0;if (newMonth < 0) {
      while (newMonth < 0)
      {
        newMonth += 12;
        years -= 1;
  ...
  for (var c = 0; c < this.cells.length; ++c)
  {
    ...
    if (this.isDateOOM(workingDate))
    {
      ...
      for (var r = 0; r < this.renderStack.length; ++r)
      {
        ...
        var day;
        var year;
        switch (type) {case YAHOO.widget.Calendar_Core.DATE:month = rArray[1][1];day = rArray[1][2];year = rArray[1][0];if (workingDate.getMonth() + 1 == month && workingDate.getDate() == day && workingDate.getFullYear() == year)
        {
          renderer = rArray[2];
