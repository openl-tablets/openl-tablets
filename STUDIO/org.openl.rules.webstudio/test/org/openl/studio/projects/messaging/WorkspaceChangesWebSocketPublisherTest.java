...
class WorkspaceChangesWebSocketPublisherTest {
    ...
    void pings_the_workspace_and_the_project_of_the_acting_user_on_a_state_change() {
        ...
        verify(notificationService).notifyWorkspaceChanged("jane", new ChangeNotes(Set.of(), Set.of()));
        verify(notificationService).notifyProjectChanged("jane", projectId, new ChangeNotes(Set.of(), Set.of()));
        // Workspace and per-project pings coalesce independently of each other.
        verify(debouncer).debounce(eq("jane"), any(ChangeNotes.class), any());
        verify(debouncer).debounce(eq("jane|" + projectId.encode()), any(ChangeNotes.class), any());
    }
    ...
    void the_files_of_an_edit_ride_the_project_ping() {
        ...
        verify(notificationService).notifyProjectChanged("jane", projectId,
                new ChangeNotes(Set.of("rules/A.xlsx"), Set.of()));
    }
    ...
    void both_pings_name_the_client_whose_request_made_the_change() {
        ...
        // The tab that acted recognises its own echo and does not re-read what it has just read.
        verify(notificationService).notifyWorkspaceChanged("jane", new ChangeNotes(Set.of(), Set.of("tab-1")));
        verify(notificationService).notifyProjectChanged("jane", projectId,
                new ChangeNotes(Set.of(), Set.of("tab-1")));
    }
