...
final class DebugChannel {
    ...
    DebugCommand awaitCommand() {
        ...
        try {
            ...
            haltCount++;
            halted.signalAll();
            // Wait for a command; a concurrent terminate (set under this lock while parked) also wakes and
            // exits the loop, so it is re-checked below and turned into a clean unwind.
