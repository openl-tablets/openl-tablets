...
public class ProjectSocketNotificationService {
    ...
    private static final String TOPIC_PROJECTS_TRACE = "/topic/projects/%s/trace";
    ...
    /**
     * Notifies user about trace execution status change.
     *
     * @param user      user to notify
     * @param projectId project id
     * @param status    new trace execution status
     */
    public void notifyTraceExecutionStatus(CommonUser user, ProjectIdModel projectId, TraceExecutionStatus status) {
        messagingTemplate.convertAndSendToUser(user.getUserName(),
                TOPIC_PROJECTS_TRACE.formatted(encodePathSegment(projectId.encode())) + STATUS,
                status.name());
    }
