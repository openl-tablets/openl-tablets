...
/**
 * Factory for creating benchmark execution progress listeners that notify users via WebSocket.
 */
@RequiredArgsConstructor
@Component
public class SocketBenchmarkExecutionProgressListenerFactory {

    private final ProjectSocketNotificationService socketNotificationService;

    /**
     * Creates an ExecutionProgressListener that notifies the user via WebSocket about benchmark progress.
     *
    ...
    public ExecutionProgressListener create(CommonUser user, ProjectIdModel projectId, String tableId) {
        return new ExecutionProgressListener() {
            ...
            public void onStatusChanged(ExecutionStatus status) {
                socketNotificationService.notifyBenchmarkExecutionStatus(user, projectId, tableId, status);
            }

            @Override
            public void onError(String message, Throwable cause) {
                socketNotificationService.notifyBenchmarkExecutionError(user, projectId, tableId, message);
            }
        };
    }
}
