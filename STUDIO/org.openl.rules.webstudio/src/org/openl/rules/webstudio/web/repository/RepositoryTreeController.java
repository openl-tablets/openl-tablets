...
public class RepositoryTreeController {
	...
	public String addFolder() {
		AProjectArtefact projectArtefact = repositoryTreeState
		...
		if (projectArtefact instanceof AProjectFolder) {
			if (folderName != null && !folderName.isEmpty()) {
				if (NameChecker.checkName(folderName)) {
					if (!NameChecker.checkIsFolderPresent(
							(AProjectFolder) projectArtefact, folderName)) {
						AProjectFolder folder = (AProjectFolder) projectArtefact;

						try {
							AProjectFolder addedFolder = folder
									.addFolder(folderName);
							repositoryTreeState.addNodeToTree(
									repositoryTreeState.getSelectedNode(),
									addedFolder);
							resetStudioModel();
						...
						errorMessage = "Folder name '" + folderName
								+ "' is invalid. " + NameChecker.FOLDER_EXISTS;
					}
				} else {
				...
				errorMessage = "Folder name '" + folderName + "' is invalid. "
						+ NameChecker.FOLDER_NAME_EMPTY;
			}
		}

		if (errorMessage != null) {
			FacesUtils
					.addErrorMessage("Failed to create folder.", errorMessage);
		}
		return null;
	}
	...
	public String saveProject() {
		try {
			UserWorkspaceProject project = repositoryTreeState
					.getSelectedProject();
	...
	public String closeProject() {
		try {
			...
			repositoryTreeState.refreshSelectedNode();
			if (repositoryTreeState.getSelectedProject().equals(
					studio.getModel().getProject())) {
	...
	public List<String> getDependingProjects() {
		...
		TreeProject projectNode = selectedNode instanceof TreeProject ? (TreeProject) selectedNode
				: null;
		if (projectNode != null) {
			...
			for (ProjectDescriptor projectDescriptor : studio.getAllProjects()) {
				if (projectDescriptor.getDependencies() != null) {
					for (ProjectDependencyDescriptor dependency : projectDescriptor
							.getDependencies()) {
	...
	public boolean getHasDependenciesForVersion() {
		...
		try {
			...
			AProject newVersion = userWorkspace.getDesignTimeRepository()
					.getProject(selectedProject.getName(),
	...
	public Collection<String> getDependencies() {
		if (!repositoryTreeState.getCanOpen()
				&& !repositoryTreeState.getCanEdit()) {
			// Because ui:repeat ignores the "rendered" property, here is a
			// workaround to reduce performance drop.
			return Collections.emptyList();
		}
		List<String> dependencies = new ArrayList<String>(getDependencies(
				getSelectedProject(), true));
	...
	private Collection<String> getDependencies(AProject project,
			boolean recursive) {
	...
	private void calcDependencies(Collection<String> result, AProject project,
			boolean recursive) {
		...
		for (ProjectDependencyDescriptor dependency : dependencies) {
			try {
				TreeProject projectNode = repositoryTreeState
						.getProjectNodeByLogicalName(dependency.getName());
				if (projectNode == null) {
					continue;
				}
				String physicalName = projectNode.getName();
				AProject dependentProject = userWorkspace.getProject(
						physicalName, false);
	...
	private boolean canOpen(AProject theProject) {
		...
		return !project.isLocalOnly() && !project.isOpenedForEditing()
				&& !project.isOpened() && !project.isDeleted();
	}
	...
	public String copyDeploymentProject() {
		...
		try {
			...
			log.error(
					"Cannot obtain deployment project '" + projectName + "'.",
					e);
		...
		if (project == null) {
			...
			errorMessage = "Deployment project '" + newProjectName
					+ "' already exists.";
		}

		if (errorMessage != null) {
			FacesUtils.addErrorMessage("Cannot copy deployment project.",
					errorMessage);
			return null;
		}
		...
		try {
			...
			ADeploymentProject newProject = userWorkspace
					.getDDProject(newProjectName);
		...
		try {
			userWorkspace.copyProject(project, newProjectName,
					new ResourceTransformer() {
						@Override
						public InputStream tranform(AProjectResource resource)
							...
							if (isProjectDescriptor(resource)) {
								...
								try {
									...
									ProjectDescriptor projectDescriptor = serializer
											.deserialize(content);
									projectDescriptor.setName(newProjectName);
									return new ByteArrayInputStream(serializer
									...
									throw new ProjectException(
											"UTF-8 charset is not supported", e);
								} finally {
						...
						private boolean isProjectDescriptor(
								AProjectResource resource) {
							String actualFullPath = resource.getArtefactPath()
									.withoutFirstSegment().getStringValue();
							String expectedFullPath = ArtefactPathImpl.SEGMENT_DELIMITER
	...
	public String createDeploymentConfiguration() {
		try {
			...
			ADeploymentProject createdProject = userWorkspace
					.getDDProject(projectName);
			createdProject.edit();
	...
	public String createNewRulesProject() {
		...
		ProjectFile[] templateFiles = getProjectTemplateFiles(TEMPLATES_PATH
				+ newProjectTemplate);
		if (templateFiles.length <= 0) {
			this.clearForm();
			String errorMessage = String.format(
					"Can`t load template files: %s", newProjectTemplate);
			FacesUtils.addErrorMessage(errorMessage);
			return null;
		}
		...
		if (creationMessage == null) {
			try {
				...
				selectProject(projectName,
						repositoryTreeState.getRulesRepository());
	...
	private String validateProjectName() {
		...
		if (StringUtils.isBlank(projectName)) {
			...
			msg = "Specified name is not a valid project name." + " "
					+ NameChecker.BAD_NAME_MSG;
		} else if (userWorkspace.hasProject(projectName)
				|| repositoryTreeState.getProjectNodeByLogicalName(projectName) != null) {
	...
	public String deleteDeploymentConfiguration() {
		String projectName = FacesUtils
				.getRequestParameter("deploymentProjectName");

		try {
			ADeploymentProject project = userWorkspace
					.getDDProject(projectName);
			project.delete(userWorkspace.getUser());
			if (repositoryTreeState.isHideDeleted()) {
				TreeNode projectInTree = repositoryTreeState
			...
			FacesUtils
					.addInfoMessage("Configuration was deleted successfully.");
		} catch (ProjectException e) {
			log.error("Cannot delete deployment configuration '" + projectName
					+ "'.", e);
	...
	public String deleteNode() {
		...
		try {
			...
			if (selectedNode != activeProjectNode) {
				...
				boolean wasMarkedForDeletion = UiConst.TYPE_DEPLOYMENT_PROJECT
						.equals(nodeType)
						|| (UiConst.TYPE_PROJECT.equals(nodeType) && !((UserWorkspaceProject) projectArtefact)
								.isLocalOnly());
				if (wasMarkedForDeletion
						&& !repositoryTreeState.isHideDeleted()) {
					repositoryTreeState.refreshSelectedNode();
				} else {
				...
				if (repositoryTreeState.isHideDeleted()
						|| ((UserWorkspaceProject) projectArtefact)
								.isLocalOnly()) {
			...
			FacesUtils
					.addErrorMessage("Failed to delete node.", e.getMessage());
		}
	...
	public String unlockNode() {
		...
		try {
			...
			FacesUtils
					.addErrorMessage("Failed to unlock node.", e.getMessage());
		}
	...
	public String unlockProject() {
		...
		try {
			...
			FacesUtils.addErrorMessage("Failed to unlock rules project.",
					e.getMessage());
		}
		return null;
	}
	...
	public String unlockDeploymentConfiguration() {
		String deploymentProjectName = FacesUtils
				.getRequestParameter("deploymentProjectName");

		try {
			ADeploymentProject deploymentProject = userWorkspace
					.getDDProject(deploymentProjectName);
			deploymentProject.unlock(userWorkspace.getUser());
			resetStudioModel();
		} catch (ProjectException e) {
			log.error("Cannot unlock deployment project '"
					+ deploymentProjectName + "'.", e);
			FacesUtils.addErrorMessage("Failed to unlock deployment project.",
					e.getMessage());
		}
		return null;
	}
	...
	public String eraseProject() {
		...
		if (!project.isDeleted()) {
			...
			repositoryTreeState.invalidateSelection();
			FacesUtils.addErrorMessage("Cannot erase project '"
					+ project.getName()
	...
	public String exportProjectVersion() {
		...
		try {
			...
			AProject forExport = userWorkspace.getDesignTimeRepository()
					.getProject(selectedProject.getName(),
							new CommonVersionImpl(version));
			zipFile = new ProjectExportHelper().export(userWorkspace.getUser(),
					forExport);
			zipFileName = String.format("%s-%s.zip", selectedProject.getName(),
					version);
		} catch (ProjectException e) {
	...
	public String exportFileVersion() {
		...
		try {
			...
			ArtefactPath selectedNodePath = selectedNode.getData()
					.getArtefactPath().withoutFirstSegment();

			is = ((AProjectResource) forExport
					.getArtefactByPath(selectedNodePath)).getContent();
	...
	private Map<String, Object> getProps() {
		RulesRepositoryArtefact dataBean = repositoryTreeState
				.getSelectedNode().getData();
		if (dataBean != null) {
	...
	public SelectItem[] getSelectedProjectVersions() {
		Collection<ProjectVersion> versions = repositoryTreeState
				.getSelectedNode().getVersions();
	...
	private void openDependenciesIfNeeded() throws ProjectException {
		if (openDependencies) {
			for (String dependency : getDependencies(getSelectedProject(), true)) {
				TreeProject projectNode = repositoryTreeState
						.getProjectNodeByLogicalName(dependency);
				if (projectNode == null) {
					if (log.isErrorEnabled()) {
						log.error(String.format("Can't find dependency %s",
								dependency));
					}
					continue;
				}
	...
	public String openProjectVersion() {
		try {
			...
			repositoryTreeState.getSelectedProject().openVersion(
					new CommonVersionImpl(version));
			openDependenciesIfNeeded();
			repositoryTreeState.refreshSelectedNode();
			resetStudioModel();
	...
	public String selectDeploymentProject() {
		String projectName = FacesUtils.getRequestParameter("projectName");
		selectProject(projectName,
				repositoryTreeState.getDeploymentRepository());
		return null;
	}
	...
	public void setDateProperty(String propName, Date propValue) {
		if (!SPECIAL_DATE.equals(propValue)) {
			...
			FacesUtils.addErrorMessage("Specified " + getPropUIName(propName)
	...
	public void uploadListener(FileUploadEvent event) {
		...
		if (fileName.contains(".")) {
			...
			if (FileTypeHelper.isZipFile(fileName)) {
				ProjectDescriptor projectDescriptor = new ZipProjectDescriptorExtractor(
						zipFilter).getProjectDescriptor(file);
	...
	public void setVersionComment(String versionComment) {
		try {
			repositoryTreeState.getSelectedNode().getData()
					.setVersionComment(versionComment);
		} catch (PropertyException e) {
			log.error("Failed to set LOB!", e);
			FacesUtils.addErrorMessage("Can not set line of business.",
					e.getMessage());
		}
	}
		...
		if (StringUtils.isNotBlank(projectName)) {
			if (uploadedFiles != null && !uploadedFiles.isEmpty()) {
				ProjectUploader projectUploader = new ProjectUploader(
						uploadedFiles, projectName, userWorkspace, zipFilter);
	...
	private String uploadAndAddFile() {
		...
		try {
			AProjectFolder node = (AProjectFolder) repositoryTreeState
					.getSelectedNode().getData();

			AProjectResource addedFileResource = node.addResource(fileName,
					getLastUploadedFile().getInputStream());

			repositoryTreeState.addNodeToTree(
					repositoryTreeState.getSelectedNode(), addedFileResource);
	...
	private void registerInProjectDescriptor(
			AProjectResource addedFileResource) {
		if (FileTypeHelper.isExcelFile(fileName)) { // Excel. Add module to
													// rules.xml.
			try {
				UserWorkspaceProject selectedProject = repositoryTreeState
						.getSelectedProject();
				...
				if (projectDescriptorArtifact instanceof AProjectResource) {
					...
					ProjectDescriptor projectDescriptor = xmlProjectDescriptorSerializer
							.deserialize(content);
					String modulePath = addedFileResource.getArtefactPath().withoutFirstSegment()
							.getStringValue();
					while (modulePath.charAt(0) == '/'){
						modulePath = modulePath.substring(1);
					}
					Module module = new Module();
					module.setName(fileName.substring(0,fileName
							.lastIndexOf(".")));
					module.setType(ModuleType.API);
					module.setRulesRootPath(new PathEntry(modulePath));
					projectDescriptor.getModules().add(module);
					String xmlString = xmlProjectDescriptorSerializer
							.serialize(projectDescriptor);
					StringInputStream newContent = new StringInputStream(
							xmlString);
	...
	private String uploadAndUpdateFile() {
		...
		try {
			AProjectResource node = (AProjectResource) repositoryTreeState
					.getSelectedNode().getData();
	...
	private String uploadProject() {
		...
		if (StringUtils.isNotBlank(projectName)) {
			...
			if (uploadedItem != null) {
				ProjectUploader projectUploader = new ProjectUploader(
						uploadedItem, projectName, userWorkspace, zipFilter);
				errorMessage = validateProjectName();
				if (errorMessage == null) {
					errorMessage = projectUploader.uploadProject();
				}
	...
	public String[] getProjectTemplates(String category) {
		...
		try {
			...
			if (templates.length == 0) {
				// File System
				templates = resourceResolver.getResources(TEMPLATES_PATH
						+ category + "/*");
			}
			...
			for (Resource resource : templates) {
				if (!ResourceUtils.isFileURL(resource.getURL())) {
					...
					String templateUrl = URLDecoder.decode(resource.getURL()
							.getPath(), "UTF8");
					String[] templateParsed = templateUrl.split("/");
					templateNames
							.add(templateParsed[templateParsed.length - 1]);
				} else {
					// File System
		...
		names = templateNames.isEmpty() ? new String[0] : templateNames
				.toArray(new String[templateNames.size()]);
	...
	private ProjectFile[] getProjectTemplateFiles(String url) {
		...
		try {
			...
			for (Resource resource : templates) {
				templateFiles.add(new ProjectFile(resource.getFilename(),
		...
		return templateFiles.isEmpty() ? new ProjectFile[0] : templateFiles
				.toArray(new ProjectFile[templateFiles.size()]);
	}
	...
	public void setProjectDescriptorResolver(
	...
	public void deleteRulesProjectListener(AjaxBehaviorEvent event) {
		...
		try {
			...
			FacesUtils.addErrorMessage("Failed to delete rules project.",
					e.getMessage());
		}
	}
	...
	public TreeNode getSelectedNode() {
		TreeNode selectedNode = repositoryTreeState.getSelectedNode();
		return activeProjectNode != null
				&& selectedNode instanceof TreeRepository ? activeProjectNode
				: selectedNode;
	}
}
