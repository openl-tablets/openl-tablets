...
public class ProjectTagsMigrator {
                ...
                try (ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream()) {
                    ...
                    try (var inputStream = new ByteArrayInputStream(byteArrayOutputStream.toByteArray())) {
                        if (repository.supports().folders()) {
                            saveTagsInFolder(projectPath, repository, inputStream, migrationUser);
                        } else {
                            saveTagsInArchive(projectPath, repository, inputStream, migrationUser);
                        }
                    }
                }
            } else {
    ...
    private void saveTagsInFolder(String projectPath, Repository repository, ByteArrayInputStream inputStream, CommonUser migrationUser) throws IOException {
        ...
        if (tagsFileData == null) {
            ...
            tagsFileData.setAuthor(migrationUser == null ? null : migrationUser.getUserInfo());
    ...
    private void saveTagsInArchive(String projectPath, Repository repository, ByteArrayInputStream tagsStream, CommonUser migrationUser) throws IOException, ProjectException {
        ...
        try(FileItem projectFileItem = repository.read(projectPath)) {
            ...
            try (ZipInputStream zis = new ZipInputStream(projectFileItem.getStream());
                ...
                zos.closeEntry();
                projectFileData.setAuthor(migrationUser.getUserInfo());
