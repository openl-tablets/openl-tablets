...
public class DatatypeTableWriter extends TableWriter<DatatypeView> {
    ...
    protected void updateBusinessBody(DatatypeView tableView) {
        ...
        for (var field : tableView.fields) {
            createOrUpdateCell(tableBody, buildCellKey(TYPE_COLUMN, row), field.type);
            createOrUpdateCell(tableBody, buildCellKey(NAME_COLUMN, row), field.name);
            createOrUpdateCell(tableBody, buildCellKey(DEFAULT_VALUE_COLUMN, row), field.defaultValue);
            row++;
        }
