...
public class AddDependencyController {
    ...
    private ProjectDependencyImpl buildDependencyObject() {
        ...
        if (projectVersion1 == null) {
            FacesContext.getCurrentInstance().addMessage(null,
                    new FacesMessage("lower version format error", "expected format - X[.Y[.Z]]"));
            return null;
        }
        if (!StringUtils.isEmpty(upperVersion)) {
            ...
            if (projectVersion2 == null) {
                FacesContext.getCurrentInstance().addMessage(null,
                    new FacesMessage("upper version format error", "expected format - X[.Y[.Z]]"));
                return null;
            }
        }
        ...
        if (projectVersion2 != null && projectVersion1.compareTo(projectVersion2) > 0) {
            FacesContext.getCurrentInstance().addMessage(null, new FacesMessage("lower version is greater than upper one"));
            return null;
        }
    ...
    public static ProjectVersion versionFromString(String s) {
        if (StringUtils.isEmpty(s) ||  s.startsWith(".") || s.endsWith("..")) return null;
        ...
        try {
            ...
            if (major < 0 || minor < 0 || rev < 0) return null;
