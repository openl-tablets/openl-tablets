...
public class CellModel implements ICellModel
{
	...
	public void toHtmlString(StringBuffer buf, TableModel table)
	{
		...
		buf.append('>')
		.append("<div ")
			.append(" onMouseDown='clickCell(")
			.append(column).append(',').append(row).append(",event)'")
			.append(" id='c").append(column).append('x').append(row).append("'>")
		.append(getContent()).append("</div></td>\n");
	}
	...
	static final int FONT_COLOR_FILTER_IDX = 0, BGR_COLOR_FILTER_IDX = 1,
			BORDER_COLOR_FILTER_IDX = 2;
