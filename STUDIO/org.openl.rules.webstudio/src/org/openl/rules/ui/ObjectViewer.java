...
public class ObjectViewer {
    ...
    public String displayDoubleValueWithExplanation(DoubleValue dv, NumberFormat format,
        String clazz) {
        return "<a href=\"" + getURL(dv) + "\""
        + ((clazz == null) ? "" : (" class=\"" + clazz + "\"")) + ">" // + new
    ...
    public String getURL(DoubleValue dv) {
        ...
        return "javascript: open_explain_win(\'?rootID=" + rootID
        + "&header=Explanation')";
    }
    ...
    public String displayResult(Object res) {
        ...
        if (res instanceof IMetaHolder) {
            return displayMetaHolder((IMetaHolder) res, stringValueof(res),
        ...
        if (res instanceof GridWithNode) {
            if (projectModel != null) {
                ...
                if (index >= 0) {
                    TableModel tableModel = ProjectModel.buildModel(((GridWithNode) res).gridTable,
                            null);
    ...
    public String displaySearch(OpenLAdvancedSearchResult searchRes) {
        ...
        for (int i = 0; i < tr.length; i++) {
            ...
            if ((tsn.getTableProperties() != null)
                    && ((prop = tsn.getTableProperties().getProperty("name")) != null)) {
                tname = prop.getValue();
            } else {
            ...
            res[(2 * i) + 1] = (cg != null) ? new GridWithNode(cg.asGridTable(), tsn)
                : "No rows selected";
        }
    ...
    private String displayDTValidationResult(DTValidationResult res) {
        ...
        if ((res.getOverlappings().length == 0) && (res.getUncovered().length == 0)) {
            return makeExcelLink(gt, "<b>Table is Complete and has no Overlappings</b>",
                buf).toString();
        }
        ...
        if (res.getOverlappings().length > 0) {
            ...
            buf.append("<p>")
                .append("The following sample input data will trigger overlapping rules ");

            int[] rules = ov.getRules();
            for (int i = 0; i < rules.length; i++) {
                buf.append("<b>").append((i > 0) ? ", " : "")
                    .append(dt.getRuleName(rules[i])).append("</b>");
            }
            ...
            for (int i = 0; i < size; i++) {
                buf.append("<th class='data_header'>").append(ov.getValue().getName(i))
                    .append("</th>");
            }
            buf.append("</tr><tr>");

            for (int i = 0; i < size; i++) {
                buf.append("<td class='data_cell'>").append(ov.getValue().getValue(i))
                    .append("</td>");
            }
        ...
        if (res.getUncovered().length > 0) {
            buf.append("<p>")
                .append("The following sample input data will not trigger any rules(Undercoverage): ");
            ...
            for (int i = 0; i < size; i++) {
                buf.append("<th class='data_header'>").append(un.getValues().getName(i))
                    .append("</th>");
            }
            buf.append("</tr><tr>");

            for (int i = 0; i < size; i++) {
                buf.append("<td class='data_cell'>").append(un.getValues().getValue(i))
                    .append("</td>");
            }
    ...
    IGridFilter makeFilter(int[] rules, DecisionTable dt) {
        ...
        return ColorGridFilter.makeTransparentFilter(new RegionGridSelector(regions, true),
            0.7, 0x00ff00);
        ...
        for (int i = 0; i < nrows; i++) {
            ...
            for (int j = 0; j < hh.length; j++) {
                buf.append("<td class=\"testtable\">")
                    .append(displayResult(tt.getTestValue(hh[j], i))).append("</td>");
            }

            buf.append("<td class=\"testtable\">").append(displayResult(tt.getExpected(i)))
                .append("</td>");

            buf.append("<td  class=\"testtable\">");
            switch (tt.getCompareResult(i)) {
                case TestResult.TR_OK:
                    buf.append("<img src='../images/test_ok.gif'/>")
                        .append(displayResult(tt.getResult(i)));
                    break;
                case TestResult.TR_EXCEPTION:
                    // buf.append("<img src='../images/test_exception.gif'>");
                    displayException((Throwable) tt.getResult(i), buf);
                    break;
                case TestResult.TR_NEQ:
                    buf.append("<img src='../images/test_neq.gif'/>")
                        .append(displayResult(tt.getResult(i)));
                    break;
            }
    ...
    private StringBuffer displayException(Throwable t, StringBuffer buf) {
        if (t instanceof OpenLRuntimeException) {
            ...
            for (int i = 1; i < tt.length; i++) {
                if (tt[i] instanceof OpenLRuntimeException
                        || tt[i] instanceof SyntaxErrorException
                        || tt[i] instanceof BoundError
                        || tt[i] instanceof OpenLRuntimeExceptionWithMetaInfo) {
        ...
        if (t instanceof SyntaxErrorException) {
            ...
            for (int i = 0; i < err.length; i++) {
                displayErrorAndCode((Throwable) err[i], err[i].getLocation(),
                    err[i].getModule(), buf);
            }
    ...
    private void displayErrorAndCode(Throwable tt, ILocation srcLocation,
        IOpenSourceCodeModule module, StringBuffer buf) {
        ...
        buf.append("<a href='../showLinks.jsp?").append(url)
            .append("' target='show_app_hidden' title='").append(uri)
    ...
    public static StringBuffer makeExcelLink(IGridTable table, String text,
        StringBuffer buf) {
