...
public class TableEditorModel {
     ...
     * – constrained data entry based on the Domain of the Parameter Type (also known as
     * Subtype in Exigen Rules; the RDF/OWL uses the word Range to define what we used
     * to call Domain, and the word Domain for what we in Java call the Declaring Class
     * or Declaring Type; we need to decide which terminology we are going to use – I
     * start to lean toward RDF/OWL even though it will conflict with our Range classes
     * – will be IntRange, DoubleRange etc. – but the superclass should be renamed to
     * ArithmeticRange); the cell editors should include comboboxes for enumerations,
     * range validators for numbers, other types of specialized editors – calendars,
     * ranges; it also should allow to input formulas – similar to Condition/Action
     * formula if the first character was ‘=’.
     */
    ...
    public synchronized void insertRows(int nRows, int beforeRow) {
        IUndoableGridAction ua = IWritableGrid.Tool.insertRows(nRows, beforeRow, region,
                wgrid());
    ...
    public synchronized void insertColumns(int nCols, int beforeCol) {
        IUndoableGridAction ua = IWritableGrid.Tool.insertColumns(nCols, beforeCol,
                region, wgrid());
    ...
    public synchronized IGridTable getUpdatedTable() {
        if (isExtendedView()) {
            return new GridTable(region.getTop() - 3, region.getLeft() - 3,
                region.getBottom() + 3, region.getRight() + 3, table.getGrid());
        }
        return new GridTable(region.getTop(), region.getLeft(), region.getBottom(),
            region.getRight(), table.getGrid());
    }
    ...
    public boolean updatedTableCellInsideTableRegion(int row, int col) {
        ...
        return ((row >= 0) && (col >= 0) && (row < IGridRegion.Tool.height(region))
            && (col < IGridRegion.Tool.width(region)));
    }

    public boolean canAddRows(int nRows) {
        GridRegion testRegion = new GridRegion(region.getBottom() + 1,
                region.getLeft() - 1, region.getBottom() + 2, region.getRight() + 1);
        for (int i = 0; i < othertables.length; i++) {
    ...
    public boolean canAddCols(int nRows) {
        GridRegion testRegion = new GridRegion(region.getTop() - 1,
                region.getRight() + 1, region.getBottom() + 1, region.getRight() + 2);
        for (int i = 0; i < othertables.length; i++) {
            if (IGridRegion.Tool.intersects(testRegion, othertables[i])) {
                return false;
            }
    ...
    public CellMetaInfo getCellMetaInfo(int row, int column) {
        CellMetaInfo metaInfo = IWritableGrid.Tool.getCellMetaInfo(table.getGrid(),
                tX(column), tY(row));
...
//	System.out.println("Meta:"  +  metaInfo);
    ...
    static class RegionAction implements IUndoableAction {
         ...
         * @param r DOCUMENT ME!
         */
        void updateRegion(boolean isInsert, boolean isColumns, int rowsOrColumns,
            GridRegion r) {
    ...
    public static enum CellType {TH_CELL_TYPE,
        CA_HEADER_CELL_TYPE,
