...
//	String period;
    ...
    public String[] htmlTable(DoubleValue value) {
        ...
        if (name == null) {
            ...
            name = WebTool.urlLink("showExplainTable.jsp?uri="
                    + StringTool.encodeURL(url) + "&text=" + name, "show", name,
                    "mainFrame");
        }
...
//		String url = findUrl(value, parentUrl);
        ...
        return WebTool.urlLink(makeExpandUrl(id), (name == null) ? "expand" : name, text,
            null);
...
//		+ "&period=" + period
    ...
    protected String makeUrl(String url) {
        ...
        String ret = makeBasicUrl() + "&wbPath=" + parser.wbPath + "&wbName="
            + parser.wbName + "&wsName=" + parser.wsName + "&range=" + parser.range;
