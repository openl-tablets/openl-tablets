	...
	static class StringFinder
	{
		...
		int findToken(String token)
		{
			...
			while (true)
			{
				...
				if (idx < 0)
					return idx;
				if (idx > 0 && Character.isLetterOrDigit(src.charAt(idx - 1))||  idx + tlen < src.length()
...
//!!!		short[] color = fontFilter == null ? font.getFontColor() : fontFilter.filterColor(font.getFontColor());
	...
	static public String makeXlsOrDocUrl(String url)
	{
		...
		String ret = "" + "&wdPath=" + wdparser.wdPath + "&wdName="
				+ wdparser.wdName + "&wdParStart=" + wdparser.wdParStart
				+ "&wdParEnd=" + wdparser.wdParEnd;
