...
public class ProjectModel implements IProjectTypes {
    ...
    private XlsWorkbookSourceCodeModule getWorkbookSourceCodeModule() {
        if (wrapper != null) {
            TableSyntaxNode[] nodes = ((XlsMetaInfo) wrapper.getOpenClass()
                    .getMetaInfo()).getXlsModuleNode().getXlsTableSyntaxNodes();
            for (TableSyntaxNode node : nodes) {
                if (node.getType().equals(ITableNodeTypes.XLS_DT)) {
                    return ((XlsSheetSourceCodeModule) node.getModule())
                            .getWorkbookSource();
                }
            }
        }
        ...
        if (savedSearches == null && isReady()) {
            ...
            for (TableSyntaxNode node : nodes) {
                if (node.getType().equals(ITableNodeTypes.XLS_PERSISTENT)) {
                    ...
                    if ((IXlsTableNames.PERSISTENCE_TABLE + " " + OpenLSavedSearch.class
                            .getName()).equals(code)) {
                        OpenLSavedSearch savedSearch = new OpenLSavedSearch()
                                .restore(new XlsSheetGridImporter(
                                        (XlsSheetGridModel) node.getTable()
                                                .getGridTable().getGrid(), node));
            ...
            this.savedSearches = savedSearches
                    .toArray(new OpenLSavedSearch[savedSearches.size()]);
        }
