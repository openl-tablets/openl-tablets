...
public class ObjectViewer
{
	...
	public String displayDoubleValueWithExplanation(DoubleValue dv,
			NumberFormat format, String clazz)
	{
	...
	public String displaySearch(OpenLAdvancedSearchResult searchRes)
	{
		...
		OpenLAdvancedSearchResultViewer sviewer =
		 new OpenLAdvancedSearchResultViewer(searchRes);
		
		for (int i = 0; i < tr.length; i++)
		{
			res[2*i] = tr[i].getTsn().getHeaderLineValue();
	...
	private String displayDTValidationResult(DTValidationResult res)
	{
		...
		if (res.getOverlappings().length > 0)
		{
			...
			buf.append("<p>").append(
					"The following sample input data will trigger overlapping rules ");

			int[] rules = ov.getRules();
			for (int i = 0; i < rules.length; i++)
			{
				buf.append("<b>").append(i > 0 ? ", " : "").append(
						dt.getRuleName(rules[i])).append("</b>");
			}
		...
		if (res.getUncovered().length > 0)
		{
			buf
					.append("<p>")
					.append(
							"The following sample input data will not trigger any rules(Undercoverage): ");
			...
			for (int i = 0; i < size; i++)
			{
				buf.append("<th class='data_header'>")
						.append(un.getValues().getName(i)).append("</th>");
			}
			buf.append("</tr><tr>");
	...
	private String displayTestResult(TestResult tt)
	{
		...
		for (int i = 0; i < nrows; i++)
		{
			...
			{
				buf.append("<td class=\"testtable\">").append(
						displayResult(tt.getTestValue(hh[j], i))).append("</td>");
			}

			buf.append("<td class=\"testtable\">").append(
					displayResult(tt.getExpected(i))).append("</td>");

			buf.append("<td  class=\"testtable\">");
			switch (tt.getCompareResult(i))
			{
			case TestResult.TR_OK:
				buf.append("<img src='../images/test_ok.gif'/>").append(
						displayResult(tt.getResult(i)));
				break;
			...
			case TestResult.TR_NEQ:
				buf.append("<img src='../images/test_neq.gif'/>").append(
						displayResult(tt.getResult(i)));
				break;
			}
	...
	private StringBuffer displayException(Throwable t, StringBuffer buf)
	{
		...
		if (t instanceof SyntaxErrorException)
		{
			...
			{
				displayErrorAndCode((Throwable) err[i], err[i].getLocation(), err[i]
						.getModule(), buf);
			}
	...
	static public StringBuffer printCodeAndErrorToHtml(ILocation location,
			IOpenSourceCodeModule module, StringBuffer buf)
	{
		...
		} else
		{
			...
			pend = Math.min(location.getEnd().getAbsolutePosition(info) + 1, src
					.length());
		}
	...
	public String displayShortParamValue(Object x)
	{
		...
		try
		{
			return String2DataConvertorFactory.getConvertor(x.getClass()).format(x,
					null);
		} catch (Throwable t)
		{
		}
