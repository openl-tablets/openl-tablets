...
public class XSSFWorkbook extends POIXMLDocument implements Workbook, Iterable<XSSFSheet> {
    ...
    /**
     * array of external sheets for this workbook
     */
    private List<ExternalSheet> extSheets = null;
    ...
    public int getSheetIndex(String name) {
        ...
        for (int i = 0 ; i < sheets.size() ; ++i) {
            ...
            if (name.equalsIgnoreCase(sheet.getSheetName().trim())) {
                return i;
            }
        }
        return -1;
    ...
    */
    ...
    public ExternalSheet getExternalSheet(int externSheetIndex) {
        if(extSheets != null && externSheetIndex < extSheets.size() )
            return extSheets.get(externSheetIndex);
        else {    // Return null to prompt resolver External sheet throw internal (I do not know why but POI sometimes consider internal sheet as "external" )
            return null;
        }
    }
    private ExternalSheet createExternalSheet(String workbookName, String sheetName){
        if(extSheets == null){
            extSheets = new ArrayList<ExternalSheet>();
        }
        ExternalSheet extSheet = new ExternalSheet(workbookName, sheetName); 
        if(!extSheets.contains(extSheet)){
            extSheets.add(extSheet);
        }
        return extSheet;
    }
    
    public int getExternalSheetIndex(String workbookName, String sheetName) {
        ExternalSheet extSheet = createExternalSheet(workbookName, sheetName);
        return extSheets.indexOf(extSheet);
    }
    
    public void listExternalReferences(){
        ...
        for(int i=0;i<array.size();i++){
            PackagePart p = array.get(i);
            try {
                PackageRelationshipCollection rels = p.getRelationships();
                for (PackageRelationship rel : rels) {
                    if(rel.getTargetMode() == TargetMode.EXTERNAL){
                        System.out.println("New abs path = "+rel.getTargetURI());
                    }
