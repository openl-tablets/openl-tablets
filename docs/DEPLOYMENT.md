...
- **Rule Services**: REST/SOAP services for rule execution
...
| **Rule Service** | 8080 | HTTP/HTTPS | REST/SOAP APIs |
...
docker pull openltablets/rule-service:latest
...
docker run -d \
  ...
  -e SPRING_DATASOURCE_USERNAME=openl \
  -e SPRING_DATASOURCE_PASSWORD=openl \
  openltablets/rule-service:latest
```
