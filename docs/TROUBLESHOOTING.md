...
docker run -it openltablets/studio:latest /bin/sh

# 3. Check entrypoint
docker inspect openltablets/studio:latest | grep -A 5 Entrypoint
```
...
docker run -it --entrypoint /bin/bash openltablets/studio:latest

# 2. Check environment variables
docker run --rm openltablets/studio:latest env

# 3. Mount logs volume
docker run -v /tmp/logs:/var/log/openl openltablets/studio:latest
```
...
docker run --network openl-network --name studio openltablets/studio:latest
...
docker pull openltablets/studio:latest
