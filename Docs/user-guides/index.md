# User Guides

Welcome to the OpenL Tablets User Guides! This section contains comprehensive documentation for users working with OpenL Tablets.

## Available Guides

### 📦 [Installation Guide](installation/index.md)
Learn how to install and set up OpenL Tablets in your environment.

- System requirements
- Installation steps
- Initial configuration
- Troubleshooting installation issues

### 🎯 [Demo Package Guide](demo-package/index.md)
Get started quickly with the OpenL Tablets demo package.

- Running the demo
- Exploring sample rules
- Understanding the demo structure
- Building on the demo examples

### 📚 [Reference Guide](reference/index.md)
Comprehensive reference documentation for OpenL Tablets features and syntax.

- Table types and syntax
- Data types and functions
- Rule syntax reference
- Best practices

### 🖥️ [OpenL Studio User Guide](webstudio/index.md)
Master the OpenL Studio interface.

- OpenL Studio overview
- Creating and editing rules
- Testing and debugging
- Project management
- Deployment workflows

### ⚙️ [Rule Services Guide](rule-services/index.md)
Learn how to deploy and use OpenL Tablets rules as web services.

- Deploying rules as services
- REST APIs
- Service configuration
- Monitoring and management
- Performance optimization

---

## Quick Links

- **Getting Started**: New to OpenL Tablets? Start with the [Installation Guide](installation/index.md)
- **Developers**: See [Developer Guides](../developer-guides/index.md)
- **Integration**: Check [Integration Guides](../integration-guides/index.md)
- **Configuration**: Visit [Configuration](../configuration/index.md)

---

## Support

Need help? Check our [Troubleshooting Guide](../onboarding/troubleshooting.md) or visit the [GitHub Issues](https://github.com/openl-tablets/openl-tablets/issues).

---

**Note**: This documentation is being actively migrated and improved. Some sections may be under construction. See [Migration Plan](../MIGRATION_PLAN.md) for status.
