...
Users can work with projects loaded in Repository in the Demo package and create their own new projects. For more information on how to manage projects in Repository, see [OpenL Studio Guide > Managing Projects](../../openl-studio/repository-editor.md#managing-projects).
...
OpenL Studio is pre-configured in single-user mode, so all projects in the Repository are created and modified by the single user, which is the operating system account that runs OpenL Studio.
