...
- A folder with `rules.xml` is always a project. A folder without it is a project when an Excel file is present in the
  folder root and the global `project.detect-by-excel-files` setting is enabled. The setting defaults to
  `false`, applies to every design repository, and must be retained by every branch-scoped mapped view.
