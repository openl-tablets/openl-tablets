...
-	**$Expr.$C1.param1** is an expression defined as a value in a column for the **param1** condition parameter. $C1 is optional. For instance, in the example below, parameter **cond** is the condition parameter for condition C2. It's important to use named parameters which is possible in decision tables of [Rules type](decision-table-overview-and-structure.md#decision-table-structure) or when working with [external conditions, actions, or returns](external-tables-usage-in-smart-decision-tables.md#external-tables-usage-in-smart-decision-tables) in smart tables.
