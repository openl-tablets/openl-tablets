...
- **Converged**: every queue row and vein is done, and `main` has yielded nothing deletable for many runs. Screened
  and closed up to *Give the DEMO URL check one deadline so it prints the demo log* — match that **subject** in
...
- 08-20/21 — runs 229-241: idle; new `main` work was a spotless bump plus three EPBDS-16473 start-up fixes, whose
  only deletions were a Lombok constructor with its field and Mockito wiring, both in-file. No `dead-code/*` PR open.
