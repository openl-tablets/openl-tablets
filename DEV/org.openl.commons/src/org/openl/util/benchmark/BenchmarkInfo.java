...
public class BenchmarkInfo
{
    ...
    public static BenchmarkOrder[] order(BenchmarkInfo[] bi)
    {
    	...
    	Collections.sort(list);
  		BenchmarkOrder[] bo = (BenchmarkOrder[])list.toArray(new BenchmarkOrder[0]);
...
//      return   printDouble(avg()) + " ms/run" + '\t' + printLargeDouble(1000/avg()) + "runs/sec"  + " \t" + runs + " \t" + unitDescription() + " 1: " + printDouble(firstRunms) ;
      return  msrun()  + " ms/run" + '\t' +  runssec() + " runs/sec"  + " \t" +" First Run: " + printDouble(firstRunms) + "ms";
    }
    ...
    public double avg()
    {
        ...
        {
          RunInfo run = (RunInfo) runs.elementAt(i);
          n += run.times;
          sum += run.ms;
        }
