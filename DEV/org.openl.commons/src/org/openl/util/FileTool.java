...
public final class FileTool
{
	...
	public static File buildRelativePath(File startDir, File targetFile)
		throws IOException
	{
		if (startDir == null)
	...
	static File[] parents(File f) throws IOException
	{
		...
		for (int i = 0; i < size; i++)
		{
			ff[i] = (File) v.elementAt(size - 1 - i);
		}
	...
	public static void copyFile(File src, File dst) throws IOException
	{
		...
		dstCan.getParentFile().mkdirs(); // make sure that parent directory exists
	...
	static public void copyDir(String srcDir, final String dstDir, FilenameFilter filter)
		throws Exception
	{
	...
	static public class CacheMap
	{
		...
		{
			LTMValue ltm = (LTMValue) _map.get(key);
	...
	public static void saveFile(OutputStream os, String s) throws Exception
	{
		...
		try
		{
			fw =
				new BufferedWriter(
					new OutputStreamWriter(os, DEFAULT_CHARACTER_ENCODING));
			fw.write(s);
		}
		finally
		{
	...
	public static void saveFile(String fname, String s, boolean compareBeforeSave)
		throws Exception
	{
		try
		{
	...
	public synchronized static String loadFile(InputStream fs) throws Exception
	{
		...
		{
			bis =
				new BufferedReader(new InputStreamReader(fs, DEFAULT_CHARACTER_ENCODING));
	...
	public synchronized static int compareFileSource(String fname, String src)
		throws Exception
	{
		...
		try
		{
			...
			bis =
				new BufferedReader(
					new InputStreamReader(fis, DEFAULT_CHARACTER_ENCODING));
	...
	public synchronized static int compareFileSource(String fname, byte[] src)
		throws Exception
	{
		if (!(new File(fname).isFile()))
		{
...
//		Log.warn("Removing: "+path);
	...
	*/
	private static void removeChildren(File dir)
	{
		...
		{
			if (file[i].isDirectory()) removeChildren(file[i]);
...
static public class DirectoryIterator
{
  ...
  public DirectoryIterator(String root, String[] extensions, Comparator file_comparator)
  {
    this(root, new ExtensionFilter(extensions), file_comparator);
  }


  public DirectoryIterator(String root, FilenameFilter filter, Comparator file_comparator)
  {
    _root = root;
    _filter = filter;
  ...
  void iterateDirectory(String dir_path, String local_path, FileHandler fh, DirectoryHandler dh)
      throws Exception
  {
    ...
    {
      String new_local_path = local_path.length() == 0 ? files[i].getName() : local_path + File.separator + files[i].getName();
      if (files[i].isDirectory())
      {
        iterateDirectory(dir_path + File.separator + files[i].getName(),
                          new_local_path,
                          fh, dh);
      }
      else
