...
public final class FileTool
{
    ...
    {
	if (startDir == null)
	    return targetFile.isFile() ? targetFile.getParentFile()
		    : targetFile;
    ...
    {
	copyFile(src, new File(dstDir.getPath() + File.separator
		+ src.getName()));
    }
    ...
    static public void copyDir(String srcDir, final String dstDir,
    ...
    public static void saveFile(String fname, String s,
	    boolean compareBeforeSave) throws Exception
    {
	try
	{
	...
	try
	{
	    bis = new BufferedReader(new InputStreamReader(fs,
		    DEFAULT_CHARACTER_ENCODING));
    ...
    public synchronized static byte[] loadFileToByteArray(String fname)
	    throws Exception
    {
	FileInputStream fis = null;
	BufferedInputStream bis = null;
    ...
    public synchronized static byte[] loadFileToByteArray(InputStream fis)
	    throws Exception
    {
	BufferedInputStream bis = null;
	...
	public DirectoryIterator(String root, String[] extensions,
		Comparator<File> file_comparator)
	{
	...
	public void iterate(FileHandler fh, DirectoryHandler dh)
		throws Exception
	{
	    iterateDirectory(_root, "", fh, dh);
	}

	void iterateDirectory(String dir_path, String local_path,
		FileHandler fh, DirectoryHandler dh) throws Exception
	{
	    ...
	    {
		String new_local_path = local_path.length() == 0 ? files[i]
			.getName() : local_path + File.separator
			+ files[i].getName();
		if (files[i].isDirectory())
		{
		    iterateDirectory(dir_path + File.separator
			    + files[i].getName(), new_local_path, fh, dh);
		} else
		{
		    if (fh != null)
	...
	static class ExtensionFilter implements FilenameFilter
	{
		...
		if (new File(dir.getPath() + File.separator + name)
			.isDirectory())
		    return true;
