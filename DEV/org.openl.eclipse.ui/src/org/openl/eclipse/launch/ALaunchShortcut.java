...
/**
 * TODO re-think: use ILaunchTarget instead of Object as target. 
 * TODO re-think: is IResource suitable for all cases? How about IStorage?
 * Generic implementation of the ILaunchShortcut:
 *  - Find targets that can be launched within selection
 *  - Find configurations or create new that launch found targets
 *  - Launch above configurations
 * 
 * @author sam
 */
...
{
  /**
   * Returns selector: <code>launch-target-is-of-my-type</code>.
   * For example, java compilation unit with main(String[]) method.
   */
  abstract public ISelector selectLaunchTargetType();

  /**
   * Returns collector: object to launchTarget.
   * For example, IResource to OpenlLaunchTarget.
   */
  ...
  public void launch(ILaunchRequest request)
  {
    ...
    boolean ok =
      DebugUITools.saveAndBuildBeforeLaunch()
        && (targets = getLaunchTargets(request)).length > 0
  ...
  ILaunchGroup getLaunchConfigurationGroup(
    ILaunchConfiguration configuration,
    ILaunchRequest request)
  {
...
//    }
    ...
    return (ILaunchGroup)group;
  }

  boolean previewConfiguration(
    ILaunchConfiguration configuration,
    ILaunchRequest request)
  {
    String groupId =
      getLaunchConfigurationGroup(configuration, request).getIdentifier();

    return DebugUITools.openLaunchConfigurationPropertiesDialog(
      getShell(),
      configuration,
      groupId)
      == Window.OK;
  ...
  public ILaunchTarget[] getLaunchTargets(ILaunchRequest request)
  {
    ILaunchTarget[] targets =
      (ILaunchTarget[])request
        .getSelection()
        .select(selectLaunchTargetType())
        .collect(asLaunchTarget())
        .select(NOT_NULLS)
        .asList()
        .toArray(NO_TARGETS);
  ...
  /**
   * Shows a selection dialog for launch targets.
   * Returns chosen targets or <code>null</code> if user cancelled the dialog.
   */
  public ILaunchTarget[] chooseLaunchTargets(
    ILaunchRequest request,
  ...
  /**
   * Returns configurations that launch given targets.
   * Creates default configuration when target has no configuration.
   */
    ...
    ILaunchTarget[] targets)
  {
    ILaunchConfiguration[] configurations =
      getExistingLaunchConfigurations(request, targets);
    ...
    if (tc.x_Y().isF())
    {
      for (int i = 0; i < targets.length; i++)
      {
        ...
        {
          ILaunchConfiguration configuration =
            targets[i].createDefaultLaunchConfiguration(request);
      ...
      configurations =
        (ILaunchConfiguration[])tc.y_X().X().toArray(NO_CONFIGURATIONS);
    }
    else
    {
      configurations =
        chooseLaunchConfigurations(request, configurations, false);
    }
   ...
   */
  protected ILaunchConfiguration[] getExistingLaunchConfigurations(
    ILaunchRequest request,
    ILaunchTarget[] targets)
  {
    try
    {
      return (ILaunchConfiguration[])getLaunchConfigurations()
        .select(
          launchConfigurationForTargets(targets).and(
            launchConfigurationSupportsMode(request.getLaunchMode())))
        .asList()
        .toArray(NO_CONFIGURATIONS);
    }
  ...
  /**
   * Dialog to choose one of the given launch configurations.
   * Returns chosen configuration or <code>null</code> if user cancelled the dialog.
   */
  public ILaunchConfiguration[] chooseLaunchConfigurations(
    ILaunchRequest request,
    ILaunchConfiguration[] launchConfigurations,
    boolean multipleSelection)
  {
    return (ILaunchConfiguration[])selectionDialog(
      launchConfigurations,
      getString(MSG_LAUNCH_CONFIG_SELECTION_TITLE),
      getFormattedString(
        MSG_LAUNCH_CONFIG_SELECTION_MESSAGE,
        request.getLaunchMode()),
      multipleSelection);
  }
  ...
  public ILaunchRequest createLaunchRequest(ISelection selection, String mode)
  {
    return selection instanceof IStructuredSelection
      ? new LaunchRequest((IStructuredSelection)selection, mode)
      : null;
  }
  ...
  public ISelector launchConfigurationForTarget(final ILaunchTarget target)
  {
    return new ASelector()
    {
      ...
      {
        return o instanceof ILaunchConfiguration
          && target.isLaunchedBy((ILaunchConfiguration)o);
      }
    };
  }

  /**
   * Returns selector "one of the targets is launched by the configuration in question".
   */
