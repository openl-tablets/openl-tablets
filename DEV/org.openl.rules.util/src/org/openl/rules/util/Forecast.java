...
public final class Forecast {
    ...
    /**
     * Calculates a forecasted Y value for a given Float X value using linear regression.
     *
     * @param x      the X value to forecast for
     * @param knownY the array of known Y values used for regression
     * @param knownX the array of known X values used for regression
     * @return the forecasted value as a Float, or null
     */
    public static Float forecast(Float x, Float[] knownY, Float[] knownX) {
        InputStatsFloat inputStats = loadInputStats(knownY, knownX);
        if (inputStats == null || x == null) {
            return null;
        }
        Float slopeB = Slope.slope(inputStats);
        var a = slopeB != null ? inputStats.getAvgY() - (slopeB * inputStats.getAvgX()) : null;
        return slopeB == null ? null : a + (slopeB * x);
    }
