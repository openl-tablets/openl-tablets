  ...
  static public void main (String[] argv){
    ...
    try{
      ...
      for (int i =0 ; i<9;i++){
         ...
         lp.setColumnBounds(i,VariableType.BOUNDED_BELOW,0,0); //all variables are bounded below
      }
      // advertising values
      lp.addColumns(3);
      for (int i = 9 ; i<=11;i++){
    ...
    for (int i=0;i<oilTypes.length;i++){
      for (int j=0;j<gasTypes.length;j++){
        lp.setColumnCoeff(i*gasTypes.length + j,
      ...
      //System.out.println(lp.errorAsString(lp.getProblemStatus()) + lp.getProblemStatus());
