...
public class TestIntExpConst extends TestCase {
  ...
  public TestIntExpConst(String name) {super(name);}
  ...
  public void testAdd(){
    ...
    assertEquals(1, (int)((Double)coeffs.get(testIntVar)).doubleValue());
      ...
      }catch(Failure f){fail("test failed due to incorrect work of ");}
  ...
  public void testMul(){
    ...
    assertEquals(10,((Double)(coeffs.get(testFloatVar))).doubleValue(), Constrainer.FLOAT_MIN);
      //event propagation
        ...
        }catch(Failure f){fail("test failed due to incorrect behaviour of FloatExp.setMax(double)");}
        //ascending
        try{
          double newmax = (4.1);
    ...
    coeffs = new HashMap();
    try{
      assertEquals("IntExpConst mul IntExpConst works wrong: calcCoeffs problems", 10*(-10), (int)intExp.calcCoeffs(coeffs));
    }catch(NonLinearExpression ex){/*it will never happen*/}
    assertTrue(coeffs.isEmpty());
    ...
    assertTrue("Zero valued IntExpConst mul IntExpConst works wrong: the expression isn't Linear",intExp.isLinear());
    coeffs = new HashMap();
    try{
      assertEquals("Zero valued IntExpConst mul IntExpConst works wrong: calcCoeffs problems", 0, (int)intExp.calcCoeffs(coeffs));
    }catch(NonLinearExpression ex){/*it will never happen*/}
    assertTrue(coeffs.isEmpty());
