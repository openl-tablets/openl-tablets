...
import com.exigen.ie.constrainer.impl.TestUtils;
...
public class TestIntVarSelectors extends TestCase {
  ...
  public TestIntVarSelectors(String name){super(name);}
  ...
  public void testIntVarSelectorMinSize(){
    ...
    }catch(Failure f){fail("test failed due to incorrect work of Constrainer.allDiff(IntExpArray)");}
    boolean flag = C.execute(new GoalGenerate(array,
                                              new IntVarSelectorMinSize(array),
  ...
  public void testIntVarSelectorFirstUnbound(){
    ...
    }catch(Failure f){fail("test failed due to incorrect behaviour of IntVar.setValue(int)");}
  }
