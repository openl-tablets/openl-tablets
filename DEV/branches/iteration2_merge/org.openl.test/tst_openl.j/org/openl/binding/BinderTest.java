...
public class BinderTest extends TestCase
{
		...
		throws OpenConfigurationException
	{
		...
		IParsedCode pc =
			op.getParser().parseAsModule(
				new StringSourceCodeModule(testCode, null));
		...
		throws OpenConfigurationException
	{
		...
		IParsedCode pc =
			op.getParser().parseAsMethodBody(
				new StringSourceCodeModule(testCode, null));
	...
	public void testBind() throws OpenConfigurationException
	{
		...
		_testNoErrorModule(
			"int foo(int x){return x+y+5;} int y= 3;",
			DynamicObject.class,
			"org.openl.j");
		...
		int numPar)
	{
		IOpenMethodHeader header =
			OpenlTool.getMethodHeader(
				new StringSourceCodeModule(code, null),
				OpenL.getInstance(openl), null);
		Assert.assertEquals(type, header.getType());
		Assert.assertEquals(
			numPar,
			header.getSignature().getParameterTypes().length);
	}
