...
public class DTCondition extends FunctionalRow implements IDTCondition
{
	...
	/**
	 * Since version 4.0.2 the Condition can have an optimized form. In this case
	 * the Condition Expression(CE) can have type that is different from boolean.
	 * For details consult DTOptimizedAlgorithm class
	 * 
	 ...
	 * <p>
	 * 2) Check if the expression depends on any of the parameters, if it does, it
	 * is not intended to be optimized (at least not in this version). In this
	 * case it has to have <code>boolean</code> type.
	 * <p>
	 ...
	 * <p>
	 * 4) Attach, expression/optimization to the Condition; change the expression
	 * header to remove CP, because they are not needed.
	 * 
	 * @see DTOptimizedAlgorithm
	 */

	public IDTConditionEvaluator prepareCondition(IMethodSignature signature,
	...
	IMethodCaller makeOptimizedConditionMethodEvaluator(IMethodSignature signature)
	{
		...
		String code = method.getMethodBodyBoundNode().getSyntaxNode().getModule()
				.getCode();
