...
public class DecisionTableBuilder extends TableBuilder {
     ...
     * That is actions & conditions title, logic, parameter declarations,
     * paramater business names.   
     */
    ...
    public void writeElement(String title, String logic, String[] parameterNames, String[] parameterSignatures) {
        ...
        if (parameterNames == null || parameterNames.length == 0) {
            throw new IllegalArgumentException(
                    "parameterNames must be not null array of positive length");
        }
        if (parameterSignatures == null || parameterSignatures.length == 0) {
            throw new IllegalArgumentException(
                    "parameterSignatures must be not null array of positive length");
        }
        if (parameterSignatures.length != parameterNames.length) {
            throw new IllegalArgumentException(
                    "numbers of parameter names and parameter signatures must be equal");
        }
