...
public class LogicalTable extends ALogicalTable implements ILogicalTable
{
	...
	// v.add(new LogicalTable(new GridTableRows(getGridTable(), i, i + cellHeight
	// - 1)));
	// }
	//			
	...
	protected ILogicalTable columnsInternal(int from, int to)
	{
		...
		return LogicalTable.logicalTable(gridTable.columns(columnOffset[from],
				columnOffset[to + 1] - 1));
	}
	...
	protected ILogicalTable rowsInternal(int from, int to)
	{
		...
		return LogicalTable.logicalTable(gridTable.rows(rowOffset[from],
				rowOffset[to + 1] - 1));
	}
	...
	public ILogicalTable getLogicalRegionInternal(int column, int row, int width,
			int height)
	{
		...
		//		
		// return new
		// LogicalTable((IGridTable)gridTable.getLogicalRegion(gridColumn, gridRow,
		...
		return LogicalTable.logicalTable(gridTable.getLogicalRegion(startColumn,
				startRow, endColumn - startColumn, endRow - startRow));
	}
	...
	static public ILogicalTable logicalTable(ILogicalTable table)
	{
		...
		if (width == gridTable.getLogicalWidth()
				&& height == gridTable.getLogicalHeight())
			return gridTable;
	...
	/**
	 * This method will produce a logical table defined by 2 tables: leftRows and
	 * topColumns Both tables are logical tables. Rows in a new table will be
	 * defined by rows in leftRows table, and columns by the columns topColumns
	 * table. "Left" and "top" points to relative location of defining tables. It
	 * should be used only with "normal" orientation
	 * 
	 * @param leftRows
	 * @param topColumns
	 * @return
