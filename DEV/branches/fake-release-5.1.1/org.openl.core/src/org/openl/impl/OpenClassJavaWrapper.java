...
public class OpenClassJavaWrapper {
	...
	static public OpenClassJavaWrapper createWrapper(String openlName,
			IUserContext ucxt, IOpenSourceCodeModule src) {
		OpenL openl = OpenL.getInstance(openlName, ucxt);

		CompiledOpenClass openClass = openl.compileModuleWithErrors(src);

		return new OpenClassJavaWrapper(openClass, openl.getVm()
				.getRuntimeEnv());
	}

	@SuppressWarnings("unchecked")
	public static OpenClassJavaWrapper createWrapper(String name,
			IUserContext ucxt, String __src, String srcClass) {
		if (srcClass == null)
	...
	static public OpenClassJavaWrapper createWrapper(String openlName,
			IUserContext ucxt, String srcFile) {
		OpenL openl = OpenL.getInstance(openlName, ucxt);

		IOpenSourceCodeModule src = null;

		String fileOrURL = PropertiesLocator.locateFileOrURL(srcFile, ucxt
				.getUserClassLoader(), new String[] { ucxt.getUserHome() });
