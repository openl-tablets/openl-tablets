...
import com.exigen.ie.constrainer.ChoicePointLabel;
import com.exigen.ie.constrainer.Constrainer;
import com.exigen.ie.constrainer.Failure;
import com.exigen.ie.constrainer.FailureLimitException;
...
import com.exigen.ie.constrainer.Session;
...
public class TestExecutionControl extends TestCase{
  ...
  public TestExecutionControl(String name) {super(name);}
  public static void main(String[] args) {
    TestRunner.run(new TestSuite(TestExecutionControl.class));
    double d = Constrainer.FLOAT_MAX;
  }

  private void prepareChessProblem(com.exigen.ie.constrainer.Constrainer C, int num) throws Failure{
  ...
  public void testCloseAndRestoreSession(){
    ...
    try{ prepareChessProblem(C, 525);}
    catch(Failure f){fail("Constrainer: internal error occured");};
    C.setTimeLimit(2);
    ...
    Session s = new Session(C, arr, null,
  ...
  public void testMinimizeRestoration(){
    ...
    try{
      ...
      for (int switcher=0;switcher<2;switcher++){
        ...
        if (switcher == 0){
          s = new Session(C, vars, null,
                          new GoalMinimize(new GoalGenerate(take), costFunc.neg(), false));
          s.getConstrainer().setTimeLimit(9);
        }
        else{
          s = new Session(C, vars, null,
                          new GoalFastMinimize(new GoalGenerate(take), costFunc.neg(), false));
        ...
        while (true){
          try{
            ...
            try{
              ...
              java.io.ObjectInputStream in = new java.io.ObjectInputStream(
                  new java.io.FileInputStream(_fileName));
              s = new Session(in);
          ...
          //Goal solve = new GoalMinimize(new GoalGenerate(take), costFunc.neg(), true);
  ...
  public void testObtainingSuccessiveSolutions(){
    ...
    class GoalGenerateSuccessive extends GoalImpl{
      public GoalGenerateSuccessive(){super(C);}
      public Goal execute() throws Failure{
        Goal g = new GoalAnd(new GoalGenerate(x, new IntVarSelectorMinSizeMin(x), null),
                         new GoalPrintSolution(x),
                         new GoalFailureAndExit(C)
                        );
        return g;
      }
    }
    ...
    while(true){
      try{
        ...
        break;
      }catch(FailureLimitException fEx){C.setFailuresLimit(0);}
    }
