...
//		ILogicalTable paramTable = conditionRow.getLogicalColumn(PARAM_COLUMN);
//
//		ILogicalTable valuesTable = conditionRow.columns(DATA_COLUMN);
	...
	void loadRow(int row, ILogicalTable table)
	{
		...
		String headerStr = table.getLogicalRow(row).getGridTable().getStringValue(
				INFO_COLUMN, 0);
		if (headerStr == null)
			return;
	...
	int countConditionsAndActions(ILogicalTable t)
	{
		...
		for (int i = 0; i < w; i++)
		{
			String s = t.getLogicalColumn(i).getGridTable()
					.getStringValue(0, 0);
			...
			if (s != null && s.length() >= 2)
			{
				cnt += (s.charAt(0) == 'C'
						|| s.charAt(0) == 'A')
						&& Character.isDigit(s.charAt(1)) ? 1 : 0;
			}
		}
	...
	public DecisionTable load(TableSyntaxNode tsn, DecisionTable dt,
			...
			throws Exception
	{
		...
		dt.bindTable((IDTCondition[]) conditions.toArray(new IDTCondition[0]),
				(IDTAction[]) actions.toArray(new IDTAction[0]),
				ruleRow,
				//			null,
