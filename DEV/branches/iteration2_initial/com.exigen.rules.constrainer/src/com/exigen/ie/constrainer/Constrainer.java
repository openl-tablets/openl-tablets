...
/**
 * An implementation of the Constrainer - a placeholder
 * for all variables, constraints, and search goals of the problem.
 * <p>
 * The Constrainer is a Java package for modeling and solving
 * different constraint satisfaction problems.
 *
 * A problem is represented in terms of the decision variables
 * and constraints, which define relationships between these variables.
 *
 * The decision variables could be represented in form of Java objects
 * which may use the predefined constrained variables such as IntVar.
 *
 ...
 * using objects called Goals as building blocks. The Constrainer supports a reversible
 * environment with multiple choice points: when constraints/goals fail, the Constrainer
 * automatically backtracks to a previous choice point (if any).
 * <p>
 ...
 * <li> Interface Subject - a base-class for constrained variables. Contains the major methods to allow
 * constraints (observers) observe the modification of the variables (subjects).
...
GOALS EXECUTION.
There are two major stacks: execution stack "EXE" and alternative stack "ALT".
At each choice point we create a new reversibility stack "REV".
EXE.push(goal);
while(!EXE.empty())
{
  execute(EXE.pop());
  Goal execution could:
  - push new subgoal on EXE (GoalAnd)
  - push goals on ALT (GoalOr)
  - fail.
  When failed:
  - pop from EXE all goals pushed on it after the last choice point (done via marker)
  - if ALT.empty, FAILURE!
  - EXE.push(ALT.pop())
}
SUCCESS!
...
{
  static public final double FLOAT_MAX = 1.79769313486231570815e+308; // IEEE 754
  static public final double FLOAT_MIN = 2.225073858507202e-308; // the smallest positive IEEE 754
...
//  private EventOfInterest     _current_event_of_interest;
 ...
 /**
   ...
   * for all other constrained objects, constraints, and goals. Each
   * problem should define at least one Constrainer object. All other
   * objects relate to this object.
   *
...
/*
  ReusableFactory undoSubjectFactory()
  {
    return _undo_subject_factory;
  }
*/
...
/*==============================================================================
  ...
  ============================================================================*/
  /**
   * Adds a constraint to the Constrainer. Note: the constraint
   ...
   *
   * Constraint is given in the form of boolean expression. Note:
   * the constraint added by this function should be posted afterwards
   * by {@link #postConstraints()}
   *
   * @param ctExp The IntBoolExp to be added to the constrainer as a constraint.
   *
  ...
  public FastVector constraints()
  {
    ...
    * constraint.The constraint is not added to internal constraint
    * storage (with which {@link #addConstraint(Constraint)} and
    * {@link #postConstraints()} operates) instead it is just
    * activated immediately.
    *
    * @param ct The constraint to be posted/executed.
    * @throws Failure When the constraint can not be posted that is the
    * constraint is incompatible with the previously activated ones.
    */
...
//    return ok;
   ...
   /**
    * Activates the passed constraint. This method executes the specified
    * constraint provided in the form of boolean expression.
    * The constraint is not added to internal constraint storage (with
    * which {@link #addConstraint(Constraint)} and {@link #postConstraints()}
   ...
   *
   * @throws Failure When the constraints added with {@link #addConstraint(Constraint)}
   * are incompatible with the previously activated ones or together so they
   * can not be posted.
   */
   ...
   * of java.lang.Object.
   * All constraints are being executed immediately.
   *
   * @param constraints An array of Constraints to post/execute.
   * @throws Failure When some constraint cannot be posted (constraint is incompatible
   * with others)
   */
...
//    return ok;
  ...
  /**
   * @return A goal that executes a set of constraints.
   * Or null if there are no constraints to post.
   *
...
//    addInternalObjectToSymbolicContext(var);
   ...
   * <br><b>Note:</b>Constrainer's users should not use this method.
   */
  public IntVar addIntVarTraceInternal(int min, int max, String name, int type, int trace)
  {
    IntVar var = trace != 0 ?
         new IntVarImplTrace(this,min,max,name, type, trace) :
...
//    IntVar var = addIntVar(exp.min(),exp.max(),exp.name());
    IntVar var = addIntVar(exp.min(),exp.max()); // no name to avoid duplicate name
    try
    {
      var.equals(exp).post();
    }
...
//    addInternalObjectToSymbolicContext(var);
  ...
  /**
   * Adds an internal constrained boolean variable to the Constrainer.
   * Used in expressions that create internal
   * variables for their own needs.
   * <br><b>Note:</b>Constrainer's users should not use this method.
   */
...
//    addInternalObjectToSymbolicContext(var);
  ...
  /**
   * Adds a constrained floating-point variable to the Constrainer, allows trace.
   */
  public FloatVar addFloatVarTrace(double min, double max, String name, int trace)
  {
    FloatVar var = trace != 0 ?
         new FloatVarImplTrace(this,min,max,name, trace) :
  ...
  /**
   * This method returns all integer constrained variables were ever being added.
   * The set of added variables is reversible.
  ...
  /**
   * Returns all float constrained variables.
   * The set of added variables is reversible.
...
/*==============================================================================
  ...
  /**
   * Adds an undo-object to the reversibility stack for a given
   * undoable object. Some undo-objects can be generated one time
   * between choice points. Constrainer notifies such objects when
   * backtrack or choice point occures.
   * @param undo_object Undo object to add.
  ...
  /**
   * Adds an undoable action. Undoable action is executed
   * during backtracking. It has not change constrainer state
   * (modify variables, add constraints, etc.).
   * It can be used to animate search process.
   * For example, you draw a square during the search and erase it
   * in undoable action during backtracking.
   *
  ...
  public void addUndoableAction(Goal goal)
  {
    ...
    allowUndos(); // the action should have the state at the time when  it was added
  }

  /**
   * Clears the undone-flags for active undoable once objects.
   * This force them to create undos again.
   * Used:
   * - when a choice point is set
   * - when backtracking is performed
   */
  void allowUndos()
  {
...
/*==============================================================================
  ...
  ============================================================================*/
  /**
   * Sets the time limit for the search execution (in seconds).
   * 0 means no time limit (default).
   *
  ...
  /**
   * Sets the limit for the number of failures during a goal
   * execution. 0 means no failures limits (default).
   *
...
/*==============================================================================
  ...
  /**
   * This method returns the number of notifications occured during search execution.
   ...
   * @param flag Boolean parameter. To enable printing should be set
  ...
  /**
   * Sets the precision of the constrained floating-point variable calculationsþ
...
/*==============================================================================
  ...
  /**
   * The function turns on the tracing for the subject.
   * The expression is printed to {@link #out()} every time
   * when subject send notification event.<br>
  ...
  /**
   * The function turns on the tracing for the array of
   * the constrained integer expressions.
   * The array is printed to {@link #out()} every time when
   * at least one variable from this vector is modified.
   *
   ...
   * Object is printed to {@link #out()} when failure occures.
  ...
  /**
   * Tells the constrainer to display the Java stack trace when a failure occurs.
   * To control the frequency of such displaying, use traceFailres(frequency).
   * For example, to display the stack on failure#25, use:
   *    C.traceFailureStack();
   *    C.traceFailures(25);
   */
  ...
  /**
   * Turn on tracing of choice points.
   * Object is printed to {@link #out()} when choice point occures.
   * @param obj Object to print.
   */
...
/*==============================================================================
  ...
  /**
   *Returns expression being equal to the number of required values in a set domain
  ...
  /**
   * Returns the constrained integer expression that equals to the sum of
   * all expressions from the array.
   *
  ...
  public IntExp sum(IntExpArray vars)
  {
    ...
    for(int i=0; i < vars.size(); ++i)
    {
      IntVar var = (IntVar)vars.elementAt(i);
      min += var.min();
      max += var.max();
    }
    IntVar result = addIntVar(min,max);
    execute(new ConstraintAddVector(vars,result));
    return result;
    */
    return vars.sum();
  }
  ...
  /**
   * Returns the constrained integer expression that equals to
   * the scalar product of the array of constrained integer expressions
   * and the array of <b>int</b> values. The expression has
   * the following symbolic form: exps[0]*values[0] + exps[1]*values[1] + ...
   *
  ...
  /**
   * Returns the constrained floating-point expression that equals to
   * the scalar product of the array of constrained floating-point expressions
   * and the array of <b>double</b> values. The expression has
   * the following symbolic form: exps[0]*values[0] + exps[1]*values[1] + ...
   *
  ...
  public FloatExp scalarProduct(IntExpArray exps, double[] values)
  {
   ...
   /**
    * Returns a constrained integer expression that is equal to the number
    * of variables in "vars" bound to the "value".
  ...
  public IntExp cardinality(IntExpArray intvars, int value) throws Failure
  {
    ...
    //ConstraintCardinality ct = new ConstraintCardinality(intvars,value,card);
    //ct.execute(); // may fail
    //return card;
  ...
  /**
   * All elements of an array are associated with their domains. The union
   * of the domains makes up the array "domain" that is the set of all
   * values that may occur in the array. An instance of {@link IntArrayCards}
   * class is associated with each instance of {@link IntExpArray}.
   * The instance keeps track the possible number of the value occurrences
   * in the array that is it has an element(constrained integer expression)
   * per value from the array "domain".
   * For each value from the array "domain" the method {@link #distribute(IntExpArray , IntExpArray)}
   * creates the constraint "equals" to the number of occurrences
   * of values. The number of occurrences for each value of array "domain"
   * is specified in <code>cards</code> parameter.
   *
   ...
   * cards[i] is equal to the number of variables in "vars" bound to
   * the value values[i].
   */
...
/*
  public Vector distribute(Vector vars, int[] values) throws Failure
  {
    ...
    IntExp card_sum = new IntExpAddVector(this, cards);
    card_sum.less(vars.size()+1).execute();
    return cards;
  }
*/
   ...
   * Returns an array of constrained integer variables "cards" such that
   * cards[i] is equal to the number of variables in "vars" bound to
   * the sequental [0,vars.size()] values.
   *
   ...
   * @return Constraint stating that all variables in the specified array
   * must be different.
   */
...
/*==============================================================================
   ...
   *
   * @return true if success
   * @param main_goal com.exigen.ie.constrainer.Goal
    ...
    while(!_goal_stack.empty())
    {
      ...
      catch(Failure f)
      {
        ...
        if (_time_limit > 0)
        {
          ...
          {
            //_out.println("Time Limit Violation: more than "+_time_limit+" sec");
            //success = false;
...
//              success = false;
  ...
  /**
   * Returns a number of the last solution (total amount of available solutions)
   * Checks from 0 to max_sol
  ...
  /**
  * Executes the search goal provided by the first parameter to find
  * all the problem solutions. Each solution should be saved/printed
  * inside the main_goal. After the goal execution the state
  * of the constrainer is restored.
...
/*==============================================================================
  ...
  ============================================================================*/
  /**
   * Adds the subject (usually variable) to the propagation queue.
   * It happenes when the subject changes its state.
   * The notificatioin events will be generated in {@link #propagate} method.
   */
...
/*==============================================================================
  ...
  ============================================================================*/
  /**
   * This method aborts the program execution.
   * It prints the "msg" and the stack trace.
   * Used to display "impossible" errors.
   *
   ...
   * This information is accumulated during the execution of the goals.
   */
  void doPrintInformation()
  {
    _out.println("\nChoice Points: " + _number_of_choice_points
                        + "  Failures: " + _number_of_failures
                        + "  Undos: " + _number_of_undos
                        + "  Notifications: " + _number_of_notifications
  ...
  /**
   * Returns the Constrainer's output stream where the
   * output information is printed out.
   * @return the Constrainer's output stream where the
   * output information is printed out.
   */
...
//  }
   ...
   /**
   * Adds a constraint written in the symbolic form. Note:
   * the constraint added by this function should be posted afterwards
   * by {@link #postConstraints()}
   *
...
//  }
   ...
   /**
    * Posts a constraint written in the symbolic form.
    * Makes passed constraint active. This method executes specified constraint
    * given in the form of boolean expression.
    * The constraint is not being added to internal constraint storage (with
    * which {@link #addConstraint(Constraint)} and {@link #postConstraints()} operate, but
    * is just being activated immediately.
    * @param s String represented constraint.
    * @throws Exception When some constraint cannot be posted (constraint is incompatible
    * with others) or when string is invalid.
    */
...
/*==============================================================================
  ...
  */
  ...
  /**
   * functions providing the possibility of supporting multi-session
   * added by E. Tseitlin
   */
