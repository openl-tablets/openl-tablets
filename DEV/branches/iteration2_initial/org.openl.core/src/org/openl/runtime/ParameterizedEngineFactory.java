 ...
 * @author snshor
 *
 * This class provides a base class for "parameterized engine invocations" - i.e. cases when Request has to be dispatched to different engine
 * based on request parameters. The examples could be engine with different rulesets depending on effective date or LOB. It is not feasible to 
 * maintain all the logic in one engine - it is better to have multiple engines and separate dispatching logic from business logic. 
 * In this case this is done through class {@link IEngineRequestHandler} that creates {@link EngineFactoryDefinition} based on the request R.
 * 
 * 
 * @param <T>
 * @param <R>
