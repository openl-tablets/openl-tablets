...
public class TestConstraintExpLessExp extends TestCase {
    ...
    public void testExecute() {
        ...
        try {
            ...
            var3.setMax(7);
            var3.propagate();
            assertTrue("The greater variable 'setMax(int)' has not been traced", var.max() <= var3.max());
            // increase minimum of the lesser variable
            var.setMin(3);
            var.propagate();
            assertTrue("The greater variable 'setMin(int)' has not been traced", var.min() <= var3.min());
        } catch (Failure f) {
