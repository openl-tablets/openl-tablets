...
public class GoalPrint extends GoalImpl
{
   ...
   * @param text The leading string of text
   *             To begin with, execute method invokes the following line of code:
   *             <code>System.out.println(text+"("+intvars.size()+"):");</code>
   */
  public GoalPrint(IntExpArray intvars, String text)
  {
    super(((IntExp)intvars.elementAt(0)).constrainer(), "Print");
    _intvars = intvars;
    _text = text;
    _newline = false;
  }
   ...
   *
   * @param intvars An array to be printed out
   * @param newline The boolean flag. If "newline"==<code>true</code>
   *        then each element of intvars will be printed starting with a new line.
   */
  ...
  public Goal execute() throws Failure
  {
    ...
    {
      IntExp var = (IntExp)_intvars.elementAt(i);
      if (!_newline)
      {
