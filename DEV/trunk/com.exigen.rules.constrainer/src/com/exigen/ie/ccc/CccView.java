...
public class CccView
{
	...
	//    public String getLastName() { return core().getObject( core().getLastFailedId() ).name(); }
	...
	public boolean process(String obj, int i)
	{
		...
		if (obj.equalsIgnoreCase("goal"))
		{
			...
			else
			{
				...
				//            System.out.println("CccView::process(goal) ms="+(time2-time1));
				setStatus(
					"Activated goal: "
						+ g.name()
						+ ", time: "
						+ core().getExecutionTime()
						+ "ms");
			}
			return true;
		}
		...
		{
			CccExecutable ex = (CccExecutable) core().getConstraintByNum(i);
			//        setLastName(ex.name());
...
//			      System.out.println("getTotal("+obj+")="+r);
	...
	public String[] getDataGroups()
	{
		...
		for (int i=0; i<gr.size(); i++)
		{
			...
			if ("objective".equals(g.getName())) continue;
			if (g.size()>0)
			{
