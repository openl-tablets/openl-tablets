...
public class TestIntArrayCards extends TestCase {
  ...
  public TestIntArrayCards(String name){super(name);}
  ...
  public void testIntArrayCards(){
    ...
    /*IntExpArray array1 = new IntExpArray(C, 10, 0, 9, "array");
    min = array1.min();
    max = array1.max();
    try{
      IntArrayCards cards = array1.cards();
      for (int i=0;i<array1.size();i++)
        C.postConstraint(cards.cardAt(i).gt(nbRepetitions));
      boolean result = C.execute(new GoalGenerate(array1));
      assertTrue(result);
      //fail("test failed!");
    }catch(Failure f){}*/
  }
