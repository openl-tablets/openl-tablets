...
//    System.out.println("exp1=" + exp1 + "[" + exp1.min() + ".." + exp1.max() + "]");
//    System.out.println("exp2=" + exp2 + "[" + exp2.min() + ".." + exp2.max() + "]");
//    System.out.println("exp3=" + exp3 + "[" + exp3.min() + ".." + exp3.max() + "]");
  }
...
/*
  ...
  public static void main(String[] args)
  {
    ...
    IntVar x = c.addIntVar(109, 201, "msft", IntVar.DOMAIN_PLAIN);
    IntVar y = c.addIntVar(-110, 210, "intc", IntVar.DOMAIN_PLAIN);
    IntVar z = c.addIntVar(-100, 100, "cmb", IntVar.DOMAIN_PLAIN);

    try
    {
      IntExp amt1 = x.mul(10);

      amt1.moreOrEqual(1100).post();
      amt1.lessOrEqual(2000).post();

      IntExp amt2 = y.mul(10);

      amt2.moreOrEqual(-100).post();
      amt2.lessOrEqual(200).post();

      IntExp amt3 = z.mul(5);



      IntExp val1 = amt1.mul(10);
      IntExp val2 = amt2.mul(20);
      IntExp val3 = amt3.mul(30);
      ...
      IntExp abs1 = val1.abs();
      IntExp abs2 = val2.abs();
      IntExp abs3 = val3.abs();

      IntExp exp2 = new IntExpArray(c, abs1, abs2, abs3).sum().mul(5);
      exp2.name("exp2 = (abs1 + abs2 + abs3) * 5");

      final IntExp cost = new IntExpArray(c, exp1, exp2).sum();
      cost.name(" Cost");

      cost.publish(EventOfInterest.VALUE);
//      IntVar cost_var = c.addIntVar(cost);
   //   IntExp exp = x.mul(y).mul(z);
...
//      Goal searchGoal = new GoalGenerate(vars,var_selector,value_selector);
      ...
      class Tracer
      {
        ...
        public String toString()
        {
          ...
          try
          {
            return "\nSolution: "+solution+", cost="+_cost.value() + ".  ";
          }
          catch(Failure f)
          {
            return "ERROR: unbound cost";
...
//      c.printInformation();
      ...
      Goal goal = new GoalFastMinimize(solution, cost, false);
//      Goal goal = new GoalMinimize(solution, cost);
