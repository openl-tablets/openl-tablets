...
public class MultiNestedSpreadsheetResultTest {
	...
	public void test() {
		...
		NestedSpreadsheetResultConverter<CodeStep, CompoundStep> conv = new NestedSpreadsheetResultConverter<CodeStep, CompoundStep>(1, conf);
	...
	public void testArraySpr() {
		...
		NestedSpreadsheetResultConverter<CodeStep, CompoundStep> converter = new NestedSpreadsheetResultConverter<CodeStep, CompoundStep>(1, conf);
		List<CodeStep> result = converter.process(upperSpr);
		
		assertNotNull(result);
		assertEquals("firstNested", result.get(0).getCode());
		assertEquals(2, ((CompoundStep)result.get(1)).getSteps().size());
		assertEquals("nestedColumn2_1", ((CompoundStep)((CompoundStep)result.get(1)).getSteps().get(0)).getSteps().get(0).getCode());
		assertEquals("nestedColumn2_2", ((CompoundStep)((CompoundStep)result.get(1)).getSteps().get(1)).getSteps().get(0).getCode());
	}
