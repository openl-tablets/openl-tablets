...
public class RowParser implements IRowParser {
    ...
    private void checkError(boolean errorCondition, StringValue srcValue, String errorMessage) throws BoundError {
        if (errorCondition) {
            throw new BoundError(errorMessage == null ? "" : errorMessage, srcValue.asSourceCodeModule());
        }
    }
    ...
    private boolean validateRow(AlgorithmRow row, boolean multiline, TableParserSpecificationBean spec)
            throws BoundError {
        // check Label
    ...
    private TableParserSpecificationBean getSpecification(StringValue operation, boolean multiline) throws BoundError {
        ...
        checkError(found, operation, "Operation can not be multiline, i.e. can not have nested operations");
