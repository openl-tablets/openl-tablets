...
public class SimpleOpenMethodDispatcher extends OpenMethodDispatcher {
    ...
    private static final Log log = LogFactory.getLog(SimpleOpenMethodDispatcher.class);
     ...
     * 
     * @param delegate delegate method
     */
    ...
    protected IOpenMethod findMatchingMethod(List<IOpenMethod> candidates, IContext context) {
        ...
        log.debug("Finding best matching method for '" + getName() + "'.");
        ...
        for (IOpenMethod candidate : candidates) {
            ...
            TableProperties.Property[] tableProperties = tableNodeProperties.getProperties();

            log.debug("Founded table properties of method: ");

            for (int i = 0; i < tableProperties.length; i++) {

                TableProperties.Property property = tableProperties[i];

                if (i == 0) {
                    log.debug(property.getKey().getValue() + "=" + property.getValue().getValue());
                } else {
                    log.debug(", " + property.getKey().getValue() + "=" + property.getValue().getValue());
                }
            ...
            for (String tableDimensionProperty : tableDimensionProperties) {
                ...
                log.debug("Found constraint for '" + tableDimensionProperty + "' property: " + constraint == null ? "null"
                        : constraint.getTablePropertyDefinition().getExpression());
            ...
            log.debug("Result of constraints evaluation: " + result);
        ...
        log.debug("Count of matching methods: " + filteredMethods.size());
     ...
     * 
     * @param definitions list of available definitions
     ...
     * of concrete method.
     * 
     * @param tableProperties table properties of concrete method
     ...
     * 
     * @param property table property to check
