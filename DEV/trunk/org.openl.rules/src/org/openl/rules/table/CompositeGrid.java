...
//	static final public String VERTICAL_DIR = "Vertical", HORIZONTAL_DIR = "Horizontal";
	...
	static class Transform
	{
		...
		IGrid grid(){return gridTable.getGrid();}
	}
	...
	public Transform transform(int col, int row)
	{
		for (int i = 0; i < mappedRegions.length; i++)
		{
			...
			{
					IGridRegion reg = gridTables[i].getRegion();
					return new Transform(gridTables[i], reg.getLeft() + col - mappedRegions[i].getLeft(), reg.getTop() + row - mappedRegions[i].getTop());
			}		
		}
