...
public class XlsNumberFormat extends XlsFormat
{
	...
	static final public DecimalFormat GENERAL_FORMAT = new DecimalFormat(
			GENERAL_FORMAT_STR);

	static final public XlsNumberFormat General = new XlsNumberFormat(
			new SegmentFormatter(new ITextFormatter.NumberTextFormatter(
					GENERAL_FORMAT, GENERAL_FORMAT_STR), null), null, null);
	...
	public Object parse(String value)
	{
		...
		if (firstTry != value) // if error we return the same string value from parse
			return firstTry;

		if (negativeFormat != null)
		{
			firstTry = negativeFormat.parse(value);
			if (firstTry != value) // if error we return the same string value from
															// parse
				return firstTry;
		} else
			return value;
	...
	static public XlsNumberFormat makeFormat(String xlsformat,
			Map<String, SegmentFormatter> existingFmts)
	{
	 ...
	 * @return
	 */
	private static SegmentFormatter getFormat(String fmt,
			Map<String, SegmentFormatter> existingFmts, boolean isNegative)
	{
	...
	static SegmentFormatter makeSegmentFormatter(String fmt, boolean isNegative)
	{
		...
		{
			Log.warn("Bad java format. Using default. Consider custom mapping: '"
					+ javaFormat + "'");
			df = DEFAULT_FORMAT;
		}
	...
	private static String detectColor(SegmentFormatter sf, String fmt)
	{
		...
		{
			if (fmt.contains(colorsStr[i])
					|| fmt.contains(colorsStr[i].toUpperCase()))
			{
				sf.color = colors[i];
