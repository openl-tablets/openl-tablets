...
/**
 * Represents a workbook color palette. Internally, the XLS format refers to
 * colors using an offset into the palette record. Thus, the first color in the
 * palette has the index 0x8, the second has the index 0x9, etc. through 0x40
 * 
...
public class HSSFPalette {
    ...
    public HSSFColor findColor(byte red, byte green, byte blue) {
        ...
        for (short i = (short) PaletteRecord.FIRST_COLOR_INDEX; b != null; b = palette.getColor(++i)) {
     ...
     * finding the distance between the colors is fairly primative.
     * 
     ...
     * @return The closest color or null if there are no custom colors currently
     *         defined.
     */
    ...
    public HSSFColor addColor(byte red, byte green, byte blue) {
        byte[] b = palette.getColor(PaletteRecord.FIRST_COLOR_INDEX);
        short i;
        for (i = (short) PaletteRecord.FIRST_COLOR_INDEX; i < PaletteRecord.STANDARD_PALETTE_SIZE
                + PaletteRecord.FIRST_COLOR_INDEX; b = palette.getColor(++i)) {
            if (b == null) {
    ...
    private static class CustomColor extends HSSFColor {
        ...
        public short[] getTriplet() {
            return new short[] { (short) (red & 0xff), (short) (green & 0xff), (short) (blue & 0xff) };
        }
