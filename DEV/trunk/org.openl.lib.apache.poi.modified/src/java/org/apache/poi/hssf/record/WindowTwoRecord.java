...
public class WindowTwoRecord extends Record {
    ...
    private BitField defaultHeader = new BitField(0x20); // if false use
                                                            // color in field 4

    // if true use default foreground
    // for headers
    private BitField arabic = new BitField(0x40); // for our desert dwelling
                                                    // friends
    ...
    protected void fillFields(byte[] data, short size, int offset) {
        ...
        if (size > 10) {
            field_5_page_break_zoom = LittleEndian.getShort(data, 10 + offset);
            field_6_normal_zoom = LittleEndian.getShort(data, 12 + offset);
        }
        if (size > 14) { // there is a special case of this record that has
                            // only 14 bytes...undocumented!
