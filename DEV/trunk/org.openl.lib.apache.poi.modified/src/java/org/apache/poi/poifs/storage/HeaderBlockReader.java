...
public class HeaderBlockReader implements HeaderBlockConstants {
    ...
    public HeaderBlockReader(final InputStream stream) throws IOException {
        ...
        if (byte_count != POIFSConstants.BIG_BLOCK_SIZE) {
            ...
            throw new IOException("Unable to read entire header; " + byte_count + type + " read; expected "
        ...
        if (signature.get() != _signature) {
            throw new IOException("Invalid header signature; read " + signature.get() + ", expected " + _signature);
        }
