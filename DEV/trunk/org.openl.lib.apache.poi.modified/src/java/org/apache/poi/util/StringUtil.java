...
public class StringUtil {
     ...
     * return a string { 0x16, 0x00 } -> 0x16
     * 
     ...
     *                the byte array (i.e., is negative or is greater than or
     *                equal to string.length)
     * @exception IllegalArgumentException if len is too large (i.e., there is
     *                not enough data in string to create a String of that
     *                length)
     * @len the length of the final string
     */

    public static String getFromUnicodeHigh(final byte[] string, final int offset, final int len)
    ...
    /**
     * given a byte array of 16-bit unicode characters, compress to 8-bit and
     * return a string { 0x00, 0x16 } -> 0x16
     * 
     * @param string the byte array to be converted
     * @param offset the initial offset into the byte array. it is assumed that
     *            string[ offset ] and string[ offset + 1 ] contain the first
    ...
    public static String getFromUnicode(final byte[] string, final int offset, final int len)
            throws ArrayIndexOutOfBoundsException, IllegalArgumentException {
        if ((offset < 0) || (offset >= string.length)) {
            throw new ArrayIndexOutOfBoundsException("Illegal offset");
        }
