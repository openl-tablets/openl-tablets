 ...
 * <code>0xC1 0x01 0x00 0x00 0x80 0x38 0x01 0x00</code> (do not recalculate)
 * and <code>0xC1 0x01 0x00 0x00 0x60 0x69 0x01
 *               0x00</code> have been seen. If
 * the field <code>isNeeded</code> is set to false (default), then this record
 * is swallowed during the serialization process
 * <P>
...
public class RecalcIdRecord extends Record {
    ...
    protected void fillFields(byte[] data, short size, int offset) {
        ...
        for (int k = 0; k < field_1_recalcids.length; k++) {
            field_1_recalcids[k] = LittleEndian.getShort(data, (k * 2) + offset);
        }
    }
    ...
    public String toString() {
        ...
        for (int k = 0; k < field_1_recalcids.length; k++) {
            buffer.append("    .element_" + k + "       = ").append(field_1_recalcids[k]).append("\n");
        }
        buffer.append("[/RECALCID]\n");
    ...
    public int serialize(int offset, byte[] data) {
        ...
        LittleEndian.putShort(data, 2 + offset, ((short) length));
