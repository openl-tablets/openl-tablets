...
public class MergeCellsRecord extends Record {
     ...
     * inner class because we do not want it used without reference to this
     * class.
     *
     */
    ...
    /**
     * Add an area to consider a merged cell. The index returned is only
     * gauranteed to be correct provided you do not add ahead of or remove ahead
     * of it (in which case you should increment or decrement
     * appropriately....in other words its an arrayList)
     *
    ...
    // public int addArea(short rowfrom, short colfrom, short rowto, short
    // colto)
    ...
    protected void fillFields(byte[] data, short size, int offset) {
        ...
        for (int k = 0; k < numAreas; k++) {
            MergedRegion region = new MergedRegion(LittleEndian.getShort(data, pos + offset), LittleEndian.getShort(
                    data, pos + 2 + offset), LittleEndian.getShort(data, pos + 4 + offset), LittleEndian.getShort(data,
                    pos + 6 + offset));
    ...
    public short getNumAreas() {
        // if the array size is larger than a short (65536), the record can't
        // hold that many merges anyway
        if (field_2_regions == null) {
            return 0;
        }
    ...
    /**
     * set the number of merged areas. You do not need to call this if you use
     * addArea, it will be incremented automatically or decremented when an area
     * is removed. If you are setting this to 0 then you are a terrible person.
     * Just remove the record. (just kidding about you being a terrible
     * person..hehe)
     *
