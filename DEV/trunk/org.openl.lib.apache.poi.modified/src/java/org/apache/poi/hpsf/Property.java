 ...
 *
 * <p>
 * <strong>FIXME:</strong> Reading is not implemented for all {@link Variant}
 * types yet. Feel free to submit error reports or patches for the types you
 * need.
 * </p>
 *
...
public class Property {
     ...
     * @param codepage The section's and thus the property's codepage. It is
     *            needed only when reading string values.
     */
    public Property(final int id, final byte[] src, final long offset, int length, int codepage) {
        this.id = id;
    ...
    protected Map readDictionary(final byte[] src, final long offset, final int length, final int codepage) {
        ...
        if (offset < 0 || offset > src.length) {
            throw new HPSFRuntimeException("Illegal offset " + offset + " while HPSF stream contains " + length
                    + " bytes.");
        }
        int o = (int) offset;
        ...
        for (int i = 0; i < nrEntries; i++) {
             ...
             * characters if the character set is Unicode or else the number of
             * bytes. The length includes terminating 0x00 bytes which we have
             * to strip off to create a Java string.
             */
            long sLength = LittleEndian.getUInt(src, o);
            o += LittleEndianConsts.INT_SIZE;

            /*
             * Read the bytes or characters depending on whether the character
             * set is Unicode or not.
             */
