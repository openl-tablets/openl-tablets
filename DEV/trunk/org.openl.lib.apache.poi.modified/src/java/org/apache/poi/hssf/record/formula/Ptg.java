...
public abstract class Ptg {
    ...
    /*
     * DO NOT REMOVE we keep this method in case we wish to change the way we
     * parse It needs a getPrecedence in OperationsPtg
     * 
     * public static List ptgsToRpn(List infixPtgs) { java.util.Stack operands =
     * new java.util.Stack(); java.util.List retval = new java.util.Stack();
     * 
     * java.util.ListIterator i = infixPtgs.listIterator(); Object p;
     * OperationPtg o ; boolean weHaveABracket = false; while (i.hasNext()) {
     * p=i.next(); if (p instanceof OperationPtg) { if (p instanceof
     * ParenthesisPtg) { if (!weHaveABracket) { operands.push(p); weHaveABracket =
     * true; } else { o = (OperationPtg) operands.pop(); while (!(o instanceof
     * ParenthesisPtg)) { retval.add(o); } weHaveABracket = false; } } else {
     * 
     * while (!operands.isEmpty() && ((OperationPtg)
     * operands.peek()).getPrecedence() >= ((OperationPtg) p).getPrecedence() ) {
     * //TODO handle ^ since it is right associative retval.add(operands.pop()); }
     * operands.push(p); } } else { retval.add(p); } } while
     * (!operands.isEmpty()) { if (operands.peek() instanceof ParenthesisPtg ){
     * //throw some error } else { retval.add(operands.pop()); } } return
     * retval; }
     */
    ...
    public static Ptg createPtg(byte[] data, int offset) {
        ...
        switch (id) {
            ...
            default:
                ...
                throw new java.lang.UnsupportedOperationException(Integer.toHexString((int) id) + " (" + (int) id + ")");
        }
    ...
    /**
     * Overridden toString method to ensure object hash is not printed. This
     * helps get rid of gratuitous diffs when comparing two dumps Subclasses may
     * output more relevant information by overriding this method
     */
