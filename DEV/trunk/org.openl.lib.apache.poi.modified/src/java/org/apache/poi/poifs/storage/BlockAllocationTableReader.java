...
public class BlockAllocationTableReader {
     ...
     * from the raw block list, and any blocks labeled as 'unused' in the block
     * allocation table will also have been removed from the raw block list.
     *
     * @param block_count the number of BAT blocks making up the block
     *            allocation table
     * @param block_array the array of BAT block indices from the filesystem's
     *            header
     ...
     *
     * @exception IOException if, in trying to create the table, we encounter
     *                logic errors
     */

    public BlockAllocationTableReader(final int block_count, final int[] block_array, final int xbat_count,
            final int xbat_index, final BlockList raw_block_list) throws IOException {
        this();
        if (block_count <= 0) {
    ...
    BlockAllocationTableReader(final ListManagedBlock[] blocks, final BlockList raw_block_list) throws IOException {
        this();
        setEntries(blocks, raw_block_list);
    }
     ...
     * The associated blocks are removed from the block list
     *
    ...
    ListManagedBlock[] fetchBlocks(final int startBlock, final BlockList blockList) throws IOException {
        ...
        return (ListManagedBlock[]) blocks.toArray(new ListManagedBlock[0]);
    }
     ...
     *
     * @return index of the next block (may be POIFSConstants.END_OF_CHAIN,
     *         indicating end of chain (duh))
     *
     ...
     * @param raw_blocks the list of blocks being managed. Unused blocks will be
