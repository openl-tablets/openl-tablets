...
public abstract class AbstractShape {
    ...
    /**
     * Add standard properties to the opt record. These properties effect all
     * records.
     *
    ...
    protected int addStandardOptions(HSSFShape shape, EscherOptRecord opt) {
        ...
        if (shape.isNoFill()) {
            // Wonderful... none of the spec's give any clue as to what these
            // constants mean.
        ...
        if (shape.getLineWidth() != HSSFShape.LINEWIDTH_DEFAULT) {
            opt
                    .addEscherProperty(new EscherSimpleProperty(EscherProperties.LINESTYLE__LINEWIDTH, shape
                            .getLineWidth()));
            options++;
        }
        if (shape.getLineStyle() != HSSFShape.LINESTYLE_SOLID) {
            opt.addEscherProperty(new EscherSimpleProperty(EscherProperties.LINESTYLE__LINEDASHING, shape
                    .getLineStyle()));
