...
public class PAPFormattedDiskPage extends FormattedDiskPage {
    ...
    protected byte[] getGrpprl(int index) {
        int papxOffset = 2 * LittleEndian
                .getUnsignedByte(_fkp, _offset + (((_crun + 1) * FC_SIZE) + (index * BX_SIZE)));
        ...
        for (int x = 0; x < index; x++) {
            ...
            if (grpprl.length > 488) {
                ...
                } else // we have some storage...
                {
                    ...
                    if (maxHugeGrpprlSize < grpprl.length - 2) // grpprl.length-2
                        // because we
                        // don't store
                        // the istd
                ...
                System.arraycopy(grpprl, 2, _dataStream, hugeGrpprlOffset + 2, grpprl.length - 2); // grpprl.length-2
                                                                                                    // because
                                                                                                    // we
                                                                                                    // don't
                // store the istd
