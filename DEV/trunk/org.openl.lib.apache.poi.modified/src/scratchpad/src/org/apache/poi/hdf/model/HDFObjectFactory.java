...
/**
 * The Object Factory takes in a stream and creates the low level objects that
 * represent the data.
 * 
 * @author andy
 */
public class HDFObjectFactory {
    ...
    protected HDFObjectFactory(InputStream istream, HDFLowLevelParsingListener l) throws IOException {
        ...
        DocumentEntry headerProps = (DocumentEntry) _filesystem.getRoot().getEntry("WordDocument");
    ...
    private void initCharacterProperties(int charOffset, PlexOfCps charPlcf, int start, int end) {
        ...
        do {
            if (currentChpxIndex < currentArraySize) {
                ...
                int currentCharPage = LittleEndian.getInt(_tableBuffer, charOffset
                        + charPlcf.getStructOffset(++currentPageIndex));
    ...
    private void initParagraphProperties(int parOffset, PlexOfCps parPlcf, int charOffset, PlexOfCps charPlcf,
            int start, int end) {
        // Initialize paragraph property stuff
        ...
        do {
            if (currentPapxIndex < currentArraySize) {
                ...
                int currentParPage = LittleEndian.getInt(_tableBuffer, parOffset
                        + parPlcf.getStructOffset(++currentPageIndex));
                byte byteFkp[] = new byte[512];
     ...
     */
    /*
     * private void initCharacterProperties() { int charOffset =
     * _fib.getFcPlcfbteChpx(); int charPlcSize = _fib.getLcbPlcfbteChpx();
     * 
     * //int arraySize = (charPlcSize - 4)/8;
     * 
     * //first we must go through the bin table and find the fkps for(int x = 0;
     * x < arraySize; x++) {
     * 
     * //get page number(has nothing to do with document page) //containing the
     * chpx for the paragraph int PN = LittleEndian.getInt(_tableBuffer,
     * charOffset + (4 * (arraySize + 1) + (4 * x)));
     * 
     * byte[] fkp = new byte[512]; System.arraycopy(_mainDocument, (PN * 512),
     * fkp, 0, 512); //take each fkp and get the chpxs int crun =
     * LittleEndian.getUnsignedByte(fkp, 511); for(int y = 0; y < crun; y++) {
     * //get the beginning fc of each paragraph text run int fcStart =
     * LittleEndian.getInt(fkp, y * 4); int fcEnd = LittleEndian.getInt(fkp,
     * (y+1) * 4); //get the offset in fkp of the papx for this paragraph int
     * chpxOffset = 2 * LittleEndian.getUnsignedByte(fkp, ((crun + 1) * 4) + y);
     * 
     * //optimization if offset == 0 use "Normal" style if(chpxOffset == 0) {
     * _characterRuns.add(new ChpxNode(fcStart, fcEnd, new byte[0])); continue; }
     * 
     * int size = LittleEndian.getUnsignedByte(fkp, chpxOffset);
     * 
     * byte[] chpx = new byte[size]; System.arraycopy(fkp, ++chpxOffset, chpx,
     * 0, size); //_papTable.put(new Integer(fcStart), papx);
            ...
            for (int y = 0; y < crun; y++) {
                ...
                do {
                    if (currentChpxIndex < currentArraySize) {
                        ...
                        currentCharPage = LittleEndian.getInt(_tableBuffer, charOffset
                                + charPlcf.getStructOffset(++currentPageIndex));
                        fkp = new byte[512];
                        System.arraycopy(_mainDocument, (currentCharPage * 512), fkp, 0, 512);
