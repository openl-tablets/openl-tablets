...
public class CHP implements Cloneable {
    ...
    int _dxaSpace;// space following each character in the run expressed in
                    // twip units
    byte _iss;// superscript/subscript indices 0 means no super/subscripting 1
                // means text in run is superscripted 2 means text in run is
                // subscripted
    byte _kul;// underline code see spec
    byte _ico;// color of text see spec
    short _hpsPos;// super/subscript position in half points; positive means
    ...
    short _ibstRMark;// index to author IDs stored in hsttbfRMark. used when
                        // text in run was newly typed when revision marking was
                        // enabled
    short _ibstRMarkDel;// index to author IDs stored in hsttbfRMark. used when
                        // text in run was newly typed when revision marking was
                        // enabled
    ...
    // recorded when revision marking is on.)Date/time at which this run of text
    // was deleted by the author. (Only recorded when revision marking is on.)
    int _istd;
    int _baseIstd = -1;
    int _fcPic;
    ...
    int _hpsKern;// kerning distance for characters in run recorded in half
                    // points
    int _fcObj;
    byte _icoHighlight;// highlight color
    boolean _fChsDiff;
    boolean _highlighted;// when true characters are highlighted with color
                            // specified by chp.icoHighlight
    boolean _fPropMark;// when true, properties have been changed with revision
                        // marking on
    short _ibstPropRMark;// index to author IDs stored in hsttbfRMark. used
                            // when properties have been changed when revision
                            // marking was enabled
    int _dttmPropRMark;// Date/time at which properties of this were changed
                        // for this run of text by the author
    byte _sfxtText;// text animation see spec
    boolean _fDispFldRMark;// see spec
    short _ibstDispFldRMark;// Index to author IDs stored in hsttbfRMark. used
                            // when ListNum field numbering has been changed
                            // when revision marking was enabled
    int _dttmDispFldRMark;// The date for the ListNum field number change
    byte[] _xstDispFldRMark = new byte[32];// The string value of the ListNum
                                            // field when revision mark tracking
                                            // began
    short _shd;// shading
