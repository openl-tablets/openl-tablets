 ...
 * 
 * Divide the coordinate line by dots (the beginning and the end of the
 * interval). And assign the value to the beginning(means that the left border
 * is included, while right not).
 * 
 * Example: if call the method {@link #putInterval(11, 23, "AnyValue")}, the
 * result will be:
 * 
 * _____11("AnyValue")______________23______________ - means that value
 * "AnyValue" belongs to [11;22].
 * 
 * If then call the method {@link #putInterval(11, 50, "Yo")} the result will
 * be: _____11("AnyValue", "Yo")______________23("Yo")____________________50___
 * - means that value "AnyValue" is the same, and "Yo" belongs to [11;49]
 * 
 * @author DLiauchuk
 */
...
public class IntervalMap<T, V> {
    ...
    private NavigableMap<Comparable<T>, List<V>> map = new TreeMap<Comparable<T>, List<V>>();

    public List<V> getInInterval(Comparable<T> key) {
        /*
         * List<V> res = map.get(key); if (res != null) { return res; }
         */

        //SortedMap<Comparable<T>, List<V>> submap = map.headMap(key);
        Entry<Comparable<T>, List<V>> entry = map.floorEntry(key);
        // if (submap.isEmpty()) {
        if (entry == null) {
    ...
    public void putInterval(Comparable<T> fromKey, Comparable<T> toKey, V value) {
        ...
        Comparable<T> entryKey = map.ceilingKey(fromKey);
        // if (submap.isEmpty() || !submap.firstKey().equals(fromKey)) {
        if (entryKey == null || !entryKey.equals(fromKey)) {
            //SortedMap<Comparable<T>, List<V>> head = map.headMap(fromKey);
            Entry<Comparable<T>, List<V>> entry = map.lowerEntry(fromKey);
            //ArrayList<V> firstList = head.isEmpty() ? new ArrayList<V>() : new ArrayList<V>(head.get(head.lastKey()));
            ArrayList<V> firstList = entry == null ? new ArrayList<V>() : new ArrayList<V>(entry.getValue());
        ...
        if (!map.containsKey(toKey)) {
            Entry<Comparable<T>, List<V>> entry = map.lowerEntry(toKey);
            //ArrayList<V> lastList = new ArrayList<V>(submap.get(submap.lastKey()));
            ArrayList<V> lastList = new ArrayList<V>(entry.getValue());
    ...
    public void removeInterval(Comparable<T> fromKey, Comparable<T> toKey, V value) {
        SortedMap<Comparable<T>, List<V>> submap = map.subMap(fromKey, toKey);
