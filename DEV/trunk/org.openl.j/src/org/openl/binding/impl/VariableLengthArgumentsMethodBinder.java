...
public class VariableLengthArgumentsMethodBinder extends ANodeBinder {
    ...
    public VariableLengthArgumentsMethodBinder(String methodName, IOpenClass[] argumentsTypes, IBoundNode[] children) {
        ...
        if (argumentsTypes == null || argumentsTypes.length < 1) {
            String message = String.format("At least one argument should exist in method signature(%s) to bind it as variable arguments method", 
                methodName);
    ...
    private IBoundNode getVarArgsMethodNode(ISyntaxNode node, IBindingContext bindingContext, IOpenClass[] methodArguments,
            int indexOfLastEqualArgumentType) {
        ...
        //
        Class<?> componentVarArgClass = methodArguments[methodArguments.length - 1].getInstanceClass().getComponentType();
