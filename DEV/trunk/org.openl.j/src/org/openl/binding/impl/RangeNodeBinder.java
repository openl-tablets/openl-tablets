    ...
    public IBoundNode bind(ISyntaxNode node, IBindingContext bindingContext) throws Exception {
        ...
        if (type.contains("binary")) {
            ...
            if (type.endsWith("minus") || type.endsWith("ddot")) {
                return new LiteralBoundNode(node, new RangeWithBounds(val, val2), JavaOpenClass
                        .getOpenClass(RangeWithBounds.class));
            }
            if (type.endsWith("tdot")) {
                return new LiteralBoundNode(node,
                        new RangeWithBounds(getMinimalIncrease(val), getMinimalDecrease(val2)), JavaOpenClass
                                .getOpenClass(RangeWithBounds.class));
            }
        }

        if (type.contains("number"))
            return new LiteralBoundNode(node, new RangeWithBounds((Number) val, (Number) val), JavaOpenClass
                    .getOpenClass(RangeWithBounds.class));

        if (type.contains("unary.prefix")) {
            if (type.endsWith("lt")) {
                return new LiteralBoundNode(node, new RangeWithBounds(getMin(val), getMinimalDecrease(val)),
                        JavaOpenClass.getOpenClass(RangeWithBounds.class));
            } else if (type.endsWith("le")) {
                return new LiteralBoundNode(node, new RangeWithBounds(getMin(val), val), JavaOpenClass
                        .getOpenClass(RangeWithBounds.class));
            } else if (type.endsWith("gt")) {
                return new LiteralBoundNode(node, new RangeWithBounds(getMinimalIncrease(val), getMax(val)),
                        JavaOpenClass.getOpenClass(RangeWithBounds.class));
            } else if (type.endsWith("ge")) {
                return new LiteralBoundNode(node, new RangeWithBounds(val, getMax(val)), JavaOpenClass
                        .getOpenClass(RangeWithBounds.class));
            }
