...
public class NameSpacedLibraryConfiguration extends AConfigurationElement
{
    ...
    public IMethodCaller getMethodCaller(String name, IOpenClass[] params,
	    ICastFactory casts, IConfigurableResourceContext cxt)

    throws AmbiguousMethodException
    {
	for (int i = 0; i < factories.length; i++)
	{
	    IMethodCaller mc = MethodSearch.getMethodCaller(name, params,
		    casts, factories[i].getLibrary(cxt));
	    if (mc != null)
		return mc;
	}
    ...
    {
	factories = (IMethodFactoryConfigurationElement[]) CollectionsUtil.add(
		factories, factory);
    }
