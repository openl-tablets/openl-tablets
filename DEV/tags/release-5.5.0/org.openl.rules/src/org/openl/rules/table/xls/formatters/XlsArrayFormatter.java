...
/**
 * 
 * Xls array format is a formatter for converting an array of elements, represented by <code>{@link String}</code> 
 * (elements  are separated by <code>{@link #ARRAY_ELEMENTS_SEPARATOR}</code> escaper for the separator is 
 * <code>{@link #ARRAY_ELEMENTS_SEPARATOR_ESCAPER}</code>) to 
 * an array of specified type (method <code>{@link #parse(String)}</code>). <br>
 * Also it provides the back convertion from specified type to a <code>{@link String}</code>, in outcome result 
 * elements will separated by <code>{@link #ARRAY_ELEMENTS_SEPARATOR}</code> 
