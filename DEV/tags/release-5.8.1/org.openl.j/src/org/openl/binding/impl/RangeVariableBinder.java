...
public class RangeVariableBinder extends ANodeBinder {
    ...
    public IBoundNode bind(ISyntaxNode node, IBindingContext bindingContext) throws Exception {
        ...
        IOpenSourceCodeModule sourceModule = new SubTextSourceCodeModule(node.getModule(), node.getSourceLocation().getStart().getAbsolutePosition(info), node.getSourceLocation().getEnd().getAbsolutePosition(info)+1 );
        ...
        IOpenField om = bindingContext.findRange(ISyntaxConstants.THIS_NAMESPACE, rangeParts[0].getIdentifier(), rangeParts[1].getIdentifier());
