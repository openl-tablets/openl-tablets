...
public class DatatypeTableMethodBoundNode implements IMemberBoundNode
{
   ...
   * @param table
   */
  public DatatypeTableMethodBoundNode(
      TableSyntaxNode xlsDatatypeNode, ModuleOpenClass module,
...
void addFields(IBindingContext cxt)
		throws Exception
	{
	  ...
	  ILogicalTable dataTable = findOrientation(cxt);
		...
		for (int i = 0; i < h; i++)
		{
		  ...
		  ILogicalTable row = dataTable.getLogicalRow(i);
		    ...
		    IOpenClass fieldType = OpenlTool.makeType(src, openl,
			...
			if (row.getLogicalWidth() < 2)
			{
				BoundError err =
					new BoundError(
						null,
						"Bad table structure: must be {header} / {type | name}",
						null,
						src);
				throw err;
			}  
...
//			String fieldName = row.getLogicalColumn(1).getGridTable().getStringValue(0,0);
			...
			catch (Throwable t)
			{
				BoundError err =
					new BoundError(
						null,
						"Can not add field " + fieldName + ": " + t.getMessage(),
						null,
						src);
				throw err;
			}
