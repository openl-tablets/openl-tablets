...
public class MethodNodeBinder extends ANodeBinder
{
			...
			throws Exception
	{
		...
		IMethodCaller om = bindingContext.findMethodCaller(
				ISyntaxConstants.THIS_NAMESPACE, methodName, types);

		if (om == null)
			throw new BoundError(node, "Method "
					+ MethodUtil.printMethod(methodName, types) + " not found");
	...
	public IBoundNode bindTarget(ISyntaxNode node,
			IBindingContext bindingContext, IBoundNode target) throws Exception
	{
		int nc = node.getNumberOfChildren();
		if (nc < 1)
		{
		...
		IMethodCaller om = MethodSearch.getMethodCaller(methodName, types,
				bindingContext, target.getType());
