...
public class TestIntBoolFloatLessExp extends TestCase {
  ...
  public TestIntBoolFloatLessExp(String name){super(name);}
  ...
  public void testIntBoolFloatLessExp(){
    ...
    try{
      for (int i=1;i<array.size();i++){
           boolexp[i-1] = new IntBoolExpFloatLessExp(
                                new FloatExpConst(C, 1.0),
                                new FloatExpAddExp(
                                        new FloatExpOpposite(array.get(i-1)),
                                        array.get(i)));
    ...
    for (int i=1;i<array.size();i++){
      ...
      }catch(Failure f){fail("test failed due to incorrect work of FloatVar.value()");}
    }
  }
