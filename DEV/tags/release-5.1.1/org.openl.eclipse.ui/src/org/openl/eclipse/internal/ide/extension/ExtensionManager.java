  ...
  implements IOpenlExtensionManager
{
  ...
  public IOpenlLanguageExtension[] getOpenlLanguageExtensions()
  {
    ...
    return (IOpenlLanguageExtension[])getExtensions(
      EXTENSION_POINT_OPENL_LANGUAGE,
      collector).toArray(
  ...
  public IOpenlBuilderExtension[] getOpenlBuilderExtensions()
  {
    ...
    return (IOpenlBuilderExtension[])getExtensions(
      EXTENSION_POINT_OPENL_BUILDER,
      collector).toArray(
      NO_BUILDER_EXTENSIONS);
  }

  /**
   * Iterates extensions for a given extensionPointId
   * and pass extensions' IConfigurationElement to the given collector.
   * Returns collected extensions. 
   */
  Collection getExtensions(String extensionPointId, IConvertor collector)
  {
    ...
    return OpenIterator.collect(result.iterator(), collector).asList();
  }
...
//      new ArrayList(Arrays.asList(getEditorRegistry().getFileEditorMappings()));
//
//    // add new files
