 ...
 * 
 */
public class LibraryFactoryConfiguration extends AConfigurationElement
	implements IConfigurationElement
{
	...
	{
	    NameSpacedLibraryConfiguration lib = (NameSpacedLibraryConfiguration) iter
		    .next();
	    lib.validate(cxt);
	}
    }
    ...
    public IOpenField getVar(String namespace, String name,
	    IConfigurableResourceContext cxt, boolean strictMatch)
    {
	NameSpacedLibraryConfiguration lib = (NameSpacedLibraryConfiguration) map
		.get(namespace);
	...
	NameSpacedLibraryConfiguration lib = (NameSpacedLibraryConfiguration) map
		.get(namespace);
	return lib == null ? null : lib.getMethodCaller(name, params, casts,
		cxt);
    }
