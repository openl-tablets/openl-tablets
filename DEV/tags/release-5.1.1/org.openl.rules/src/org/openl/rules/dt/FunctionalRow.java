...
public abstract class FunctionalRow implements IDecisionRow,
	...
	public FunctionalRow(String name, int row, ILogicalTable decisionTable) {
		...
		this.paramsTable = decisionTable.getLogicalRegion(PARAM_COLUMN, row, 1,
				1);
	...
	public void prepare(IOpenClass methodType, IMethodSignature signature,
			OpenL openl, ModuleOpenClass dtModule,
	...
	Object[][] prepareParamValues(OpenlToolAdaptor ota, RuleRow ruleRow)
		...
		for (int col = 0; col < len; col++) {
			...
			for (int j = 0; j < paramDecl.length; j++) {
				...
				Object v = null;
				try {
					v = loadParam(LogicalTable
							.logicalTable(singleParamGridTable), paramDecl[j]
							.getType(), paramDecl[j].getName(), ruleName, ota);
		...
		if (errors.size() > 0) {
			throw new SyntaxErrorException("Error:", errors
					.toArray(new IBoundError[0]));
		}
	...
	Object loadParam(ILogicalTable dataTable, IOpenClass paramType,
		...
		if (!indexed)
			return loadSingleParam(paramType, paramName, ruleName, dataTable,
					ota);
	...
	static public Object loadSingleParam(IOpenClass paramType,
			String paramName, String ruleName, ILogicalTable cell,
		...
		if (ota != null && ota.getHeader() != null) {
			...
			OpenMethodHeader newHeader = new OpenMethodHeader(old_header
					.getName(), paramType, old_header.getSignature(),
					old_header.getDeclaringClass());
			ota.setHeader(newHeader);

			if (src.startsWith("{") && src.endsWith("}")) {
				GridCellSourceCodeModule srcCode = new GridCellSourceCodeModule(
						cell.getGridTable());
	...
	private static void validateValue(Object res, IOpenClass paramType) {
		...
		String errorMsg = "The value " + res + " is outside of domain "
				+ domain.toString();
	 ...
	 * @param paramType
	 */
	static public void setCellMetaInfo(ILogicalTable cell, String paramName,
			IOpenClass paramType) {
		CellMetaInfo meta = new CellMetaInfo(CellMetaInfo.Type.DT_DATA_CELL,
				paramName, paramType);
	...
	public static void setMetaInfo(IMetaHolder holder, ILogicalTable cell,
		...
		vmi.setFullName(ruleName == null ? paramName : ruleName + "."
				+ paramName);
	...
	static public IOpenClass getType(String typeCode, IBindingContext cxt)
		...
		if (typeCode.endsWith("[]")) {
			...
			IOpenClass baseType = cxt.findType(ISyntaxConstants.THIS_NAMESPACE,
					baseCode);
			if (baseType == null)
				return null;
		...
		IOpenClass type = cxt.findType(ISyntaxConstants.THIS_NAMESPACE,
				typeCode);
		;
		// if (type == null)
	...
	IParameterDeclaration no_parameter() {
		return new ParameterDeclaration(JavaOpenClass.STRING, NO_PARAM
				+ np_idx++);
	}
		...
		if (params == null) {
			...
			for (int i = 0; i < len; i++) {
				...
				if (paramNames.contains(pname))
					throw new BoundError(nodes[1],
							"Duplicated parameter name: " + pname);
	...
	boolean hasNoParams() {
		if (hasNoParams == null)
			hasNoParams = params[0].getName().startsWith(NO_PARAM) ? Boolean.TRUE
					: Boolean.FALSE;
	...
	public IOpenSourceCodeModule getSourceCodeModule() {
		...
		return method == null ? null : method.getMethodBodyBoundNode()
				.getSyntaxNode().getModule();
	}
	...
	static public void loadParams(Object[] ary, int from, Object[] params,
