...
public class TestIntExpElementAt extends TestCase {
  ...
  public TestIntExpElementAt(String name){super(name);}
  ...
  public void testIntExpElementAt(){
    try{
      ...
      assertTrue(flag);
      assertTrue((array.get(cursor1.value())
                   +
                    array.get(cursor2.value()))
                   ==
                    array.get(cursor3.value()));
  ...
  public void testContainsMinMax(){
    ...
    for (int i=0;i<array.length; i++){
      try{
        cursor.setMin(i);
        C.propagate();
      }catch(Failure f){fail("test failed due to incorrect behsviour of IntVar.setMin(int)");}
    ...
    for (int i=0;i<array.length-1; i++){
      try{
        ...
        C.propagate();
      }catch(Failure f){fail("test failed due to incorrect behaviour of IntVar.removeValue(int)");}
    ...
    for (int i=0;i<array.length; i++){
      try{
        ...
        C.propagate();
      }catch(Failure f){fail("test failed due to incorrect behsviour of IntVar.setMax(int)");}
