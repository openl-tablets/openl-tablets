...
public class TestIntBoolExpFloatEqExp extends TestCase {
  ...
  public TestIntBoolExpFloatEqExp(String name){super(name);}
  ...
  public void testIntBoolExpFloatEqExp(){
    ...
    for (int i=0;i<array.size();i++){
      ...
      }catch(Failure f){fail("test failed due to incorrect work of FlaotExpArray.set(FloatExp, int)");}
    }
    ...
    try{
      ...
      C.postConstraint(new IntBoolExpFloatEqExp(new FloatExpConst(C, Math.sqrt(3)/2),
                                                array.get(0),
  ...
  public void testAll(){
    FloatVar floatvar1 = C.addFloatVar(-10.0989, 1.3652, "Floatvar1"),
             floatvar2 = C.addFloatVar( -1.3784, 89.123, "floatvar2");
    ...
    }catch(Failure f){fail("test failed due to incorrect behaviour of Constrainer.addFloatVar(int min, int max)");}
