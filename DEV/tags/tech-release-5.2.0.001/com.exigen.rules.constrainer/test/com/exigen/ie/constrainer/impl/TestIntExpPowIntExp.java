...
public class TestIntExpPowIntExp extends TestCase {
  ...
  public TestIntExpPowIntExp(String name){super(name);}
  ...
  public void testA(int testDoesntWork){
    try{
    int resultValue = 9;
    IntVar base = C.addIntVar(-5, 5),
           exponent = C.addIntVar(0, 4);
  ...
  public void testMaxMinCase1(){
    ...
    IntVar base = C.addIntVar(-5, -1, "base", IntVar.DOMAIN_PLAIN),
           power = C.addIntVar(2, 6, "power", IntVar.DOMAIN_BIT_FAST);

    IntExp exp = new IntExpPowIntExp(base, power);
    assertEquals(-Math.pow(5,5), (double)exp.min(), Double.MIN_VALUE);
    assertEquals(Math.pow(5,6), (double)exp.max(), Double.MIN_VALUE);
    ...
    for (int i=power.min();i<=power.max();i++){
      if (power.contains(i) && ((i%2)==1) )
        try{
          power.removeValue(i);
        }catch(Failure f){fail("IntVar.removeValue works incorrectly");}
    }
    try{
      C.propagate();
    }catch(Failure f){fail("unexpected exception");}
    assertEquals(1, (double)exp.min(), Double.MIN_VALUE);
    assertEquals(Math.pow(5,6), (double)exp.max(), Double.MIN_VALUE);
  ...
  IntVar base = C.addIntVar(2, 7, "base", IntVar.DOMAIN_PLAIN),
         power = C.addIntVar(0, 8, "power", IntVar.DOMAIN_BIT_FAST);
  ...
  IntVar base = C.addIntVar(-9, 7, "base", IntVar.DOMAIN_BIT_FAST),
         power = C.addIntVar(2, 8, "power", IntVar.DOMAIN_BIT_FAST);
  ...
  public void testMinComplete(){
    ...
    assertEquals("odd powers && positive base",
                 (int)Math.pow(base.min(), power.min()), exp.min());
    ...
    assertEquals("odd powers and negative base",
                (int)Math.pow(base.min(), power.max()), exp.min());
    ...
    //only odd powers
    removeAllEven(power);
    assertEquals("odd powers && changable base",
                 (int)Math.pow(base.min(), power.max()), exp.min());
    ...
    assertEquals("even powers && changable base",
                 0
                 ,exp.min());
    //2)
    ...
    //only odd powers
    removeAllOdd(power);
    assertEquals("even powers && changable base 2) ",
                 0
                 ,exp.min());
    ...
    power = C.addIntVar(2, 6);
    exp = new IntExpPowIntExp(base, power);
    assertEquals("any powers && changable base ",
                 (int)Math.pow(base.min()
                              ,5 /*power.max_odd()*/)
                 ,exp.min());
    ...
    assertEquals("any powers && positive base ",
                 (int)Math.pow(base.min()
                              ,power.min())
                 ,exp.min());
    ...
    assertEquals("any powers && negative base ",
                 (int)Math.pow(base.min()
                              ,3 /*power.max_odd()*/)
                 ,exp.min());
  ...
  public void testMaxComplete(){
    //even powers && changable base
    IntVar base = C.addIntVar(-21, 3, "base"),
           power = C.addIntVar(2, 10, "power");
    IntExp exp = new IntExpPowIntExp(base, power);
    removeAllOdd(power);
    assertEquals("even powers && changable base",
                 (int)Math.pow(Math.max(-base.min(),
                                   base.max()),
                               power.max()),
                  exp.max());
    ...
    exp = new IntExpPowIntExp(base, power);
    removeAllOdd(power);
    try{base.setMin(1);C.propagate();}catch(Failure f){fail("test failed");}
    assertEquals("even powers && positive base",
                 (int)Math.pow(Math.max(-base.min(),
                                   base.max()),
                               power.max()),
                  exp.max());
    ...
    exp = new IntExpPowIntExp(base, power);
    removeAllOdd(power);
    try{base.setMax(-1);C.propagate();}catch(Failure f){fail("test failed");}
    assertEquals("even powers && negative base",
                 (int)Math.pow(Math.max(-base.min(),
                                   base.max()),
                               power.max()),
                  exp.max());
    ...
    assertEquals("odd powers && changable base",
                 (int)Math.pow(base.max(),
                               power.max()),
                  exp.max());
    assertEquals("odd powers && changable base",
                 (int)Math.pow(12,11), exp.max());
    ...
    removeAllEven(power);
    try{base.setMax(-1);C.propagate();}catch(Failure f){fail("test failed");}
    assertEquals("odd powers && negative base",
                 (int)Math.pow(base.max(),
                               power.min()),
                  exp.max());
    ...
    exp = new IntExpPowIntExp(base, power);
    removeAllEven(power);
    try{base.setMin(2);C.propagate();}catch(Failure f){fail("test failed");}
    assertEquals("odd powers && positive base",
                 (int)Math.pow(base.max(),
                               power.max()),
                  exp.max());
    ...
    exp = new IntExpPowIntExp(base, power);
    assertEquals("any powers && positive base",
                 (int)Math.max(Math.pow(base.max(), power.max()),
                               Math.pow(base.min(), 10 /*power.max_even()*/)),
                  exp.max());
  }
  ...
  private void removeAllEven(IntVar var){
    ...
    try{
      C.propagate();
    }catch(Failure f){fail("Constrainer propagation failed");}
  }
  ...
  public void testUsage(){
    IntVar base = C.addIntVar(-4, 2, "base"),
