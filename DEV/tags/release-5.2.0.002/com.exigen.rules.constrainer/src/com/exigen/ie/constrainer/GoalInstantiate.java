 ...
 * <p>
 * It uses {@link IntValueSelector} to select the next value
 * to be removed from the domain of the variable.
 * <p>
...
public class GoalInstantiate extends GoalImpl
{
  ...
  /**
   * Creates GoalInstantiate using <code>IntValueSelectorMin</code> as
   * a default ValueSelector. The goal constructed that way uses a recursive
   * search algorithm.
   */
  ...
  class RecursiveImpl implements Impl
  {
    ...
    public Goal instantiate(int chosen_value) throws Failure
    {
      ...
      //Debug.on();Debug.print(this + " by "+chosen_value);Debug.off();
        ...
        return new GoalOr(_goal_value,
                           new GoalAnd(goal_limit, GoalInstantiate.this) );
    }
