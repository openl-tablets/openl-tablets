...
public class OpenlTool {
	...
	static public IOpenClass getType(String typeName, OpenL openl) {
		...
		return binder.makeBindingContext().findType(
				ISyntaxConstants.THIS_NAMESPACE, typeName);
	}

	static public IOpenMethodHeader getMethodHeader(IOpenSourceCodeModule src,
			OpenL openl, IBindingContextDelegator cxt) {
		if (cxt == null)
			cxt = new BindingContextDelegator(new BindingContext((Binder) openl
					.getBinder(), JavaOpenClass.VOID, openl));
	...
	static public void compileMethod(IOpenSourceCodeModule src, OpenL openl,
			CompositeMethod comp, IBindingContext cxt) {
		try {
			cxt.pushErrors();
			IOpenParser parser = openl.getParser();
			...
			MethodBindingContext mbc = new MethodBindingContext(comp
					.getHeader(), cxt);
			...
			try {
				ibmn = bindMethodType((IBoundMethodNode) bc.getTopNode(), cxt,
						comp.getHeader().getType());
			} catch (Exception ex) {

				BoundError be = new BoundError(bc.getTopNode().getSyntaxNode(),
						"", ex);
	...
	static public CompositeMethod makeMethod(IOpenSourceCodeModule src,
			OpenL openl, IOpenMethodHeader header, IBindingContext cxt) {
		return makeMethod(src, openl, header,
	...
	static public CompositeMethod makeMethod(IOpenSourceCodeModule src,
		...
		try {
			...
			MethodBindingContext mbc = new MethodBindingContext(header, cxt,
					searchInContext, depthParameterSearchLevel);
	...
	static public IOpenClass makeType(IOpenSourceCodeModule src, OpenL openl,
			IBindingContextDelegator cxt) {
		try {

			if (cxt == null)
				cxt = new BindingContextDelegator(openl.getBinder()
						.makeBindingContext());
			cxt.pushErrors();
			IOpenParser parser = openl.getParser();
	...
	static IBoundMethodNode bindMethodType(IBoundMethodNode bnode,
			IBindingContext bindingContext, IOpenClass type) throws Exception
	...
	static class MethodCastNode extends CastNode implements IBoundMethodNode {
		 ...
		 * @param castedType
		 */
		public MethodCastNode(IBoundNode bnode, IOpenCast cast,
	...
	static public CompositeMethod makeMethodWithUnknownType(
			IOpenSourceCodeModule src, OpenL openl, String name, IMethodSignature signature, IOpenClass declaringClass , 
			int depthParameterSearchLevel, IBindingContext cxt) 
	{
	...
	static public CompositeMethod makeMethodWithUnknownType(IOpenSourceCodeModule src,
			OpenL openl, String name, IMethodSignature signature, IOpenClass declaringClass ,
			IBindingContext cxt)
	{
		return makeMethodWithUnknownType(src, openl, name, signature, declaringClass, MethodBindingContext.DEFAULT_CONTEXT_LEVEL, cxt);
	}
}
