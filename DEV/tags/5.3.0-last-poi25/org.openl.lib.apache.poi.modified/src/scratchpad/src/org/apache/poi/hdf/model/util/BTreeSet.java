...
/*
 * A B-Tree like implementation of the java.util.Set inteface. This is a
 * modifiable set and thus allows elements to be added and removed. An instance
 * of java.util.Comparator must be provided at construction else all Objects
 * added to the set must implement java.util.Comparable and must be comparable
 * to one another. No duplicate elements will be allowed in any BTreeSet in
 * accordance with the specifications of the Set interface. Any attempt to add a
 * null element will result in an IllegalArgumentException being thrown. The
 * java.util.Iterator returned by the iterator method guarantees the elements
 * returned are in ascending order. The Iterator.remove() method is supported.
 * Comment me
 *
 * @author Ryan Ackley
 *
...
public class BTreeSet extends AbstractSet {
    ...
    public class BTreeNode {
        ...
        private final int MIN = (order - 1) / 2;

        BTreeNode(BTreeNode parent) {
            this.parent = parent;
            entries = new Entry[order];
            entries[0] = new Entry();
        }

        /*
         * Possibly a deceptive name for a pretty cool method. Uses binary
         * search to determine the postion in entries[] in which to traverse to
         * find the correct BTreeNode in which to insert a new element. If the
         * element exists in the calling BTreeNode than -1 is returned. When the
         * parameter position is true and the element is present in the calling
         * BTreeNode -1 is returned, if position is false and the element is
         * contained in the calling BTreeNode than the position of the element
        ...
        private int childToInsertAt(Object x, boolean position) {
            ...
            while (lo <= hi) {
                if (compare(x, entries[index].element) > 0) {
                    lo = index + 1;
                    index = (hi + lo) / 2;
                } else {
                    hi = index - 1;
            ...
            hi++;
            if (entries[hi] == null || entries[hi].element == null) {
                return hi;
            }
            return (!position ? hi : compare(x, entries[hi].element) == 0 ? -1 : hi);
        }
        ...
        boolean delete(Object x, int parentIndex) {
            ...
            } // Now temp contains element to delete and temp's parentIndex is
                // parentIndex

            if (temp.isLeaf()) { // If leaf and have more than MIN elements,
                                    // simply delete
                ...
                else { // If leaf and have less than MIN elements, than prepare
                        // the BTreeSet for deletion
            ...
            else { // Only delete at leaf so first switch with successor than
                    // delete
        ...
        private void deleteElement(Object x) {
            ...
            for (; index < (nrElements - 1); index++) {
                entries[index] = entries[index + 1];
            }

            if (nrElements == 1) {
                entries[index] = new Entry(); // This is root and it is empty
            } else {
                entries[index] = null;
            }
        ...
        private void fixAfterDeletion(int parentIndex) {
            ...
            if (parent.nrElements < MIN) { // If parent lost it's n/2 element
                                            // repair it
                ...
                if (!temp.parent.isRoot() && temp.parent.nrElements < MIN) { // If
                                                                                // need
                                                                                // be
                                                                                // recurse
                    ...
                    for (; i < entries.length; i++) {
                        if (x.entries[i].child == temp.parent) {
                            break;
                        }
                    }
        ...
        boolean insert(Object x, int parentIndex) {
            if (isFull()) { // If full, you must split and promote splitNode
                            // before inserting
                ...
                if (isRoot()) { // Grow a level
                    ...
                    if (compare(x, root.entries[0].element) < 0) {
                        insert(x, 0);
                    } else {
                    ...
                    if (compare(x, parent.entries[parentIndex].element) < 0) {
            ...
            else if (isLeaf()) { // If leaf, simply insert the non-duplicate
                                    // element
            ...
            else { // If not full and not leaf recursively find correct node to
                    // insert at
                int insertAt = childToInsertAt(x, true);
        ...
        /*
         * This method is called only when stealLeft and stealRight could not be
         * called, the BTreeNode has the minimum number of elements, has a
         * leftSibling, and the leftSibling has more than the minimum number of
         * elements. If after completion parent has fewer than the minimum
         * number of elements than the parents entries[0] slot is left empty in
         * anticipation of a recursive call to stealLeft, stealRight, mergeLeft,
         * or mergeRight to fix the parent. All of the before-mentioned methods
        ...
        private void mergeLeft(int parentIndex) {
            ...
            if (isLeaf()) { // Don't worry about children
                ...
                insertNewElement(p.entries[parentIndex - 1].element, add); // Could
                                                                            // have
                                                                            // been
                                                                            // a
                                                                            // successor
                ...
                if (p.nrElements == MIN && p != root) {

                    for (int x = parentIndex - 1, y = parentIndex - 2; y >= 0; x--, y--) {
                        p.entries[x] = p.entries[y];
                    }
                    p.entries[0] = new Entry();
                    p.entries[0].child = ls; // So p doesn't think it's a
                                                // leaf this will be deleted in
                ...
                if (p.isRoot() && p.nrElements == 0) { // It's the root and
                                                        // it's empty
                    root = this;
                    parent = null;
                }
                ...
                if (p.nrElements == MIN && p != root) { // Push
                                                                        // everything
                                                                        // to
                                                                        // the
                                                                        // right
                ...
                else { // Either p.nrElements > MIN or p == BTreeSet.this.root
                        // so push everything to the left
                    for (int x = parentIndex - 1, y = parentIndex; y <= p.nrElements; x++, y++) {
                        p.entries[x] = p.entries[y];
                    }
                    p.entries[p.nrElements] = null;
        ...
        /*
         * This method is called only when stealLeft, stealRight, and mergeLeft
         * could not be called, the BTreeNode has the minimum number of
         * elements, has a rightSibling, and the rightSibling has more than the
         * minimum number of elements. If after completion parent has fewer than
         * the minimum number of elements than the parents entries[0] slot is
         * left empty in anticipation of a recursive call to stealLeft,
         * stealRight, mergeLeft, or mergeRight to fix the parent. All of the
         * before-mentioned methods expect the parent to be in such a condition.
         */
        private void mergeRight(int parentIndex) {
            ...
            if (isLeaf()) { // Don't worry about children
                ...
                if (p.nrElements == MIN && p != root) {
                    ...
                    p.entries[0] = new Entry();
                    p.entries[0].child = rs; // So it doesn't think it's a
                                                // leaf, this child will be
                                                // deleted in the next recursive
                                                // call
                }
        ...
        private void prepareForDeletion(int parentIndex) {
            if (isRoot()) {
                return; // Don't attempt to steal or merge if your the root
            ...
            else if (parentIndex < entries.length && parent.entries[parentIndex + 1] != null
                    && parent.entries[parentIndex + 1].child != null
                    && parent.entries[parentIndex + 1].child.nrElements > MIN) {
            ...
            // If not root and can't steal left or right then try to merge left
                ...
                mergeRight(parentIndex);
            }
        }

        /*
         * Splits a BTreeNode into two BTreeNodes, removing the splitNode from
         * the calling BTreeNode.
         */
        private BTreeNode split() {
            ...
            rightSibling.nrElements--; // Need to correct for copying the last
                                        // Entry which has a null element and a
                                        // child
            return rightSibling;
        }

        /*
         * Creates a new BTreeSet.root which contains only the splitNode and
         * pointers to it's left and right child.
         */
        ...
        /*
         * This method is called only when the BTreeNode has the minimum number
         * of elements, has a leftSibling, and the leftSibling has more than the
        ...
        private void stealLeft(int parentIndex) {
            ...
            if (isLeaf()) { // When stealing from leaf to leaf don't worry about
                            // children
                int add = childToInsertAt(p.entries[parentIndex - 1].element, true);
        ...
        /*
         * This method is called only when stealLeft can't be called, the
         * BTreeNode has the minimum number of elements, has a rightSibling, and
         * the rightSibling has more than the minimum number of elements.
         */
    ...
    private class Iterator implements java.util.Iterator {
        private int index = 0;
        private Stack parentIndex = new Stack(); // Contains all
                                                    // parentIndicies for
                                                    // currentNode
        ...
        private Object nextElement() {
            if (currentNode.isLeaf()) {
                if (index < currentNode.nrElements) {
                ...
                } else if (!parentIndex.empty()) { // All elements have been
                                                    // returned, return
                                                    // successor of lastReturned
                                                    // if it exists
                    ...
                    if (index == currentNode.nrElements) {
                        return null; // Reached root and he has no more
                    }
                                        // children
                    return currentNode.entries[index++].element;
                }
            ...
            else { // Your not a leaf so simply find and return the successor
                    // of lastReturned
    ...
    /*
     * Constructors A no-arg constructor is supported in accordance with the
     * specifications of the java.util.Collections interface. If the order for
     * the B-Tree is not specified at construction it defaults to 32.
     */
    ...
    /*
     * Guarantees that the Objects are returned in ascending order. Due to the
     * volatile structure of a B-Tree (many splits, steals and merges can happen
     * in a single call to remove) this Iterator does not attempt to track any
     * concurrent changes that are happening to it's BTreeSet. Therefore, after
     * every call to BTreeSet.remove or BTreeSet.add a new Iterator should be
     * constructed. If no new Iterator is constructed than there is a chance of
     * receiving a NullPointerException. The Iterator.delete method is
     * supported.
     */
