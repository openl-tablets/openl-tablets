...
public class PAP implements Cloneable {
    ...
    byte _fKeepFollow;// keep paragraph on same page with next paragraph if
                        // possible
    ...
    byte _fNoLnn;// no line numbering for this paragraph. (makes this an
                    // exception to the section property of line numbering)
    int _ilfo;// when non-zero, (1-based) index into the pllfo identifying the
    ...
    byte _fWindowControl;// when 1, Word will prevent widowed lines in this
                            // paragraph from being placed at the beginning of a
                            // page
    ...
    byte _fOverflowPunct;// when 1, apply overflow punctuation rules when
    ...
    int _wAlignFont;// font alignment 0 Hanging 1 Centered 2 Roman 3 Variable 4
                    // Auto
    short _fontAlign;// multiVal see Spec.
    byte _fInTable;// when 1, paragraph is contained in a table row
    byte _fTtp;// when 1, paragraph consists only of the row mark special
    ...
    int _dxaWidth;// when not == 0, paragraph is constrained to be dxaWidth
    ...
    short[] _brcBottom = new short[2];// paragraphspecification for border
                                        // below
    short[] _brcRight = new short[2];// paragraphspecification for border to
                                        // the
    short[] _brcBetween = new short[2];// right of paragraphsee spec
    short[] _brcBar = new short[2];// specification of border to place on
    short _brcTop1;// outside of text when facing pages are to be
                    // displayed.spec
    ...
    short _brcBar1;// specification of border to place on outside of text when
                    // facing pages are to be displayed.
    int _dxaFromText;// horizontal distance to be maintained between an
                        // absolutely positioned paragraph and any non-absolute
                        // positioned text
    int _dyaFromText;// vertical distance to be maintained between an
                        // absolutely positioned paragraph and any non-absolute
                        // positioned text
    int _dyaHeight;// see spec
    int _shd;// shading
    int _dcs;// drop cap specifier
    byte[] _anld = new byte[84];// autonumber list descriptor (see ANLD
                                // definition)
    short _fPropRMark;// when 1, properties have been changed with revision
                        // marking on
    short _ibstPropRMark;// index to author IDs stored in hsttbfRMark. used
                            // when properties have been changed when revision
                            // marking was enabled
    byte[] _dttmPropRMark = new byte[4];// Date/time at which properties of this
                                        // were changed for this run of text by
                                        // the author. (Only recorded when
                                        // revision marking is on.)
    byte[] _numrm = new byte[8];// paragraph numbering revision mark data (see
                                // NUMRM)
    short _itbdMac;// number of tabs stops defined for paragraph. Must be >= 0
                    // and <= 64.
