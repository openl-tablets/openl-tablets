...
public class EventBridge implements HDFLowLevelParsingListener {
    ...
    public void bodySection(SepxNode sepx) {
        SectionProperties sep = (SectionProperties) StyleSheet.uncompressProperty(sepx.getSepx(),
                new SectionProperties(), _stsh);
    ...
    public void characterRun(ChpxNode chpx) {
        ...
        CharacterProperties chp = (CharacterProperties) StyleSheet.uncompressProperty(byteChpx, _currentStd.getCHP(),
                _stsh);
    ...
    private void flushHeaderProps(int start, int end) {
        ...
        for (int x = 0; x < size; x++) {
            ...
            // SepxNode node = new SepxNode(-1, secStart, secEnd,
            // oldNode.getSepx());
            // bodySection(node);
    ...
    private void flushHeldParagraph() {
        ...
        ParagraphProperties pap = (ParagraphProperties) StyleSheet.uncompressProperty(bytePapx, _currentStd.getPAP(),
                _stsh);
        ...
        CharacterProperties numChp = (CharacterProperties) StyleSheet.uncompressProperty(((ChpxNode) _onHold.get(size))
                .getChpx(), std.getCHP(), _stsh);
    ...
    public void paragraph(PapxNode papx) {
        ...
        if (pap.getFTtp() > 0) {
            TableProperties tap = (TableProperties) StyleSheet.uncompressProperty(bytePapx, new TableProperties(),
                    _stsh);
