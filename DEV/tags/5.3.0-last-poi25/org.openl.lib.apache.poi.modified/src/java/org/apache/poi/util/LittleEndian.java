...
public class LittleEndian implements LittleEndianConsts {
     ...
     */
    private static long getNumber(final byte[] data, final int offset, final int size) {
        long result = 0;
     ...
     * @param size Description of the Parameter
     */
    private static void putNumber(final byte[] data, final int offset, final long value, final int size) {
     ...
     * @param size the number of bytes to read; in 99.99% of cases, this will be
     *            SHORT_SIZE, INT_SIZE, or LONG_SIZE -- but it doesn't have to
     *            be.
     * @return the byte array containing the required number of bytes. The array
     *         will contain all zero's on end of stream
     * @exception IOException will be propagated back to the caller
     * @exception BufferUnderrunException if the stream cannot provide enough
     *                bytes
     */

    public static byte[] readFromStream(final InputStream stream, final int size) throws IOException,
            BufferUnderrunException {
