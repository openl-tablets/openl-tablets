...
public class HeaderRecord extends Record {
    ...
    protected void fillFields(byte[] data, short size, int offset) {
        if (size > 0) {
            ...
            field_2_header = StringUtil.getFromCompressedUnicode(data, 3 + offset, // [Shawn]
                                                                                    // Changed
                                                                                    // 1 to
                                                                                    // 3
                                                                                    // for
    ...
    public int serialize(int offset, byte[] data) {
        ...
        LittleEndian.putShort(data, 2 + offset, (short) ((len - 4) + getHeaderLength()));

        if (getHeaderLength() > 0) {
            data[4 + offset] = (byte) getHeaderLength();
            StringUtil.putCompressedUnicode(getHeader(), data, 7 + offset); // [Shawn]
                                                                            // Place
                                                                            // the
                                                                            // string
                                                                            // in
