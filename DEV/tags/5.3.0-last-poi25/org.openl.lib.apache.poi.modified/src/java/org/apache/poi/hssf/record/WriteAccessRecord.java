 ...
 * (on unix the user's login, on Windoze its the name you typed when you
 * installed the thing)
 * <P>
...
public class WriteAccessRecord extends Record {
     ...
     *
     * @return username of the user who is logged in (probably "tomcat" or
     *         "apache")
     */
    ...
    public int serialize(int offset, byte[] data) {
        ...
        while (temp.length() < 0x70 - 0x3) {
            temp.append(" "); // (70 = fixed lenght -3 = the overhead bits of
                                // unicode string)
        }
        ...
        LittleEndian.putShort(data, 2 + offset, (short) (stringbytes.length)); // 112
                                                                                // bytes
                                                                                // (115
                                                                                // total)
     ...
     * logged in user.
     *
     * @param username of the user who is logged in (probably "tomcat" or
     *            "apache")
     */
