 ...
 * many features are not supported and some just aren't implemented yet. If in
 * doubt test the specific calls you wish to make. Graphics calls are always
 * drawn into an EscherGroup so one will need to be created.
 * <p>
...
public class EscherGraphics2d extends Graphics2D {
    ...
    public boolean drawImage(Image image, int x, int y, Color bgColor, ImageObserver imageobserver) {
        return drawImage(image, x, y, image.getWidth(imageobserver), image.getHeight(imageobserver), bgColor,
                imageobserver);
    }
    ...
    public void drawRenderedImage(RenderedImage renderedimage, AffineTransform affinetransform) {
        BufferedImage bufferedimage = new BufferedImage(renderedimage.getColorModel(), renderedimage.getData()
                .createCompatibleWritableRaster(), false, null);
    ...
    // Image transformImage(Image image, Rectangle rectangle, Rectangle
    // rectangle1, ImageObserver imageobserver, Color color1)
    // {
    // logger.log(POILogger.WARN,"transformImage() not supported");
    // return null;
    // }
    //
    // Image transformImage(Image image, int ai[], Rectangle rectangle,
    // ImageObserver imageobserver, Color color1)
    // {
    // logger.log(POILogger.WARN,"transformImage() not supported");
    // return null;
    // }
