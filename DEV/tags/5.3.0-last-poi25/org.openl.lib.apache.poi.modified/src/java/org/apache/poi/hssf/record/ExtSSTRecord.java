...
public class ExtSSTRecord extends Record {
    ...
    // Cant seem to find this documented but from the biffviewer it is clear
    // that
    ...
    protected void fillFields(byte[] data, short size, int offset) {
        ...
        for (int k = 2; k < (size - offset); k += 8) {
            ...
            ExtSSTInfoSubRecord rec = new ExtSSTInfoSubRecord((short) 0, (short) 8, tempdata);
    ...
    public void setBucketOffsets(int[] bucketAbsoluteOffsets, int[] bucketRelativeOffsets) {
        field_2_sst_info = new ArrayList(bucketAbsoluteOffsets.length);
    ...
    public String toString() {
        ...
        for (int k = 0; k < getNumInfoRecords(); k++) {
            ...
            buffer.append("    .streampos      = ").append(Integer.toHexString(getInfoRecordAt(k).getStreamPos()))
                    .append("\n");
            buffer.append("    .sstoffset      = ")
                    .append(Integer.toHexString(getInfoRecordAt(k).getBucketSSTOffset())).append("\n");
        }
        buffer.append("[/EXTSST]\n");
