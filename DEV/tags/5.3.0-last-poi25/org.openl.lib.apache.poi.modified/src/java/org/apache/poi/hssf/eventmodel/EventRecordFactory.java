...
/**
 * Event-based record factory. As opposed to RecordFactory this refactored
 * version throws record events as it comes accross the records. I throws the
 * "lazily" one record behind to ensure that ContinueRecords are processed
 * first.
 *
 * @author Andrew C. Oliver (acoliver@apache.org) - probably to blame for the
 *         bugs (so yank his chain on the list)
 * @author Marc Johnson (mjohnson at apache dot org) - methods taken from
 *         RecordFactory
 * @author Glen Stampoultzis (glens at apache.org) - methods taken from
 *         RecordFactory
...
public class EventRecordFactory {
    ...
    static {
        records = new Class[] { BOFRecord.class, InterfaceHdrRecord.class, MMSRecord.class, InterfaceEndRecord.class,
                ...
                StyleRecord.class, UseSelFSRecord.class, BoundSheetRecord.class, CountryRecord.class, SSTRecord.class,
                ExtSSTRecord.class, EOFRecord.class, IndexRecord.class, CalcModeRecord.class, CalcCountRecord.class,
                RefModeRecord.class, IterationRecord.class, DeltaRecord.class, SaveRecalcRecord.class,
                PrintHeadersRecord.class, PrintGridlinesRecord.class, GridsetRecord.class, GutsRecord.class,
                DefaultRowHeightRecord.class, WSBoolRecord.class, HeaderRecord.class, FooterRecord.class,
                HCenterRecord.class, VCenterRecord.class, PrintSetupRecord.class, DefaultColWidthRecord.class,
                DimensionsRecord.class, RowRecord.class, LabelSSTRecord.class, RKRecord.class, NumberRecord.class,
                DBCellRecord.class, WindowTwoRecord.class, SelectionRecord.class, ContinueRecord.class,
                LabelRecord.class, BlankRecord.class, ColumnInfoRecord.class, MulRKRecord.class, MulBlankRecord.class,
                MergeCellsRecord.class, BoolErrRecord.class, ExternSheetRecord.class, NameRecord.class,
    ...
    public static Record[] createRecord(short rectype, short size, byte[] data) {
        ...
        try {
            ...
            if (constructor != null) {
                retval = (Record) constructor.newInstance(new Object[] { new Short(rectype), new Short(size), data });
            } else {
    ...
    /**
     * Construct an abortable EventRecordFactory. The same as calling new
     * EventRecordFactory(true)
     *
     ...
     *
     * @param abortable specifies whether the return from the listener handler
     *            functions are obeyed. False means they are ignored. True means
     *            the event loop exits on error.
     */
        ...
        try {
            ...
            do {
                ...
                if (rectype != 0) {
                    ...
                    in.read(data);
                    Record[] recs = createRecord(rectype, recsize, data); // handle
                                                                            // MulRK
                                                                            // records
                    ...
                    if (recs.length > 1) {
                        for (int k = 0; k < recs.length; k++) {
                            ...
                            last_record = recs[k]; // do to keep the algorythm
                                                    // homogenous...you can't
                        } // actually continue a number record anyhow.
                    } else {
                        Record record = recs[0];

                        if (record != null) {
                            if (rectype == ContinueRecord.sid && !(last_record instanceof ContinueRecord) && // include
                                                                                                                // continuation
                                                                                                                // records
                                                                                                                // after
                                    !(last_record instanceof UnknownRecord)) // unknown
                                                                                // records
                                                                                // or
                                                                                // previous
                                                                                // continuation
    ...
    /**
     * Register a listener for records. These can be for all records or just a
     * subset.
     *
     * @param sids an array of Record.sid values identifying the records the
     *            listener will work with. Alternatively if this is "null" then
     ...
     * @return boolean abort. If exitability is turned on this aborts out of the
     *         event loop should any listener specify to do so.
     */
...
/**
 * ListenerWrapper just wraps an ERFListener and adds support for throwing the
 * event to multiple SIDs
 */
