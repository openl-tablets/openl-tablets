 ...
 * they are really just standard fields that are in several records. It is
...
public class UnicodeString extends Record implements Comparable {
    ...
    public boolean equals(Object o) {
        ...
        return ((field_1_charCount == other.field_1_charCount) && (field_2_optionflags == other.field_2_optionflags) && field_3_string
    ...
    protected void fillFields(byte[] data, short size) {
        ...
        if ((field_2_optionflags & 1) == 0) {
            try {
                field_3_string = new String(data, 3, getCharCount(), StringUtil.getPreferredEncoding());
            ...
            for (int j = 0; j < array.length; j++) {
                array[j] = (char) LittleEndian.getShort(data, 3 + (j * 2));
            }
    ...
    public int serialize(int offset, byte[] data) {
        ...
        try {
            ...
            if (getOptionFlags() == 0) {
                ...
                StringUtil.putUncompressedUnicode(unicodeString, data, 0x3 + offset);
            }
        } catch (Exception e) {
            if (getOptionFlags() == 0) {
                StringUtil.putCompressedUnicode(getString(), data, 0x3 + offset);
            } else {
                StringUtil.putUncompressedUnicode(getString(), data, 0x3 + offset);
            }
        }
     ...
     * string currently. effectively setCharCount(getString.length()).
     *
