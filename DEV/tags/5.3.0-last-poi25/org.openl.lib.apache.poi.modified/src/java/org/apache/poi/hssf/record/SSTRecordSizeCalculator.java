 ...
 * sucks because it's similar to the SST serialization code. In general the SST
 * serialization code needs to be rewritten.
 *
...
class SSTRecordSizeCalculator {
        ...
        for (; unipos < strings.size(); unipos++) {
            ...
            if (unistr.getRecordSize() <= available) {
                ...
                if (available >= SSTRecord.STRING_MINIMAL_OVERHEAD) {
                    ...
                    stringReminant = (unistr.getRecordSize() - toBeWritten) + LittleEndianConsts.BYTE_SIZE;
                    if (available != toBeWritten) {
                        int shortrecord = recordSize - (available - toBeWritten);

                        recordLengths.set(recordLengths.size() - 1, new Integer(shortrecord
    ...
    private void calcReminant() {
        ...
        if (stringReminant <= available) {
            ...
            if (available != toBeWritten) {
                int shortrecord = recordSize - (available - toBeWritten);
                recordLengths.set(recordLengths.size() - 1, new Integer(shortrecord - SSTRecord.STD_RECORD_OVERHEAD));
                recordSize = shortrecord;
            }
