...
/**
 * Red-Black tree-based implementation of Map. This class guarantees that the
 * map will be in both ascending key order and ascending value order, sorted
 * according to the natural order for the key's and value's classes.
 * <p>
 *
 ...
 * that implementation, particularly when trying to keep the two TreeMaps
 * synchronized with each other. And if the data contained in the TreeMaps is
 * large, the cost of redundant storage becomes significant.
 * <p>
 *
 * This solution keeps the data properly synchronized and minimizes the data
 * storage. The red-black algorithm is based on TreeMap's, but has been modified
 * to simultaneously map a tree node by key and by value. This doubles the cost
 * of put operations (but so does using two TreeMaps), and nearly doubles the
 * cost of remove operations (there is a savings in that the lookup of the node
 * to be removed only has to be performed once). And since only one node
 * contains the key and value, storage is significantly less than that required
 * by two TreeMaps.
 * <p>
 *
 * There are some limitations placed on data kept in this Map. The biggest one
 * is this:
 * <p>
 *
 * When performing a put operation, neither the key nor the value may already
 * exist in the Map. In the java.util Map implementations (HashMap, TreeMap),
 * you can perform a put with an already mapped key, and neither cares about
 * duplicate values at all ... but this implementation's put method with throw
 * an IllegalArgumentException if either the key or the value is already in the
 * Map.
 * <p>
 *
 * Obviously, that same restriction (and consequence of failing to heed that
 * restriction) applies to the putAll method.
 * <p>
 *
 ...
 * <p>
 *
 * New methods are added to take advantage of the fact that values are kept
 * sorted independently of their keys:
 * <p>
 *
 * Object getKeyForValue(Object value) is the opposite of get; it takes a value
 * and returns its key, if any.
 * <p>
 *
 * Object removeValue(Object value) finds and removes the specified value and
 * returns the now un-used key.
 * <p>
 *
 * Set entrySetByValue() returns the Map.Entry's in a Set whose iterator will
 * iterate over the Map.Entry's in ascending order by their corresponding
 * values.
 * <p>
 *
 * Set keySetByValue() returns the keys in a Set whose iterator will iterate
 * over the keys in ascending order by their corresponding values.
 * <p>
 *
 * Collection valuesByValue() returns the values in a Collection whose iterator
 * will iterate over the values in ascending order.
 * <p>
 *
...
public final class BinaryTree // final for performance
    ...
    private abstract class BinaryTreeIterator implements Iterator {
        ...
        BinaryTreeIterator(final int type) {
            _type = type;
            _expected_modifications = _modifications;
         ...
         * @exception ConcurrentModificationException if the BinaryTree is
         *                modified behind the iterator's back
         */
         ...
         * the iterator. This method can be called only once per call to next.
         ...
         *                called, or the remove method has already been called
    ...
    private static final class Node implements Map.Entry {
        ...
        /**
         * Make a new cell with given key and value, and with null links, and
         * black (true) colors.
         *
         * @param key
         * @param value
        ...
        Node(final Comparable key, final Comparable value) {
            _data = new Comparable[] { key, value };
            _left = new Node[] { null, null };
            _right = new Node[] { null, null };
            _parent = new Node[] { null, null };
            _black = new boolean[] { true, true };
        ...
        public boolean equals(Object o) {
            ...
            return _data[_KEY].equals(e.getKey()) && _data[_VALUE].equals(e.getValue());
        }
        ...
        public int hashCode() {
            if (!_calculated_hashcode) {
                _hashcode = _data[_KEY].hashCode() ^ _data[_VALUE].hashCode();
     ...
     * @param index _KEY or _VALUE (used to put the right word in the exception
     *            message)
     *
    ...
    /**
     * copy the color from one node to another, dealing with the fact that one
     * or both nodes may, in fact, be null
     *
    ...
    private static boolean isLeftChild(final Node node, final int index) {
        return (node == null) ? true : ((node.getParent(index) == null) ? false : (node == node.getParent(index)
                .getLeft(index)));
    }
    ...
    /**
     * find the least node from a given node. very useful for starting a sorting
     * iterator ...
     *
     * @param node the node from which we will start searching
     * @param index _KEY or _VALUE
     *
     * @return the smallest node, from the specified node, in the specified
     *         mapping
     */
    ...
    /**
     * Constructs a new BinaryTree from an existing Map, with keys and values
     * sorted
     *
     * @param map the map whose mappings are to be placed in this map.
     *
     * @exception ClassCastException if the keys in the map are not Comparable,
     *                or are not mutually comparable; also if the values in the
     *                map are not Comparable, or are not mutually Comparable
     * @exception NullPointerException if any key or value in the map is null
     * @exception IllegalArgumentException if there are duplicate keys or
     ...
     * @param index _KEY or _VALUE
     *
     * @return the key (if the value was mapped) or the value (if the key was
     *         mapped); null if we couldn't find the specified object
     */
    ...
    private void doRedBlackDelete(final Node deleted_node) {
        for (int index = _MINIMUM_INDEX; index < _INDEX_COUNT; index++) {
            ...
            Node replacement = ((deleted_node.getLeft(index) != null) ? deleted_node.getLeft(index) : deleted_node
                    .getRight(index));
     ...
     * rebalances the tree (somewhat, as red-black trees are not perfectly
    ...
    private void doRedBlackDeleteFixup(final Node replacement_node, final int index) {
        ...
        while ((current_node != _root[index]) && (isBlack(current_node, index))) {
     ...
     * @param index _KEY or _VALUE
     *
     * @return the key, if remove by value, or the value, if remove by key. null
     *         if the specified key or value could not be found
     */
        ...
        if (_entry_set[_KEY] == null) {
            _entry_set[_KEY] = new AbstractSet() {
                ...
                public boolean contains(Object o) {
                    ...
                    Node node = lookup((Comparable) entry.getKey(), _KEY);
     ...
     * <p>
     *
     * The difference between this method and entrySet is that entrySet's
     * iterator() method returns an iterator that iterates over the mappings in
     * ascending order by key. This method's iterator method iterates over the
     * mappings in ascending order by value.
     *
     * @return a set view of the mappings contained in this map.
     */
        ...
        if (_entry_set[_VALUE] == null) {
            _entry_set[_VALUE] = new AbstractSet() {
                ...
                public boolean contains(Object o) {
                    ...
                    Node node = lookup((Comparable) entry.getValue(), _VALUE);
    ...
    /**
     * Returns the key to which this map maps the specified value. Returns null
     * if the map contains no mapping for this value.
     *
     * @param value value whose associated key is to be returned.
     *
     * @return the key to which this map maps the specified value, or null if
     *         the map contains no mapping for this value.
     *
     * @exception ClassCastException if the value is of an inappropriate type
     *                for this map.
     ...
     *
     * @exception IllegalArgumentException if the node already exists in the
     *                value mapping
     */
    ...
    private void insertValue(final Node newNode) throws IllegalArgumentException {
        ...
        while (true) {
            ...
            if (cmp == 0) {
                throw new IllegalArgumentException("Cannot store a duplicate value (\"" + newNode.getData(_VALUE)
                        + "\") in this Map");
            } else if (cmp < 0) {
     ...
     * <p>
     *
     * The difference between this method and keySet is that keySet's iterator()
     * method returns an iterator that iterates over the keys in ascending order
     * by key. This method's iterator method iterates over the keys in ascending
     * order by value.
     *
     * @return a set view of the keys contained in this map.
     */
    ...
    /**
     * increment the modification count -- used to check for concurrent
     * modification of the map through the map and through an Iterator from one
     * of its Set or Collection views
     */
     ...
     * @exception NullPointerException if the specified key or value is null
     * @exception IllegalArgumentException if the key duplicates an existing
     *                key, or if the value duplicates an existing value
     */

    @Override
    public Object put(final Object key, final Object value) throws ClassCastException, NullPointerException,
     ...
     *
     * @return previous key associated with specified value, or null if there
     *         was no mapping for value.
     */
    ...
    /**
     * swap two nodes (except for their content), taking care of special cases
     * where one is the other's parent ... hey, it happens.
     *
     * @param x one node
    ...
    private void swapPosition(final Node x, final Node y, final int index) {
        ...
        boolean y_was_left_child = (y.getParent(index) != null) && (y == y.getParent(index).getLeft(index));
     ...
     * <p>
     *
     * The difference between this method and values is that values's iterator()
     * method returns an iterator that iterates over the values in ascending
     * order by key. This method's iterator method iterates over the values in
