...
public class Section {
    ...
    public Section(final byte[] src, int offset) {
         ...
         * the property list. The problem is that we have to read the property
         * with ID 1 before we read other properties, at least before other
         * properties containing strings. The reason is that property 1
         * specifies the codepage. If it is 1200, all strings are in Unicode. In
         * other words: Before we can read any strings we have to know whether
         * they are in Unicode or not. Unfortunately property 1 is not
         * guaranteed to be the first in a section.
         *
        ...
        for (int i = 0; i < properties.length; i++) {
            ...
            if (i == properties.length - 1) {
                ...
                length = (int) LittleEndian.getUInt(src, pass1Offset + LittleEndianConsts.INT_SIZE) - sOffset;
            }
        ...
        for (int i = 0; i < properties.length; i++) {
            ...
            if (i == properties.length - 1) {
                ...
                length = (int) LittleEndian.getUInt(src, offset + LittleEndianConsts.INT_SIZE) - sOffset;
            }

            /* Create it. */
            properties[i] = new Property(id, src, this.offset + sOffset, length, codepage);
        }
     ...
     * found there, the method calls {@link SectionIDMap#getPIDString}.
     * </p>
     *
     ...
     * is not available, <code>null</code> is returned and a subsequent call
     * to {@link #wasNull} will return <code>true</code>.
     * </p>
     *
     ...
     * call to {@link #wasNull} will return <code>true</code> to let the
     * caller distinguish that case from a real property value of
     * <code>false</code>.
     * </p>
     *
     * @param id The property's ID
    ...
    protected int getPropertyIntValue(final int id) {
        /*
         * FIXME: Find out why the following is a Long instead of an Integer!
         */
