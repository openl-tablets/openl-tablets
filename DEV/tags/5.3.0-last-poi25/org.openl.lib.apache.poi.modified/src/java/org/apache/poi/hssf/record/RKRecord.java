...
/**
 * Title: RK Record Description: An internal 32 bit number with the two most
 * significant bits storing the type. This is part of a bizarre scheme to save
 * disk space and memory (gee look at all the other whole records that are in
 * the file just "cause"..,far better to waste processor cycles on this then
...
public class RKRecord extends Record implements CellValueRecordInterface {
     ...
     * Debugging main()
     * <P>
     * Normally I'd do this in a junit test, but let's face it -- once this
     * algorithm has been tested and it works, we are never ever going to change
     * it. This is driven by the Faceless Enemy's minions, who dare not change
     * the algorithm out from under us.
     *
     * @param ignored_args command line arguments, which we blithely ignore
     */

    public static void main(String ignored_args[]) {
        int[] values = { 0x3FF00000, 0x405EC001, 0x02F1853A, 0x02F1853B, 0xFCDD699A };
        double[] rvalues = { 1, 1.23, 12345678, 123456.78, -13149594 };

        for (int j = 0; j < values.length; j++) {
            System.out.println("input = " + Integer.toHexString(values[j]) + " -> " + rvalues[j] + ": "
     ...
     * <P>
     * The mechanism for determining the value is dependent on the two low order
     * bits of the raw number. If bit 1 is set, the number is an integer and can
     * be cast directly as a double, otherwise, it's apparently the exponent and
     * mantissa of a double (and the remaining low-order bits of the double's
     * mantissa are 0's).
     * <P>
     * If bit 0 is set, the result of the conversion to a double is divided by
     * 100; otherwise, the value is left alone.
     * <P>
     * [insert picture of Screwy Squirrel in full Napoleonic regalia]
     *
     * @return the value as a proper double (hey, it <B>could</B> happen)
     */
