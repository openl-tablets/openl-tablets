 ...
 * <P>
 * Description: Unknown record just tells you the sid so you can figure out what
 * records you are missing. Also helps us read/modify sheets we don't know all
 * the records to. (HSSF leaves these alone!)
 * <P>
...
public class UnknownRecord extends Record {
    ...
    /**
     * construct an unknown record. No fields are interperated and the record
     * will be serialized in its original form more or less
     *
    ...
    public UnknownRecord(short id, short size, byte[] data, int offset) {
        sid = id;
        size = (short) Math.min(size, data.length - offset);
    ...
    /**
     * print a sort of string representation ([UNKNOWN RECORD] id = x [/UNKNOWN
     * RECORD])
     */
    ...
    protected void validateSid(short id) {
        ...
        // if we had a valid sid we wouldn't be using the "Unknown Record"
        // record now would we?
    }
}
