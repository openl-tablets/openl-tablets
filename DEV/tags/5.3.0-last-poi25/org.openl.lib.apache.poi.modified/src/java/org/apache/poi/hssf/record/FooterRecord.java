...
public class FooterRecord extends Record {
    ...
    protected void fillFields(byte[] data, short size, int offset) {
        if (size > 0) {
            ...
            field_2_footer = StringUtil.getFromCompressedUnicode(data, 3 + offset, // [Shawn]
                                                                                    // Changed
                                                                                    // 1 to
                                                                                    // 3
                                                                                    // for
    ...
    public int serialize(int offset, byte[] data) {
        ...
        LittleEndian.putShort(data, 2 + offset, (short) ((len - 4) + getFooterLength()));
        if (getFooterLength() > 0) {
            data[4 + offset] = (byte) getFooterLength();
            StringUtil.putCompressedUnicode(getFooter(), data, 7 + offset); // [Shawn]
                                                                            // Place
                                                                            // the
                                                                            // string
                                                                            // in
