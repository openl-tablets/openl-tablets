...
public class Range {
    ...
    public CharacterRun getCharacterRun(int index) {
        ...
        int[] point = findRange(_paragraphs, _parStart, Math.max(chpx.getStart(), _start), chpx.getEnd());
    ...
    public Table getTable(Paragraph paragraph) {
        ...
        r.initAll();
        int tableEnd = r._parEnd - _parStart;

        int parStart = r._parStart - _parStart;

        if (parStart != 0 && getParagraph(parStart - 1).isInTable()
                && getParagraph(parStart - 1)._sectionEnd >= r._sectionStart) {
        ...
        return new Table(r._parStart, tableEnd + _parStart, r._doc.getRange(), paragraph.getTableLevel());
    }
    ...
    public String text() {
        ...
        for (int x = _textStart; x < _textEnd; x++) {
            ...
            if (piece.usesUnicode()) // convert the byte pointers to char
                                        // pointers
            {
                start /= 2;
                end /= 2;
