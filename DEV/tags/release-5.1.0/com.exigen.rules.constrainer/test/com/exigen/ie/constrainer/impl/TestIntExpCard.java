...
public class TestIntExpCard extends TestCase {
  ...
  public TestIntExpCard(String name){super(name);}
  ...
  public void testAttachDetachObserver(){
    ...
    public void testIntExpCard(){
      ...
      }catch(Failure f){fail("test failed due to some reason!");}
    }
    ...
    public void testBindAll(){
      ...
      try{
        card.bindAll();
      }catch(Failure f){fail("test of IntExpCard.bindAll() failed!");}

      for (int i=0;i<array.size();i++){
        try{
          assertEquals(cardValue, array.get(i).value());
        }catch(Failure f){fail("test failed due to incorrect behaviour of IntVar.value()");}
      }
    }
    ...
    public void testRemoveIndex(){
      ...
      try{
        ...
        C.propagate();
      }catch(Failure f){fail("test failed: removeIndex(int idx) works incorrectly");}
    ...
    public void testRemoveUnbounds(){
      ...
      try{
        card = new IntExpCard(C, array, cardValue);
      }catch(Failure f){fail("IntExpCard(Constrainer, IntExpArray, card_value) failed");}

      try{
        array.get(indexToBeRemoved[0]).setValue(5);
        array.get(indexToBeRemoved[1]).setValue(5);
        array.get(2).setValue(3);
      }catch(Failure f){fail("test failed due to incorrect work of IntVar.setValue()");}

      try{
        card.removeUnbounds();
      }catch(Failure f){fail("test failed");}

      //all variables except those having been bounded doesn't have to contain cardValue
      // in their domains
      for (int i=0;i<array.size();i++){
        if ((i!=indexToBeRemoved[0]) && (i!=indexToBeRemoved[1]))
      ...
      try{
        card.removeIndex(indexToBeRemoved[0]);
        card.removeIndex(indexToBeRemoved[1]);
      }catch(Failure f){fail("test failed due to incorrect work of IntExpCard.removeIndex(int idx)");}

      try{
        card.removeUnbounds();
      }catch(Failure f){fail("test failed");}
      //all variables except those their indices has been removed doesn't have to contain cardValue
      for (int i=0;i<array.size();i++){
        if ((i!=indexToBeRemoved[0]) && (i!=indexToBeRemoved[1]))
          assertTrue(!array.get(i).contains(cardValue));
        else
