...
public class JDTUtil {
    ...
    static public String[] getJavaProjectClasspath(IProject project)
            throws Exception {
    ...
    static private void addJavaLibraries(IJavaProject javaProject,
            Collection classpath, boolean exportedOnly) throws Exception {
...
//        UtilBase util = new UtilBase();
        ...
        for (int i = 0; i < classpathEntries.length; i++) {
            ...
            String path2 = ResourceUtil.toCanonicalUrl(
                    javaProject.getProject(), path);
            if (!classpath.contains(path2))
                classpath.add(path2);
        }
    }

    static private void addRequiredJavaProjectNames(String projectName,
            Collection projects, boolean exportedOnly) throws Exception {
        ...
        for (int i = 0; i < classpathEntries.length; i++) {
            ...
            case IClasspathEntry.CPE_PROJECT:
                ...
                String requiredProjectName = classpathEntries[i].getPath()
                        .lastSegment();
                boolean exportedOnlyForNonRootProject = true;
                addRequiredJavaProjectNames(requiredProjectName, projects,
                        exportedOnlyForNonRootProject);
                break;
            }
        }
    }
