...
public class OpenLProjectPropertiesLoader
{
	...
	public Properties loadProjectProperties(String projectHome)
	{
		...
		try
		{
			...
			fis = new FileInputStream(new File(getOpenLPropertiesFolder(projectHome),
					OPENL_PROPERTIES_FNAME));
			p.load(fis);
	 ...
	 * @param p
	 * @param override -
	 *          if false, only updates existing properties
	 */
...
//				newProps = true;
			...
			throws IOException
	{
		...
		String[] tokens = StringTool
				.tokenize((String) element.getValue(), splitter);
