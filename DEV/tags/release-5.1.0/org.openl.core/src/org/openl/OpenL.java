 ...
 * considered as a Language Configuration(LC) <href />. You may have as many
 * LCs in your application as you want. Current OpenL architecture allows to
 * have different OpenL configurations in separate classloaders, so they will
 * not interfere with each other. It allows, for example, to have 2 LCs using
...
public class OpenL
{
            ...
            throws OpenLRuntimeException
    {
        ...
        if (error.length > 0) { throw new SyntaxErrorException(
                "Binding Error:", error); }
        return vm.getRunner().run((IBoundMethodNode) bc.getTopNode(),
                new Object[0]);
    }
