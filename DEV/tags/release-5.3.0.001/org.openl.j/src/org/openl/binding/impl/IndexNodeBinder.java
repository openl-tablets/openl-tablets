...
public class IndexNodeBinder extends ANodeBinder {
    ...
    public IBoundNode bind(ISyntaxNode node, IBindingContext bindingContext) throws Exception {
        throw new UnsupportedOperationException("This node always binds  with target");
    }
    ...
    public IBoundNode bindTarget(ISyntaxNode node, IBindingContext bindingContext, IBoundNode targetNode)
        ...
        if (node.getNumberOfChildren() != 1) {
            throw new BoundError(node, "Index node must have  exactly 1 subnode");
        }
        ...
        throw new BoundError(node, "Index operator " + targetNode.getType() + "[" + indexExprType.getName()
                + "] not found");

    }

    IOpenIndex getMehodBasedIndex(IOpenClass indexExprType, IOpenClass componentType, IBindingContext cxt) {

        IMethodCaller reader = MethodSearch.getMethodCaller(INDEX_METHOD_NAME, new IOpenClass[] { indexExprType }, cxt,
                componentType);
        ...
        IMethodCaller writer = MethodSearch.getMethodCaller(INDEX_METHOD_NAME, new IOpenClass[] { indexExprType,
                returnType }, cxt, componentType);

        return new MethodBasedIndex(reader, writer);
    }

    IOpenIndex getMehodBasedIndex(IOpenClass[] types, IBindingContext bindingContext) {

        IMethodCaller reader = BinaryOperatorNodeBinder.findBinaryOperatorMethodCaller(INDEX_METHOD_NAME, types,
                bindingContext);
        ...
        IMethodCaller writer = bindingContext.findMethodCaller("org.openl.operators", INDEX_METHOD_NAME,
