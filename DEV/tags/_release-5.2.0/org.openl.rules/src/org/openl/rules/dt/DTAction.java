...
public class DTAction extends FunctionalRow implements IDTAction
{
	...
	public DTAction(String name, int row, ILogicalTable decisionTable, boolean isReturAction)
	{
		...
		this.isReturnAction = isReturAction;
	}
	...
	public void prepareAction(IOpenClass methodType, IMethodSignature signature, OpenL openl, ModuleOpenClass dtModule, IBindingContextDelegator cxtd, RuleRow ruleRow) throws Exception
	{
		prepare(methodType, signature, openl, dtModule, cxtd, ruleRow);
		
		IParameterDeclaration[] params = getParams();
		
		isSingleReturnParam = (params.length == 1 && params[0].getName().equals(method.getMethodBodyBoundNode().getSyntaxNode().getModule().getCode()));
	}
