		...
		public void preLoad(OpenlToolAdaptor ota) throws Exception 
		{
			...
			for (int i = startRow; i < rows; i++)
			{
				...
				for (int j = 0; j < columns; j++)
				{
					...
					{
						if (isConstructor)
							target = cd.getLiteral(dataModel.getType(), data
									.getLogicalRegion(j, i, 1, 1), ota);
						else
							cd
									.populateLiteral(target, data.getLogicalRegion(j, i, 1, 1),
											ota);
					}
				}
			...
			for (int i = startRow; i < rows; i++)
			{
				...
				for (int j = 0; j < columns; j++)
				{
					...
					if (cd != null && cd.isReference())
					{
						if (cd.isConstructor())
						{
							target = cd.getLink(dataModel.getType(), data.getLogicalRegion(j,
									i, 1, 1), db, cxt);
						} else
...
//			return null;
		...
		public Object getValue(int col, int row)
		{
			...
			Object colObject = dataModel.getDescriptor()[col]
					.getColumnValue(rowObject);
			return colObject;
		}
		...
		public Map<String, Integer> makeUniqueIndex(int colIdx) throws BoundError
		{
			...
			for (int i = startRow; i < rows; i++)
			{
				...
				if (index.containsKey(key))
					throw new BoundError("Duplicated key in an unique index: " + key, new GridCellSourceCodeModule(gridTable));
