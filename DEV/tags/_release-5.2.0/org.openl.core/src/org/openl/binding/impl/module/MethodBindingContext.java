...
public class MethodBindingContext extends BindingContextDelegator
{
    ...
    public MethodBindingContext(IOpenMethodHeader header,
	    IBindingContext delegate, boolean searchInParameterContext, int parameterContextDepthLevel)
    {
	super(delegate);
	this.header = header;
	...
	for (int i = 0; i < params.length; i++)
	{
	    try
	    {
		addParameter(ISyntaxConstants.THIS_NAMESPACE, signature
			.getParameterName(i), params[i]);
