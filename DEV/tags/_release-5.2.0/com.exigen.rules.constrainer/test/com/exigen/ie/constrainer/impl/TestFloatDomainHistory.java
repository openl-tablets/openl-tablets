...
public class TestFloatDomainHistory extends TestCase {
  ...
  public TestFloatDomainHistory(String name) {super(name);}
  ...
  public void testSaveRestore(){
    ...
    }catch(Failure f){fail("test failed: can't create a variable of type FloatVar");}
    ...
    for (int i=1;i<10;i++){
      ...
      }catch(Failure f) {fail("test failed due to incorrect behaviour of FloatVar.setMin(double)");}
  ...
  public void testSetMaxSetMin(){
    ...
    try{
      floatVar.setMax(10);
    }catch(Failure f){fail("test failed due to incorrect behaviour of FloatVar.setMax(double)");}
