...
public class TestFloatCalc extends TestCase{
  private double[] temp = new double[]{1.0, 2.123, Math.E, Math.PI, Math.sqrt(10), 5.125,
                           Math.PI*Math.E, Math.pow(Math.PI, Math.E), Math.pow(Math.E, Math.PI)};
  protected double[] ascendant = null;
  protected double[] descendant = null;

  public TestFloatCalc(String name) {super (name);}
  ...
  public void setUp(){
    double[] temp = new double[]{1.0, 2.123, Math.E, Math.PI, Math.sqrt(10), 5.125,
                           Math.PI*Math.E, Math.pow(Math.PI, Math.E), Math.pow(Math.E, Math.PI)};
  ...
  public void testSqrMin(){
    ...
    for (int i=0; i<ascendant.length-1; i++){
      ...
      min = ascendant[i]; max = ascendant[i+1];
      assertTrue("sqrMin("+min + "," + max +")= " + result + ">" + min + "*" + min,
                 result <= min*min);
      assertTrue("sqrMin("+min + "," + max +")= " + result + ">" + max + "*" + max,
                 result <= max*max);
      min = descendant[i+1];
      max = descendant[i];
      result = FloatCalc.sqrMin(min, max);
      assertTrue("sqrMin("+max + "," + min +")= " + result + ">" + max + "*" + max,
                 result <= max*max);
      assertTrue("sqrMin("+max + "," + min +")= " + result + ">" + min + "*" + min,
                 result <= descendant[i+1]*descendant[i+1]);
    }
    result = FloatCalc.sqrMin(-1, 1);
    assertEquals("sqrMin(-1,1) = " + result,
                 0, result, Double.MIN_VALUE);
  }
