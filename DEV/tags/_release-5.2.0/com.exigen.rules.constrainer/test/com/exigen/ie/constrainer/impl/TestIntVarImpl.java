...
public class TestIntVarImpl extends TestCase {
  ...
  public TestIntVarImpl(String name){super(name);}
  ...
  public void testSetMinAndCreateUndo(){
    Undo[] bit_fastUndo  = new Undo[bit_fast.size()],
           bit_smallUndo = new Undo[bit_fast.size()],
    ...
    for (int i=-10;i<=10;i++){
      ...
      try{
        bit_fast.setMin(i);
      }catch(Failure f){fail("test failed due to incorrect work of IntVar.setMin(int) (Domain type: IntVar.DOMAIN_BIT_FAST)");}

      bit_smallUndo[i-min] = bit_small.createUndo();
      bit_smallUndo[i-min].undoable(bit_small);
      try{
        bit_small.setMin(i);
      }catch(Failure f){fail("test failed due to incorrect work of IntVar.setMin(int) (Domain type: IntVar.DOMAIN_BIT_SMALL)");}

      plainUndo[i-min] = plain.createUndo();
      plainUndo[i-min].undoable(plain);
      try{
        plain.setMin(i);
      }catch(Failure f){fail("test failed due to incorrect work of IntVar.setMin(int) (Domain type: IntVar.DOMAIN_PLAIN)");}
    ...
    try{
      assertEquals("IntVar.value() failed", max, bit_fast.value());
    }catch(Failure f){fail("test failed due to incorrect work of IntVar.setMin(int) (Domain type: IntVar.DOMAIN_BIT_PLAIN)");}
    /*******************************************************************************************************/
  ...
  public void testSetMaxAndCreateUndo(){
    ...
    }catch(Failure f){fail("test failed due to incorrect work of IntVar.setMax(int) (Domain type: IntVar.DOMAIN_BIT_FAST)");}
    try{
      assertEquals("IntVar.value() failed", min, bit_small.value());
    }catch(Failure f){fail("test failed due to incorrect work of IntVar.setMax(int) (Domain type: IntVar.DOMAIN_BIT_SMALL)");}
    try{
      assertEquals("IntVar.value() failed", min, bit_fast.value());
    }catch(Failure f){fail("test failed due to incorrect work of IntVar.setMax(int) (Domain type: IntVar.DOMAIN_BIT_PLAIN)");}
    /*******************************************************************************************************/
    //  restoration from undos
    for (int i=-10;i<10;i++){
  ...
  public void testRemoveValue(){
    Undo[] bit_fastUndo  = new Undo[bit_fast.size()/2+1],
           bit_smallUndo = new Undo[bit_fast.size()/2+1],
    ...
    for (int i=-10;i<=-1;i++){
      ...
      try{
        ...
        bit_fast.removeValue(-i);
      }catch(Failure f){fail("test failed due to incorrect work of IntVar.removeValue(int) (Domain type: IntVar.DOMAIN_BIT_FAST)");}
      ...
      try{
        ...
        bit_small.removeValue(-i);
      }catch(Failure f){fail("test failed due to incorrect work of IntVar.removeValue(int) (Domain type: IntVar.DOMAIN_BIT_SMALL)");}
      ...
      try{
        plain.removeValue(i);
        plain.removeValue(-i);
      }catch(Failure f){fail("test failed due to incorrect work of IntVar.removeValue(int) (Domain type: IntVar.DOMAIN_PLAIN)");}

      int newMin = i+1,
          newMax = -i-1,
          curSize = newMax-newMin+1;
    ...
    try{
      assertEquals("IntVar.value() failed", 0, bit_fast.value());
    }catch(Failure f){fail("test failed due to incorrect work of IntVar.removeValue(int) (Domain type: IntVar.DOMAIN_BIT_PLAIN)");}
    /*******************************************************************************************************/
  ...
  public void testPropagate(){
    TestUtils.TestObserver observer1 = TestUtils.createTestObserver(),
                           observer2 = TestUtils.createTestObserver();
