...
public class OpenlToolTest extends TestCase {
    ...
    public void testMakeMethod() {
        ...
        IOpenMethod m = OpenlTool.makeMethodWithUnknownType(srcCode.asSourceCodeModule(), openl, name, signature,
                declaringClass, depthParameterSearchLevel, cxt);
        assertEquals(JavaOpenClass.INT, m.getType());

        srcCode = new StringValue("if (true) return 5.0; else return 9.1;");
        m = OpenlTool.makeMethodWithUnknownType(srcCode.asSourceCodeModule(), openl, name, signature, declaringClass,
                depthParameterSearchLevel, cxt);
