...
import com.exigen.ie.constrainer.GoalGenerateAll;
...
public class TestElement
{
   ...
   *
   *  ILOG Solver:
   * IlcIntVarArrayI[x[0] y[0]]
   * IlcIntVarArrayI[x[1] y[0]] // ???
...
//    int trace = IntVar.TRACE_ALL;
    ...
    IntExpArray vars = new IntExpArray(c, x,y); vars.name("vars");
    IntArray values = new IntArray(c,1,2); values.name("values");
...
//    int trace = IntVar.TRACE_ALL;
    ...
    IntExpArray vars = new IntExpArray(c, x,y,z); vars.name("vars");
    IntArray values = new IntArray(c,1,2,3,4,5); values.name("values");

//    c.postConstraint("|x-y|==1 && values[ values[x+y<5] ] == z");
    c.postConstraint( x.sub(y).abs().eq(1).and(
                      values.elementAt( values.elementAt( x.add(y).lt(5) )  ).eq(z) ) );
  ...
  /**
   * Test of the ElementConstraint.
   * Generates AllDiff-indeses (channels) and print elementAt-costs.
   */
