...
public class TestIntSetVar extends TestCase {
  ...
  static{C.showInternalNames(true);}

  public TestIntSetVar(String name) {super(name);}
  ...
  static public int intersectionCardinality(Set set1, Set set2){
    ...
    int counter = 0;
    while(iter.hasNext()){
      if (set1.contains((Integer)iter.next()))
        counter++;
    }
    return counter;
  }
  ...
  static class TestCrews{
    ...
    static final String[] names = {"Tom", "David", "Jeremy",  "Ron",  "Joe",  "Bill",  "Fred",
      "Bob",  "Mario", "Ed", "Carol", "Janet", "Tracy", "Marilyn", "Carolyn", "Cathy",
      "Inez", "Jean", "Heather", "Juliet"};

    static final int
    Tom =0, David =1, Jeremy =2, Ron=3, Joe=4,
    Bill=5, Fred=6, Bob=7, Mario=8, Ed=9,
    Carol=10, Janet=11, Tracy=12, Marilyn=13, Carolyn=14,
    Cathy=15, Inez=16, Jean=17, Heather=18, Juliet=19;

    static final int
    iNbMembers = 0, iStewards = 1, iHostesses = 2,
    iFrench = 3, iSpanish = 4, iGerman = 5;

    static int[] Staff = {Tom, David, Jeremy, Ron, Joe,
                  Bill, Fred, Bob, Mario, Ed,
                  Carol, Janet, Tracy, Marilyn, Carolyn,
                  Cathy, Inez, Jean, Heather, Juliet};

    static IntSetVar Stewards = C.addIntSetVar(new int[]{Tom, David, Jeremy, Ron, Joe,
                                       Bill, Fred, Bob, Mario, Ed}, "Stewards");

    static IntSetVar Hostesses = C.addIntSetVar(new int[]{Carol, Janet, Tracy, Marilyn,
                                        Carolyn, Cathy, Inez, Jean,
                                        Heather, Juliet}, "Hostesses");
    ...
    static IntSetVar Spanish = C.addIntSetVar(new int[]{Bill, Fred, Joe, Mario,
                                   Marilyn, Inez, Heather}, "Spanish");

    static IntSetVarArray crews = new IntSetVarArray(C, 10, "crews");

    static private int[][] req =
    {/*crew  nbMembers stewards hostesses french spanish german */
      /*#1*/ {4, 1, 1, 1, 1, 1},
      /*#2*/ {5, 1, 1, 1, 1, 1},
      /*#3*/ {5, 1, 1, 1, 1, 1},
      /*#4*/ {6, 2, 2, 1, 1, 1},
    ...
    static void postTeamConstraints(IntSetVar E,
                                    int n,
                                    int[] nbMembers,
                                    IntSetVarArray memberSets) throws Failure
    {
    ...
    static void test(){
      ...
      try{
        ...
        for (int i=0; i<crews.size(); i++){
          postTeamConstraints(crews.get(i),
                              req[i][iNbMembers],
                              new int[]{
                                     req[i][iStewards],
                                     req[i][iHostesses],
        ...
        assertTrue("Solution for crews task wasn't found" ,
                   C.execute(new GoalIntSetGenerate(crews)));
        checkResults(crews);

      }catch(Failure f){fail("test failed due to Failure exception's being thrown");}
    ...
    static void checkResults(IntSetVarArray array){
      ...
      for (int i=0; i<size-2; i++){
        assertTrue(intersectionCardinality( array.get(i).requiredSet(),
                                            array.get(i+1).requiredSet()) == 0);

      }

      assertTrue(intersectionCardinality( array.get(size-2).requiredSet(),
                                            array.get(size-1).requiredSet()) == 0);
    }
  ...
  public void testEventPropagation(){
    class IntSetVarValueObserver extends Observer{
      ...
      public Object master(){return this;}
      public int subscriberMask(){
        return IntSetVar.VALUE;
      }
    }
