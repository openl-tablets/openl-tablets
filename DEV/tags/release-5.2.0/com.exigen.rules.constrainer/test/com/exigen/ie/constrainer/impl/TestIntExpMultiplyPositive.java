...
public class TestIntExpMultiplyPositive extends TestCase {
  ...
  public TestIntExpMultiplyPositive(String name){super(name);}
  ...
  public void testMul(){
    ...
    try{
      ...
      }catch(Failure f){fail("test failed due to incorrect work of IntExpMultiplyPositive.add(int)");}
    }
    catch(Failure f){
  ...
  public void testSetMaxSetMin(){
    ...
    try{
      ...
      assertEquals((int)42/5, intvar.max());

      mulexp.setMin(-33);
      assertEquals((-33-(-33)%5), mulexp.min());
      assertEquals((int)(-33/5), intvar.min());
