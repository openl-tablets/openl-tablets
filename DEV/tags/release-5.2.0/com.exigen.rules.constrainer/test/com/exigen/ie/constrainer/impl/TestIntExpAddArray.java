...
public class TestIntExpAddArray extends TestCase {
  ...
  public TestIntExpAddArray(String name){super(name);}
  ...
  public void testSetMax(){
    ...
    IntExpAddArray sum = new IntExpAddArray(C, array);
    // setting sum[i=1..10](array[i].min()) as maxValue has to result in assigning values to
    ...
    }catch(Failure f){fail("test failed due to incorrect work of setMax(int)");}

    // setting (sum[i=1..10](array[i].min())-1) as maxValue has to result in throwing Failure
    ...
    try{
      ...
      C.propagate();
    }catch(Failure f){fail("test failed due to incorrect work of IntExp.removeValue(int)");}
    try{
      sum.setMax(10);
      fail("allow to assign maxvalue that less then sum[i=1..n](array[i].min())");
    }catch(Failure f){}
    ...
    try{
      try{
        sum.setMax(15);
        C.propagate();
      }catch(Failure f){fail("incorrect work of setMax(int)");}
  ...
  public void testSetMin(){
    ...
    try{
      try{
        sum.setMin(-15);
        C.propagate();
      }catch(Failure f){fail("incorrect work of setMin(int)");}
