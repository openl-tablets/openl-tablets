...
public class TestIntExpPowIntValue extends TestCase {
  ...
  public TestIntExpPowIntValue(String name){super(name);}
  ...
  public void testFunctionality(){
    ...
    class IntegerValue {
      ...
      public void value(int value){_value = value;}
    }
    ...
    for (int i=0; i< pow.length; i++){
      Goal g = new GoalAnd(
                            exp.equals(pow[i]),
                            new GoalInstantiate(base),
                            new GoalSaveSolution(base, lastResult)
                          );
      C.execute(g, true);
  ...
  public void testMinMaxContains(){
   ...
   /* assertEquals(0, exp.min());
    assertEquals(Math.pow(1, -2), exp.max(), Double.MIN_VALUE);

    exponent = -1;
    base = C.addIntVar(-10, 10, "base", IntVar.DOMAIN_BIT_FAST);
    exp = new IntExpPowIntValue(base, exponent);
    assertEquals(0, exp.min());
    assertEquals(Math.pow(1, -1), exp.max(), Double.MIN_VALUE);
...
*/
    ...
    IntExp powexp = new IntExpOpposite(intvar);
