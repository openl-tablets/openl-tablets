...
public class CccInteger extends CccVariable
{
	...
	public CccGoal getMinimizeGoal()
	{
		...
		{
			//      System.out.println("Creating goal minimize for silution: "+core().solution().executable());
			...
			_goal_minimize.executable(new GoalAnd(new GoalImpl(core().constrainer())
			{
				...
				new GoalMinimize(
					core().getGoalSolution().executable(),
					_constrainer_integer),
				new GoalImpl(core().constrainer())
			{
				public Goal execute()
				{
	...
	public CccGoal getMaximizeGoal()
	{
		if (_goal_maximize == null)
		{
			...
			_goal_maximize.executable(
				new GoalMinimize(
					core().getGoalSolution().executable(),
					_constrainer_integer.neg()));
		}
	...
	{
		return "observers="
			+ _constrainer_integer.observers().size()
			+ "deps="
