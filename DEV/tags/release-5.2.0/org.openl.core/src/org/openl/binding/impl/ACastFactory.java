...
public class ACastFactory implements ICastFactory {
    ...
    public IOpenCast findCast(IOpenClass from, IOpenClass to, IMethodFactory mf) {
        ...
        try {
            castCaller = mf.getMatchingMethod("autocast", new IOpenClass[] { from, to });
        ...
        if (castCaller == null) {
            ...
            distance = 2;
            try {
                castCaller = mf.getMatchingMethod("cast", new IOpenClass[] { from, to });
            } catch (AmbiguousMethodException ex) {
            }
        }
        ...
        try {
            distanceCaller = mf.getMatchingMethod("distance", new IOpenClass[] { from, to });
        } catch (AmbiguousMethodException ex) {
        }
