...
public class RowRecordsAggregate extends Record {
     ...
     *
     * public int construct(int offset, List records) { int k = 0;
     *
     * for (k = offset; k < records.size(); k++) { Record rec = ( Record )
     * records.get(k);
     *
     * if (!rec.isInValueSection() && !(rec instanceof UnknownRecord)) { break; }
     * if (rec.getSid() == RowRecord.sid) { insertRow(( RowRecord ) rec); } }
     * return k; }
     */
    ...
    public int writeHidden(RowRecord rowRecord, int row, boolean hidden) {
        int level = rowRecord.getOutlineLevel();
        while (rowRecord != null && getRow(row).getOutlineLevel() >= level) {
