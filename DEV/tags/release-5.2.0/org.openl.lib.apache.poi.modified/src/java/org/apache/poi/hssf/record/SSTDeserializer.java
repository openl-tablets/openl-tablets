...
class SSTDeserializer {
    ...
    /**
     * this is true if the string is a far east string or some other wierd
     * string
     */
    ...
    /**
     * Okay, we are doing some major cheating here. Because we can't handle rich
     * text strings properly we end up getting duplicate strings. To get around
     * this I'm doing two things: 1. Converting rich text to normal text and 2.
     * If there's a duplicate I'm adding a space onto the end. Sneaky perhaps
     ...
     * <code>srcOffset+length-1</code> were first copied to a temporary array
     * with <code>length</code> components and then the contents of the
     * temporary array were copied into positions <code>dstOffset</code>
     * through <code>dstOffset+length-1</code> of the destination array.
     * <p>
     * If <code>dst</code> is <code>null</code>, then a
     * <code>NullPointerException</code> is thrown.
     * <p>
     * If <code>src</code> is <code>null</code>, then a
     * <code>NullPointerException</code> is thrown and the destination array
     * is not modified.
     * <p>
     ...
     * array.
     * <li>The <code>src</code> argument and <code>dst</code> argument
     ...
     * <code>srcOffset</code> through <code>srcOffset+length-1</code> cannot
     * be converted to the component type of the destination array by assignment
     * conversion, an <code>ArrayStoreException</code> is thrown. In this
     * case, let <b><i>k</i></b> be the smallest nonnegative integer less
     * than length such that <code>src[srcOffset+</code><i>k</i><code>]</code>
     * cannot be converted to the component type of the destination array; when
     * the exception is thrown, source array components from positions
     * <code>srcOffset</code> through <code>srcOffset+</code><i>k</i><code>-1</code>
     * will already have been copied to destination array positions
     * <code>dstOffset</code> through <code>dstOffset+</code><i>k</I><code>-1</code>
     ...
     *                array could not be stored into the <code>dest</code>
    ...
    /**
     * @return the number of characters we expect in the first sub-record in a
    ...
    public void manufactureStrings(final byte[] data, final int initialOffset) {
        ...
        while (offset < dataSize) {
            ...
            if (stringContinuesOverContinuation) {
                ...
                // Only read the size of the string or whatever is left before
                // the
                // continuation
                charsRead = Math.min(charsRead, calculateCharCount(remainingBytes));
                setContinuationCharsRead(charsRead);
                if (charsRead == charCount) {
                    // Since all of the characters will have been read, but the
                    // entire string (including formatting runs etc)
                    // hasnt, Compute the number of bytes to skip when the
                    // continue record starts
    ...
    private int offsetForContinuedRecord(int stringRemainderSizeInBytes) {
        ...
        if (stringRemainderSizeInBytes != 0) {
            // If a portion of the string remains then the wideChar options byte
            // is repeated,
    ...
    /**
     * Process a Continue record. A Continue record for an SST record contains
     * the same kind of data that the SST record contains, with the following
     * exceptions:
     * <P>
     * <OL>
     * <LI>The string counts at the beginning of the SST record are not in the
     * Continue record
     * <LI>The first string in the Continue record might NOT begin with a size.
     * If the last string in the previous record is continued in this record,
     * the size is determined by that last string in the previous record; the
     * first string will begin with a flag byte, followed by the remaining bytes
     * (or words) of the last string from the previous record. Otherwise, the
     * first string in the record will begin with a string length
     * </OL>
     *
    ...
    public void processContinueRecord(final byte[] record) {
        if (isStringFinished()) {
            ...
            // reset the wide bit because that can change across a continuation.
            // the fact that it's
            // actually rich text doesn't change across continuations even
     ...
     * simply suck the remaining string data into the existing
     * <code>unfinishedString</code>.
     *
    ...
    private int processString(final byte[] data, final int dataIndex, final int characters) {
        ...
        arraycopy(data, dataIndex + stringHeaderOverhead(), unicodeStringBuffer, SSTRecord.STRING_MINIMAL_OVERHEAD,
                bytesRead);
        // Create the unicode string
        UnicodeString string = new UnicodeString(UnicodeString.sid, (short) unicodeStringBuffer.length,
                unicodeStringBuffer);
    ...
    /**
     * Detemines the option types for the string (ie, compressed or uncompressed
     * unicode, rich text string or plain string etc) and calculates the length
    ...
    /**
     * Reads the remainder string and any subsequent strings from the
     * continuation record.
     *
    ...
    private void readStringRemainder(final byte[] record) {
        ...
        byte[] unicodeStringData = new byte[SSTRecord.STRING_MINIMAL_OVERHEAD + stringRemainderSizeInBytes];
        ...
        arraycopy(record, LittleEndianConsts.BYTE_SIZE, unicodeStringData, SSTRecord.STRING_MINIMAL_OVERHEAD,
                stringRemainderSizeInBytes);

        // use special constructor to create the final string
        UnicodeString string = new UnicodeString(UnicodeString.sid, (short) unicodeStringData.length,
                unicodeStringData, unfinishedString);
        Integer integer = new Integer(strings.size());
    ...
    private int stringHeaderOverhead() {
        return SSTRecord.STRING_MINIMAL_OVERHEAD + (richText ? LittleEndianConsts.SHORT_SIZE : 0)
    ...
    /**
     * Calculates the size of the string in byes. This figure includes all the
     * over heads for the string.
     */
    private int totalStringSize() {
        return stringSizeInBytes() + stringHeaderOverhead() + LittleEndianConsts.INT_SIZE * runCount + extensionLength;
    }
