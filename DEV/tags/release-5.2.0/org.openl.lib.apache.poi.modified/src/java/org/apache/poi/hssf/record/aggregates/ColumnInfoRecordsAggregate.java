...
public class ColumnInfoRecordsAggregate extends Record {
    ...
    public void collapseColInfoRecords(int columnIdx) {
        ...
        boolean columnsMatch = previousCol.getXFIndex() == currentCol.getXFIndex()
    ...
    public boolean isColumnGroupCollapsed(int idx) {
        ...
        if (getColInfo(endOfOutlineGroupIdx).getLastColumn() + 1 != getColInfo(endOfOutlineGroupIdx + 1)
                .getFirstColumn()) {
            return false;
        } else {
    ...
    public boolean isColumnGroupHiddenByParent(int idx) {
        ...
        if (endOfOutlineGroupIdx >= records.size()) {
            endLevel = 0;
            endHidden = false;
        } else if (getColInfo(endOfOutlineGroupIdx).getLastColumn() + 1 != getColInfo(endOfOutlineGroupIdx + 1)
                .getFirstColumn()) {
            endLevel = 0;
            endHidden = false;
        } else {
        ...
        if (startOfOutlineGroupIdx <= 0) {
            startLevel = 0;
            startHidden = false;
        } else if (getColInfo(startOfOutlineGroupIdx).getFirstColumn() - 1 != getColInfo(startOfOutlineGroupIdx - 1)
                .getLastColumn()) {
            startLevel = 0;
            startHidden = false;
        } else {
    ...
    public void setColumn(short column, Short width, Integer level, Boolean hidden, Boolean collapsed) {
        ...
        if (ci != null) {
            ...
            if (!columnChanged) {
            ...
            } else if ((ci.getFirstColumn() == column) && (ci.getLastColumn() == column)) { // if
                                                                                            // its
                                                                                            // only
                                                                                            // for
                                                                                            // this
                ...
                // okay so the width is different but the first or last column
                // == the column we'return setting
            ...
            // okay so there ISN'T a column info record that cover's this column
            // so lets create one!
            ColumnInfoRecord nci = (ColumnInfoRecord) createColInfo();
