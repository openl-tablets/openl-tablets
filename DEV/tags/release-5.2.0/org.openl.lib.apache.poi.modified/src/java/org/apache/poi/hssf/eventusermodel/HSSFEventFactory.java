 ...
 *
 * This will cause your file to be processed a record at a time. Each record
 * with a static id matching one that you have registed in your HSSFRequest will
 * be passed to your associated HSSFListener.
 *
...
public class HSSFEventFactory {
    ...
    public short abortableProcessWorkbookEvents(HSSFRequest req, POIFSFileSystem fs) throws IOException,
            HSSFUserException {
    ...
    protected short genericProcessEvents(HSSFRequest req, InputStream in) throws IOException, HSSFUserException {
        ...
        process: try {
            ...
            while (bytesread > 0) {
                ...
                //
                // for some reasons we have to make the workbook to be at least
                // 4096 bytes
                // but if we have such workbook we fill the end of it with zeros
                // (many zeros)
                //
                // it is not good:
                // if the length( all zero records ) % 4 = 1
                // e.g.: any zero record would be readed as 4 bytes at once ( 2
                // - id and 2 - size ).
                // And the last 1 byte will be readed WRONG ( the id must be 2
                // bytes )
                //
                // So we should better to check if the sid is zero and not to
                // read more data
                // The zero sid shows us that rest of the stream data is a fake
                // to make workbook
                // certain size
                //
                if (sid == 0) {
                ...
                if (sid != ContinueRecord.sid) {
                    ...
                    Record[] recs = RecordFactory.createRecord(sid, size, data);

                    if (recs.length > 1) { // we know that the multiple
                        for (int k = 0; k < (recs.length - 1); k++) { // record
                                                                        // situations
                                                                        // do
                                                                        // not
                            userCode = req.processRecord(recs[k]); // contain
                                                                    // continue
                                                                    // records
                            if (userCode != 0) {
                                break process;
                ...
                } else { // we do have a continue record
                    short size = LittleEndian.readShort(in);
                    byte[] data = new byte[size];
            ...
            throw new RecordFormatException("Error reading bytes" + "while processing record sid=" + sid);
        }
        return userCode;
     ...
     * during this call the method will return just as with
     * <code>abortableProcessEvents</code>, but no user code or
     * <code>HSSFUserException</code> will be passed back.
     *
     * @see org.apache.poi.poifs.filesystem.POIFSFileSystem#createDocumentInputStream(String)
     * @param req an Instance of HSSFRequest which has your registered listeners
     * @param in a DocumentInputStream obtained from POIFS's POIFSFileSystem
    ...
    public void processEvents(HSSFRequest req, InputStream in) throws IOException {
        try {
            genericProcessEvents(req, in);
                                             ...
                                             * If an HSSFUserException user
                                             * exception is thrown, ignore it.
                                             */
        }
    }
