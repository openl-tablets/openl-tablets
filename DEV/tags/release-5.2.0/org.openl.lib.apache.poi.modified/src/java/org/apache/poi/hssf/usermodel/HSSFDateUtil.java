...
public class HSSFDateUtil {
    ...
    private static final long DAY_MILLISECONDS = 24 * 60 * 60 * 1000;
                                            private static final double CAL_1900_ABSOLUTE = absoluteDay(new GregorianCalendar(1900, Calendar.JANUARY,
            1)) - 2.0;
    /**
    ...
    private static Calendar dayStart(final Calendar cal) {
        cal.get(Calendar.HOUR_OF_DAY); // force recalculation of internal
                                        // fields
     ...
     * representation, which is the number of days since 1/1/1900. Fractional
    ...
    public static double getExcelDate(Date date) {
        ...
        calStart.setTime(date); // If date includes hours, minutes, and seconds,
                                // set them to 0
        if (calStart.get(Calendar.YEAR) < 1900) {
            ...
            double fraction = (((calStart.get(Calendar.HOUR_OF_DAY) * 60 + calStart.get(Calendar.MINUTE)) * 60 + calStart
                    .get(Calendar.SECOND)) * 1000 + calStart.get(Calendar.MILLISECOND))
                    / (double) DAY_MILLISECONDS;
            calStart = dayStart(calStart);

            return fraction + absoluteDay(calStart) - CAL_1900_ABSOLUTE;
        }
    }
     ...
     * value, that is the comparison
     * <CODE>excelDate == getExcelDate(getJavaDate(excelDate,false))</CODE> is
     * not always true. For example if default timezone is
     * <code>Europe/Copenhagen</code>, on 2004-03-28 the minute after 01:59
     * CET is 03:00 CEST, if the excel date represents a time between 02:00 and
     * 03:00 then it is converted to past 03:00 summer time
     *
     * @param date The Excel date.
     * @param use1904windowing true if date uses 1904 windowing, or false if
     *            using 1900 date windowing.
    ...
    public static Date getJavaDate(double date, boolean use1904windowing) {
        if (isValidExcelDate(date)) {
            ...
            int millisecondsInDay = (int) ((date - Math.floor(date)) * DAY_MILLISECONDS + 0.5);
    ...
    /**
     * Check if a cell contains a date Since dates are stored internally in
     * Excel as double values we infer it is a date if it is formatted as such.
     *
    ...
    /**
     * given a format ID this will check whether the format represents an
     * internal date format or not.
     */
