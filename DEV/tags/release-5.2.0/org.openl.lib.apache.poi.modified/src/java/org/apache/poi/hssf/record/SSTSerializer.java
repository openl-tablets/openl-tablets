...
class SSTSerializer {
    ...
    public SSTSerializer(List recordLengths, BinaryTree strings, int numStrings, int numUniqueStrings) {
        ...
        sstRecordHeader = new SSTRecordHeader(numStrings, numUniqueStrings);
    ...
    /**
     * Create a byte array consisting of an SST record and any required Continue
     * records, ready to be written out.
     * <p>
     * If an SST record and any subsequent Continue records are read in to
     * create this instance, this method should produce a byte array that is
     * identical to the byte array produced by concatenating the input records'
     * data.
     *
    ...
    /**
     * Large records are serialized to an SST and to one or more CONTINUE
     * records. Joy. They have the special characteristic that they can change
     * the option field when a single string is split across to a CONTINUE
     * record.
     */
    private void serializeLargeRecord(int record_size, int record_length_index, byte[] buffer, int offset) {
        ...
        while (totalWritten != record_size) {
            ...
            RecordProcessor recordProcessor = new RecordProcessor(buffer, recordLength, numStrings, numUniqueStrings);
            ...
            if (lastneedcontinue) {
                ...
                // completely
                stringReminant = recordProcessor.writeStringRemainder(lastneedcontinue, stringReminant, offset,
                        totalWritten);
            }

            // last string's remnant, if any, is cleaned up as best as can be
            // done ... now let's try and write
            // some more strings
            for (; stringIndex < strings.size(); stringIndex++) {
                ...
                if (stringIndex % ExtSSTRecord.DEFAULT_BUCKET_SIZE == 0) {
                    ...
                    if (index < ExtSSTRecord.MAX_BUCKETS) {
                        bucketAbsoluteOffsets[index] = offset + totalWritten + recordProcessor.getRecordOffset()
                                - startOfSST;
                        bucketRelativeOffsets[index] = offset + totalWritten + recordProcessor.getRecordOffset()
                                - startOfRecord;
                    }
                }
    ...
    /**
     * This case is chosen when an SST record does not span over to a continue
     * record.
     */
    private void serializeSingleSSTRecord(byte[] data, int offset, int record_length_index) {
        ...
        for (int k = 0; k < strings.size(); k++) {
            ...
            System.arraycopy(getUnicodeString(k).serialize(), 0, data, pos + offset, getUnicodeString(k)
                    .getRecordSize());
