 ...
 * String cells cannot conatin numbers and numeric cells cannot contain strings
 * (at least according to our model). Client apps should do the conversions
 * themselves. Formula cells are treated like string cells, simply containing a
 * formula string. They'll be rendered differently.
 * <p>
...
public class HSSFCell {
    ...
    protected HSSFCell(Workbook book, Sheet sheet, int row, CellValueRecordInterface cval) {
        ...
        switch (cellType) {
            ...
            case CELL_TYPE_STRING:
                stringValue = book.getSSTString(((LabelSSTRecord) cval).getSSTIndex());
                break;
        ...
        setCellStyle(new HSSFCellStyle(cval.getXFIndex(), xf));
    }
     ...
     * @deprecated As of 22-Jan-2002 use
     * @see org.apache.poi.hssf.usermodel.HSSFRow#createCell(short) and use
     *      setCellValue to specify the type lazily.
     */
    ...
    private int determineType(CellValueRecordInterface cval) {
        ...
        switch (sid) {
            ...
            case BoolErrRecord.sid:
                ...
                retval = (boolErrRecord.isBoolean()) ? HSSFCell.CELL_TYPE_BOOLEAN : HSSFCell.CELL_TYPE_ERROR;
                break;
        }
        return retval;
    }
    ...
    public String getCellFormula() {
        ...
        String retval = FormulaParser.toFormulaString(book, ((FormulaRecordAggregate) record).getFormulaRecord()
                .getParsedExpression());
    ...
    /**
     * used for internationalization, currently 0 for compressed unicode or 1
     * for 16-bit
     *
     * @see #ENCODING_COMPRESSED_UNICODE
     * @see #ENCODING_UTF_16
     *
     * @return 1 or 0 for compressed or uncompressed (used only with String
     *         type)
     */
    ...
    public void setAsActiveCell() {
        sheet.setActiveCellRow(row);
        sheet.setActiveCellCol(cellNum);
    }

    /**
     * set a error value for the cell
     *
     * @param value the error value to set this cell to. For formulas we'll set
     *            the precalculated value ??? IS THIS RIGHT??? , for errors
     *            we'll set its value. For other types we will change the cell
     *            to an error cell and set its value.
     */
    ...
    /**
     * set the cells type (numeric, formula or string) -- DONT USE FORMULAS IN
     * THIS RELEASE WE'LL THROW YOU A RUNTIME EXCEPTION IF YOU DO
     *
     * @see #CELL_TYPE_NUMERIC
     * @see #CELL_TYPE_STRING
     * @see #CELL_TYPE_FORMULA
     ...
     * The
     *
     * @see #setCellValue method will call this method with false in setValue
    ...
    private void setCellType(int cellType, boolean setValue) {
        ...
        switch (cellType) {
            ...
            case CELL_TYPE_FORMULA:
                ...
                frec.setXFIndex(cellStyle.getIndex());
                frec.setRow(row);
                record = frec;
                break;
            ...
            case CELL_TYPE_NUMERIC:
                ...
                nrec.setXFIndex(cellStyle.getIndex());
                nrec.setRow(row);
                record = nrec;
                break;
            ...
            case CELL_TYPE_STRING:
                ...
                lrec.setRow(row);
                lrec.setXFIndex(cellStyle.getIndex());
                if (setValue) {
                    if ((getStringCellValue() != null) && (!getStringCellValue().equals(""))) {
                        int sst = 0;
            ...
            case CELL_TYPE_BLANK:
                ...
                // During construction the cellStyle may be null for a Blank
                // cell.
                if (cellStyle != null) {
                    brec.setXFIndex(cellStyle.getIndex());
                } else {
            ...
            case CELL_TYPE_BOOLEAN:
                ...
                boolRec.setXFIndex(cellStyle.getIndex());
                boolRec.setRow(row);
                record = boolRec;
                break;
            ...
            case CELL_TYPE_ERROR:
                ...
                errRec.setXFIndex(cellStyle.getIndex());
                errRec.setRow(row);
                record = errRec;
                break;
        }
     ...
     *            set the precalculated value, for booleans we'll set its value.
     *            For other types we will change the cell to a boolean cell and
     *            set its value.
     */
     ...
     *
     * @param value the date value to set this cell to. For formulas we'll set
     ...
     *            set the precalculated value, for numerics we'll set its value.
     *            For other types we will change the cell to a numeric cell and
     *            set its value.
     */
    ...
    /**
     * set a string value for the cell. Please note that if you are using full
     * 16 bit unicode you should call <code>setEncoding()</code> first.
     *
    ...
    /**
     * set the encoding to either 8 or 16 bit. (US/UK use 8-bit, rest of the
     * western world use 16bit)
     *
     * @see #ENCODING_COMPRESSED_UNICODE
     * @see #ENCODING_UTF_16
     *
     * @param encoding either ENCODING_COMPRESSED_UNICODE (0) or ENCODING_UTF_16
     *            (1)
     */
