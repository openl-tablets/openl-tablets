...
public class TextObjectRecord extends TextObjectBaseRecord {
    ...
    int continueRecordCount = 0; // how many times has continue record been
                                    // called?
    ...
    private ContinueRecord createContinue2() {
        ...
        for (int i = 0; i < str.numFormattingRuns(); i++) {
            ...
            pos += 2;
            LittleEndian.putShort(c2Data, pos, str.getFontOfFormattingRun(i) == HSSFRichTextString.NO_FONT ? 0 : str
    ...
    public int serialize(int offset, byte[] data) {
        // Temporarily blank out str so that record size is calculated without
        // the continue records.
        ...
        if (bytesWritten1 != getRecordSize()) {
            throw new RecordFormatException(bytesWritten1 + " bytes written but getRecordSize() reports "
                    + getRecordSize());
        }
