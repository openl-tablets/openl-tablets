 ...
 * {@link PropertySet#PropertySet(byte[])}. If the constructor's argument is
 * not in the Horrible Property Set Format, i.e. not a property set stream, or
 * if any other error occurs, an appropriate exception is thrown.
 * </p>
 *
 * <p>
...
public class PropertySet {
     ...
     * </p>
     */
    final static byte[] BYTE_ORDER_ASSERTION = new byte[] { (byte) 0xFE, (byte) 0xFF };
     ...
     */
    public static boolean isPropertySetStream(final byte[] src, int offset, final int length) {
        /*
     ...
     * @throws NoPropertySetStreamException if the byte array is not a property
     *             set stream.
     */
     ...
     * @param offset The offset in <var>stream</var> where the stream data
     *            begin. If the stream data begin with the first byte in the
     *            array, the <var>offset</var> is 0.
     * @param length The length of the stream data.
     * @throws NoPropertySetStreamException if the byte array is not a property
     *             set stream.
     */
    public PropertySet(final byte[] stream, final int offset, final int length) throws NoPropertySetStreamException {
     ...
     * @throws IOException if the {@link InputStream} cannot not be accessed as
     *             needed.
     */
    public PropertySet(final InputStream stream) throws NoPropertySetStreamException, MarkUnsupportedException,
            IOException {
     ...
     *
     * <p>
     * <strong>FIXME:</strong> Return an <code>int</code> instead of a
     * <code>long</code> in the next major version, i.e. when incompatible
     * changes are allowed.
     * </p>
     *
     ...
     * the first {@link Section}. However, it can only be used if the {@link
     * PropertySet} contains exactly one {@link Section}, so check
     * {@link #getSectionCount} first!
     * </p>
     *
     ...
     * ID. If the property is not available, <code>null</code> is returned and
     * a subsequent call to {@link #wasNull} will return <code>true</code> .
     * </p>
     *
     ...
     * subsequent call to {@link #wasNull} will return <code>true</code> to
     * let the caller distinguish that case from a real property value of 0.
     * </p>
     *
     * @param id The property ID
    ...
    private void init(final byte[] src, int offset, final int length) {
        ...
        offset += LittleEndianConsts.INT_SIZE;
        if (sectionCount <= 0) {
            throw new HPSFRuntimeException("Section count " + sectionCount + " must be greater than 0.");
        }
        ...
        for (int i = 0; i < sectionCount; i++) {
            ...
            offset += ClassID.LENGTH + LittleEndianConsts.INT_SIZE;
            sections.add(s);
        }
    }
    ...
    public boolean isSummaryInformation() {
        return Util.equal(((Section) sections.get(0)).getFormatID().getBytes(), SectionIDMap.SUMMARY_INFORMATION_ID);
    }
     ...
     * @throws NoSingleSectionException if the {@link PropertySet} has more than
     *             one {@link Section}.
     */
