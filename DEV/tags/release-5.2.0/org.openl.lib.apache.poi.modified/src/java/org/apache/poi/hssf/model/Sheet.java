 ...
 * @author Shawn Laubach (slaubach at apache dot org) Gridlines, Headers,
 *         Footers, and PrintSetup
...
public class Sheet implements Model {
     ...
     *
     * @param records array containing those records in the sheet in sequence
     *            (normally obtained from RecordFactory)
     * @param sheetnum integer specifying the sheet's number (0,1 or 2 in this
     *            release)
     * @return Sheet object
     */
    ...
    public static Sheet createSheet(List records, int sheetnum) {
        if (log.check(POILogger.DEBUG)) {
            log.log(POILogger.DEBUG, "Sheet createSheet (exisiting file) assumed offset 0");
        }
     ...
     *
     * @param recs array containing those records in the sheet in sequence
     *            (normally obtained from RecordFactory)
     * @param sheetnum integer specifying the sheet's number (0,1 or 2 in this
     *            release)
    ...
    public static Sheet createSheet(List recs, int sheetnum, int offset) {
        if (log.check(POILogger.DEBUG)) {
            log.logFormatted(POILogger.DEBUG, "Sheet createSheet (existing file) with %", new Integer(recs.size()));
        }
        Sheet retval = new Sheet();
            ...
            if (rec.getSid() == RowRecord.sid) {
                ...
                if ((rownum % 32) == 0) {
                    ...
                    for (int rn = recnum; rn < records.size(); rn++) {
                        ...
                        if ((!rec.isInValueSection()) || (rec.getSid() == RowRecord.sid)) {

                            // here is the next row or last value record
                            records.add(rn, createDBCell(offset, rowOffsets, index));
                            recnum = rn;
                            break;
                        }
                    }
    ...
    private void addDbCellToIndex(int offset, IndexRecord index) {
        ...
        // stupid but whenever we add an offset that causes everything to be
        // shifted down 4
    ...
    public void addRow(RowRecord row) {
        ...
        /*
         * for (int k = loc; k < records.size(); k++) { Record rec = ( Record )
         * records.get(k);
         *
         * if (rec.getSid() == IndexRecord.sid) { index = ( IndexRecord ) rec; }
         * if (rec.getSid() == RowRecord.sid) { RowRecord rowrec = ( RowRecord )
         * rec;
         *
         * if (rowrec.getRowNumber() > row.getRowNumber()) { records.add(k,
         * row); loc = k; break; } } if (rec.getSid() == WindowTwoRecord.sid) {
         * records.add(k, row); loc = k; break; } } if (index != null) { if
         * (index.getLastRowAdd1() <= row.getRowNumber()) {
    ...
    public void addValueRecord(int row, CellValueRecordInterface col) {
        checkCells();
        log.logFormatted(POILogger.DEBUG, "add value record  row,loc %,%", new int[] { row, loc });
        DimensionsRecord d = (DimensionsRecord) records.get(getDimsLoc());
         ...
         * rec;
         *
         * if (rowrec.getRowNumber() == col.getRow()) { records.add(k + 1, col);
         * loc = k; if (rowrec.getLastCol() <= col.getColumn()) {
         * rowrec.setLastCol((( short ) (col.getColumn() + 1))); } break; } } }
         */
    }
    ...
    public int aggregateDrawingRecords(DrawingManager drawingManager) {
        ...
        if (noDrawingRecordsFound) {
            ...
            int startloc = loc;
            while (loc + 1 < records.size() && records.get(loc) instanceof DrawingRecord
    ...
    /**
     * in the event the record is a dimensions record, resets both the loc index
     * and dimsloc index
     */
    ...
    public Sheet cloneSheet() {
        ArrayList clonedRecords = new ArrayList(records.size());
        for (int i = 0; i < records.size(); i++) {
            Record rec = (Record) ((Record) records.get(i)).clone();
            // Need to pull out the Row record and the Value records from their
            // Aggregates.
            // This is probably the best way to do it since we probably dont
            // want the createSheet
     ...
     * interface conformance with NO impelmentation). In short, you need to call
     ...
     * written to the Shared String tabel (SSTRecord) within the Workbook.
     *
    ...
    /**
     * create a BLANK record (does not add it to the records contained in this
     * sheet)
     *
    ...
    public BlankRecord createBlank(int row, short col) {
        ...
        // short[]
        log.logFormatted(POILogger.DEBUG, "create blank row,col %,%", new int[] { row, col });
    ...
    /**
     * creates the CalcCount record and sets it to 0x64 (default number of
     * iterations)
     *
    ...
    /**
     * Attempts to parse the formula into PTGs and create a formula record DOES
     * NOT WORK YET
     *
    ...
    public FormulaRecord createFormula(int row, short col, String formula) {
        ...
        rec.setValue(0);
        rec.setXFIndex((short) 0x0f);
        FormulaParser fp = new FormulaParser(formula, null); // fix - do we
                                                                // need this
                                                                // method?
        fp.parse();
    ...
    /**
     * get the NEXT RowRecord or CellValueRecord(from LOC). The first record
     * that is a Row record or CellValueRecord(starting at LOC) will be
     * returned.
     * <P>
     * This method is "loc" sensitive. Meaning you need to set LOC to where you
     * want it to start searching. If you don't know do this:
     ...
     *
     * @return RowRecord representing the next row record or
     *         CellValueRecordInterface representing the next cellvalue or NULL
    ...
    /*
     * public Record getNextRowOrValue() { POILogger.DEBUG((new
     * StringBuffer("getNextRow loc= ")).append(loc) .toString()); if
     * (this.getLoc() < records.size()) { for (int k = this.getLoc(); k <
     * records.size(); k++) { Record rec = ( Record ) records.get(k);
     *
     * this.setLoc(k + 1); if (rec.getSid() == RowRecord.sid) { return rec; }
     * else if (rec.isValue()) { return rec; } } } return null; }
     */

    /**
     * creates the HCenter Record and sets it to false (don't horizontally
     * center)
     *
    ...
    /**
     * Create a LABELSST Record (does not add it to the records contained in
     * this sheet)
     *
     * @param row the row the LabelSST is a member of
     * @param col the column the LabelSST defines
     * @param index the index of the string within the SST (use workbook
     *            addSSTString method)
    ...
    public LabelSSTRecord createLabelSST(int row, short col, int index) {
        log.logFormatted(POILogger.DEBUG, "create labelsst row,col,index %,%,%", new int[] { row, col, index });
    ...
    /**
     * Create a NUMBER Record (does not add it to the records contained in this
     * sheet)
     *
     ...
     *
     * @return NumberRecord for that row, col containing that value as added to
     *         the sheet
     */
    ...
    public NumberRecord createNumber(int row, short col, double value) {
        log.logFormatted(POILogger.DEBUG, "create number row,col,value %,%,%", new double[] { row, col, value });
    ...
    /**
     * creates the PrintSetup Record and sets it to defaults and marks it
     * invalid
     *
    ...
    /**
     * Create a row record. (does not add it to the records contained in this
     * sheet)
     *
     * @param row number
    ...
    /**
     * creates the VCenter Record and sets it to false (don't horizontally
     * center)
     *
    ...
    /**
     * get the width of a given column in units of 1/20th of a point width
     * (twips?)
     *
     * @param column index
     * @see org.apache.poi.hssf.record.DefaultColWidthRecord
    ...
    /**
     * get the location of the DimensionsRecord (which is the last record before
     * the value section)
     *
    ...
    /**
     * Returns the location pointer to the first record to look for when adding
     * rows/values
     *
     */
    ...
    public RowRecord getNextRow() {
        ...
        /*
         * if (this.getLoc() < records.size()) { for (int k = this.getLoc(); k <
         * records.size(); k++) { Record rec = ( Record ) records.get(k);
         *
         * this.setLoc(k + 1); if (rec.getSid() == RowRecord.sid) { return (
         * RowRecord ) rec; } } }
         */
    }
     ...
     * record (starting at LOC) will be returned.
     *
     * <P>
     * This method is "loc" sensitive. Meaning you need to set LOC to where you
     * want it to start searching. If you don't know do this:
    ...
    public CellValueRecordInterface getNextValueRecord() {
        ...
        /*
         * if (this.getLoc() < records.size()) { for (int k = getLoc(); k <
         * records.size(); k++) { Record rec = ( Record ) records.get(k);
         *
         * this.setLoc(k + 1); if (rec instanceof CellValueRecordInterface) {
         * return ( CellValueRecordInterface ) rec; } } } return null;
         */
    }
    ...
    /**
     * Returns the number of low level binary records in this sheet. This
     * adjusts things for the so called AgregateRecords.
     *
     * @see org.apache.poi.hssf.record.Record
     */
    ...
    public int getNumRecords() {
        ...
        if (log.check(POILogger.DEBUG)) {
            ...
            log.logFormatted(POILogger.DEBUG, "returning % + % + % - 2 = %", new int[] { records.size(),
                    cells.getPhysicalNumberOfCells(), rows.getPhysicalNumberOfRows(),
                    records.size() + cells.getPhysicalNumberOfCells() + rows.getPhysicalNumberOfRows() - 2 });
        }
        return records.size() + cells.getPhysicalNumberOfCells() + rows.getPhysicalNumberOfRows() - 2;
    }
    ...
    public RowRecord getRow(int rownum) {
         ...
         * records.size(); k++) { Record rec = ( Record ) records.get(k);
         *
         * this.setLoc(k + 1); if (rec.getSid() == RowRecord.sid) { if (((
         * RowRecord ) rec).getRowNumber() == rownum) { return ( RowRecord )
         * rec; } } } }
         */
    ...
    public void removeRow(RowRecord row) {
         ...
         * for (int k = loc; k < records.size(); k++) { Record rec = ( Record )
         * records.get(k);
         *  // checkDimsLoc(rec,k); if (rec.getSid() == RowRecord.sid) {
         * RowRecord rowrec = ( RowRecord ) rec;
         *
         * if (rowrec.getRowNumber() == row.getRowNumber()) { records.remove(k);
         * break; } } if (rec.getSid() == WindowTwoRecord.sid) { break; } }
         */
    }
    ...
    public void removeValueRecord(int row, CellValueRecordInterface col) {
        checkCells();
        log.logFormatted(POILogger.DEBUG, "remove value record row,dimsloc %,%", new int[] { row, dimsloc });
        loc = dimsloc;
         ...
         * for (int k = loc; k < records.size(); k++) { Record rec = ( Record )
         * records.get(k);
         *  // checkDimsLoc(rec,k); if (rec.isValue()) {
         * CellValueRecordInterface cell = ( CellValueRecordInterface ) rec;
         *
         * if ((cell.getRow() == col.getRow()) && (cell.getColumn() ==
         * col.getColumn())) { records.remove(k); break; } } }
         */
    }
     ...
     * worries.
     *
     * @param newval - a record supporting the CellValueRecordInterface. this
     *            will replace the cell value with the same row and column. If
     *            there isn't one, one will be added.
     */
    ...
    public void replaceValueRecord(CellValueRecordInterface newval) {
         ...
         *
         * while (oldval != null) { if (oldval.isEqual(newval)) { records.set((
         * short ) (getLoc() - 1), newval); return; } oldval =
         * getNextValueRecord(); } addValueRecord(newval.getRow(), newval);
         * setLoc(dimsloc);
         */
    }
     ...
     * write the sheet out.
     *
     * @return byte[] array containing the binary representation of the records
     *         in this sheet
     *
     */
    ...
    public byte[] serialize() {
        ...
        for (int k = 0; k < records.size(); k++) {
            ...
            pos += ((Record) records.get(k)).serialize(pos, retval); // rec.length;
        }
        if (log.check(POILogger.DEBUG)) {
    ...
    public int serialize(int offset, byte[] data) {
        ...
        for (int k = 0; k < records.size(); k++) {
            ...
            // if (LittleEndian.getUShort(data2, 2) != record.getRecordSize() -
            // 4
            ...
            // LittleEndian.getUShort(data2, 2) - record.getRecordSize() - 4) +
            // " records.");
            // }
    ...
    public void setDimensions(int firstrow, short firstcol, int lastrow, short lastcol) {
        if (log.check(POILogger.DEBUG)) {
            ...
            log.log(POILogger.DEBUG, (new StringBuffer("firstrow")).append(firstrow).append("firstcol")
                    .append(firstcol).append("lastrow").append(lastrow).append("lastcol").append(lastcol).toString());
        }
     ...
     * can set this to 0 and watch performance go down the tubes but it will
     * work. After a value is set this is automatically advanced. Its also set
     * by the create method. So you probably shouldn't mess with this unless you
     * have a compelling reason why or the help for the method you're calling
     * says so. Check the other methods for whether they care about the loc
     * pointer. Many of the "modify" and "remove" methods re-initialize this to
     * "dimsloc" which is the location of the Dimensions Record and presumably
     * the start of the value section (at or around 19 dec).
     *
    ...
    // ColumnInfoRecord columnInfo = (ColumnInfoRecord) columnSizes.get(
    // findStartOfColumnOutlineGroup( idx ) );
    //
    // // Hide all the columns until the end of the group
    // columnInfo = writeHidden( columnInfo, idx, true );
    //
    // // Write collapse field
    // setColumn( (short) ( columnInfo.getLastColumn() + 1 ), null, null, null,
    // Boolean.TRUE);
    // }
    ...
    // return false;
    // if (getColInfo(endOfOutlineGroupIdx).getLastColumn() + 1 !=
    // getColInfo(endOfOutlineGroupIdx + 1).getFirstColumn())
    // return false;
    // else
    ...
    // endHidden = false;
    // }
    // else if (getColInfo(endOfOutlineGroupIdx).getLastColumn() + 1 !=
    // getColInfo(endOfOutlineGroupIdx + 1).getFirstColumn())
    // {
    // endLevel = 0;
    // endHidden = false;
    ...
    // startHidden = false;
    // }
    // else if (getColInfo(startOfOutlineGroupIdx).getFirstColumn() - 1 !=
    // getColInfo(startOfOutlineGroupIdx - 1).getLastColumn())
    // {
    // startLevel = 0;
    // startHidden = false;
    // }
    ...
    // while (idx != 0)
    // {
    // ColumnInfoRecord prevColumnInfo = (ColumnInfoRecord) columnSizes.get( idx
    // - 1 );
    ...
    // {
    // ColumnInfoRecord nextColumnInfo = (ColumnInfoRecord) columnSizes.get( idx
    // + 1 );
    ...
    // {
    // if (getRow(currentRow) == null || getRow(currentRow).getOutlineLevel() <
    // level)
    // {
    // break;
    // }
    ...
    // if (startOfOutlineGroupIdx - 1 < 0 || getRow(startOfOutlineGroupIdx - 1)
    // == null)
    // {
    // startLevel = 0;
    // startHidden = false;
