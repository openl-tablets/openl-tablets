...
public class HSSFRow implements Comparable {
    ...
    protected HSSFRow(Workbook book, Sheet sheet, int rowNum) {
        ...
        this.sheet = sheet;
        row = new RowRecord();
        row.setOptionFlags((short) 0x100); // seems necessary for outlining to
                                            // work.
    ...
    /**
     * Creates an HSSFRow from a low level RowRecord object. Only HSSFSheet
     * should do this. HSSFSheet uses this when an existing file is read in.
     *
     * @param book low-level Workbook object containing the sheet that contains
     *            this row
     * @param sheet low-level Sheet object that contains this Row
     ...
     *
     * @return HSSFCell a high level representation of the created cell.
     * @deprecated As of 22-Jan-2002 use createCell(short) and use setCellValue
     *             to specify the type lazily.
     */
    ...
    /**
     * used internally to refresh the "first cell" when the first cell is
     * removed.
     */
    ...
    /**
     * get the hssfcell representing a given column (logical cell) 0-based. If
     * you ask for a cell that is not defined....you get a null.
     *
    ...
    public HSSFCell getCell(short cellnum) {
        ...
        /*
         * for (int k = 0; k < cells.size(); k++) { HSSFCell cell = ( HSSFCell )
         * cells.get(k);
         *
         * if (cell.getCellNum() == cellnum) { return cell; } }
         */
    ...
    /**
     * get the row's height or ff (-1) for undefined/default-height in twips
     * (1/20th of a point)
     *
    ...
    /**
     * get the row's height or ff (-1) for undefined/default-height in points
     * (20*getHeight())
     *
     * @return rowheight or 0xff for undefined (use sheet default)
     */
     ...
     * row!). That is to say if only columns 0,4,5 have values then there would
     * be 3.
     *
    ...
    public int getPhysicalNumberOfCells() {
        if (cells == null) {
            return 0; // shouldn't be possible but it is due to missing API
                        // support for BLANK/MULBLANK
        }
        return cells.size();
    }
    ...
    /**
     * get the lowlevel RowRecord represented by this object - should only be
     * called by other parts of the high level API
     *
    ...
    public void setRowNum(int rowNum) {
        this.rowNum = rowNum;
        if (row != null) {
            row.setRowNumber(rowNum); // used only for KEY comparison
                                        // (HSSFRow)
        }
    }
}
