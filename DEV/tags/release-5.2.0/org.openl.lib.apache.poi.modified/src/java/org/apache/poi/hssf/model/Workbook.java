...
public class Workbook implements Model {
    ...
    private short maxformatid = -1; // holds the max format id
                                                    private boolean uses1904datewindowing = false; // whether 1904 date
    ...
    public static Workbook createWorkbook() {
        ...
        for (int i = 0; i <= 7; i++) {
            ...
            retval.maxformatid = retval.maxformatid >= ((FormatRecord) rec).getIndexCode() ? retval.maxformatid
    ...
    public static Workbook createWorkbook(List recs) {
        if (log.check(POILogger.DEBUG)) {
            log.log(DEBUG, "Workbook (readfile) created with reclen=", new Integer(recs.size()));
        }
        Workbook retval = new Workbook();
    ...
    public int addSSTString(String string, boolean use16bits) {
        if (log.check(POILogger.DEBUG)) {
            log.log(DEBUG, "insert to sst string='", string, "' and use16bits= ", new Boolean(use16bits));
        }
        if (sst == null) {
            insertSST();
        }
    ...
    protected Record createDSF() {
        ...
        retval.setDsf((short) 0); // we don't even support double stream files
        return retval;
    }
     ...
     *            position in a file as MS Excel would create it.)
     *
     ...
     *            in a file as M$ Excel would create it.)
    ...
    public short createFormat(String format) {
        ...
        maxformatid = maxformatid >= (short) 0xa4 ? (short) (maxformatid + 1) : (short) 0xa4; // Starting
                                                                                                // value
                                                                                                // from
                                                                                                // M$
                                                                                                // empiracle
    ...
    protected Record createProtect() {
        ...
        retval.setProtect(false); // by default even when we support it we
                                    // won't
    ...
    /**
     * creates the TabId record containing an array of 0,1,2. This release of
     * HSSF always has the default three sheets, no less, no more.
     *
    ...
    public FontRecord getFontRecordAt(int idx) {
        ...
        if (index > (numfonts - 1)) {
            throw new ArrayIndexOutOfBoundsException("There are only " + numfonts + " font records, you asked for "
                    + idx);
        }
    ...
    public String getSheetName(int sheetnum) {
        return ((BoundSheetRecord) boundsheets.get(sheetnum)).getSheetname();
    }
    ...
    public String getSSTString(int str) {
        ...
        if (log.check(POILogger.DEBUG)) {
            log.log(DEBUG, "Returning SST for index=", new Integer(str), " String= ", retval);
        }
        return retval;
    }
     ...
     *
     * @param sheetnum the number of the sheet to set the positing of the bof
     *            for
    ...
    public void setSheetBof(int sheetnum, int pos) {
        if (log.check(POILogger.DEBUG)) {
            log.log(DEBUG, "setting bof for sheetnum =", new Integer(sheetnum), " at pos=", new Integer(pos));
        }
        checkSheets(sheetnum);
        ((BoundSheetRecord) boundsheets.get(sheetnum)).setPositionOfBof(pos);
    }

    /**
     * sets the name for a given sheet. If the boundsheet record doesn't exist
     * and its only one more than we have, go ahead and create it. If its > 1
     * more than we have, except
     *
     * @param sheetnum the sheet number (0 based)
