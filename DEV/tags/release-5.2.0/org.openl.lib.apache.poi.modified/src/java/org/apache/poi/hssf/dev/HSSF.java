 ...
 *
 * THIS IS NOT THE MAIN HSSF FILE!! This is a util for testing functionality. It
 * does contain sample API usage that may be educational to regular API users.
 *
 * @see #main
...
public class HSSF {
    ...
    public static void main(String[] args) {
        if (args.length < 2) {
            ...
            /*
             * try { HSSF hssf = new HSSF(args[ 0 ]);
             *
             * System.out.println("Data dump:\n"); HSSFWorkbook wb =
             * hssf.hssfworkbook;
             *
             * for (int k = 0; k < wb.getNumberOfSheets(); k++) {
             * System.out.println("Sheet " + k); HSSFSheet sheet =
             * wb.getSheetAt(k); int rows = sheet.getPhysicalNumberOfRows();
             *
             * for (int r = 0; r < rows; r++) { HSSFRow row =
             * sheet.getPhysicalRowAt(r); int cells =
             * row.getPhysicalNumberOfCells();
             *
             * System.out.println("ROW " + row.getRowNum()); for (int c = 0; c <
             * cells; c++) { HSSFCell cell = row.getPhysicalCellAt(c); String
             * value = null;
             *
             * switch (cell.getCellType()) {
             *
             * case HSSFCell.CELL_TYPE_FORMULA : value = "FORMULA "; break;
             *
             * case HSSFCell.CELL_TYPE_NUMERIC : value = "NUMERIC value=" +
             * cell.getNumericCellValue(); break;
             *
             * case HSSFCell.CELL_TYPE_STRING : value = "STRING value=" +
             * cell.getStringCellValue(); break;
             *
             * default : } System.out.println("CELL col=" + cell.getCellNum() + "
            ...
            try // delete row 0-24, row 74 - 99 && change cell 3 on row 39 to
    ...
    /**
     * Constructor HSSF - creates an HSSFStream from an InputStream. The
     * HSSFStream reads in the records allowing modification.
     *
     *
     * @param filename
     *
    ...
    /**
     * Constructor HSSF - takes in file - attempts to read it then reconstruct
     * it
     *
     *
     * @param infile
