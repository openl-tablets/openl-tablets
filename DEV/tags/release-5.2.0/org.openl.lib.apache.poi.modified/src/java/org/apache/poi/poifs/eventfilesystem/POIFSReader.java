...
/**
 * An event-driven reader for POIFS file systems. Users of this class first
 * create an instance of it, then use the registerListener methods to register
 * POIFSReaderListener instances for specific documents. Once all the listeners
 * have been registered, the read() method is called, which results in the
 * listeners being notified as their documents are read.
 *
...
public class POIFSReader {
    ...
    private void processProperties(final BlockList small_blocks, final BlockList big_blocks, final Iterator properties,
            final POIFSDocumentPath path) throws IOException {
            ...
            if (property.isDirectory()) {
                ...
                if (listeners.hasNext()) {
                    ...
                    if (property.shouldUseSmallBlocks()) {
                        document = new POIFSDocument(name, small_blocks.fetchBlocks(startBlock), size);
                    } else {
                        document = new POIFSDocument(name, big_blocks.fetchBlocks(startBlock), size);
                    }
                    while (listeners.hasNext()) {
                        POIFSReaderListener listener = (POIFSReaderListener) listeners.next();

                        listener.processPOIFSReaderEvent(new POIFSReaderEvent(new DocumentInputStream(document), path,
                                name));
                    }
                } else {
    ...
    public void read(final InputStream stream) throws IOException {
        ...
        new BlockAllocationTableReader(header_block_reader.getBATCount(), header_block_reader.getBATArray(),
                header_block_reader.getXBATCount(), header_block_reader.getXBATIndex(), data_blocks);

        // get property table from the document
        PropertyTable properties = new PropertyTable(header_block_reader.getPropertyStart(), data_blocks);

        // process documents
        processProperties(SmallBlockTableReader.getSmallDocumentBlocks(data_blocks, properties.getRoot(),
                header_block_reader.getSBATStart()), data_blocks, properties.getRoot().getChildren(),
     ...
     * @exception IllegalStateException if read() has already been called
     */
     ...
     *
     * @param listener the listener to be registered
     * @param path the document path; if null, the root directory is assumed
     * @param name the document name
     *
     * @exception NullPointerException if listener is null or name is null or
     *                empty
     * @exception IllegalStateException if read() has already been called
     */
