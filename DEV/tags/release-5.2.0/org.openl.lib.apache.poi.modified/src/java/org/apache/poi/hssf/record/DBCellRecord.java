...
/**
 * Title: DBCell Record (Currently read only. Not required.) Description: Used
 * to find rows in blocks...TODO
 * <P>
 * REFERENCE: PG 299/440 Microsoft Excel 97 Developer's Kit (ISBN:
 * 1-57231-498-2)
 * <P>
 *
...
public class DBCellRecord extends Record {
    ...
    public void addCellOffset(short offset) {
        if (field_2_cell_offsets == null) {
            ...
            System.arraycopy(field_2_cell_offsets, 0, temp, 0, field_2_cell_offsets.length);
    ...
    protected void fillFields(byte[] data, short size, int offset) {
        ...
        for (int k = 4; k < data.length; k += 2) {
            field_2_cell_offsets[element++] = LittleEndian.getShort(data, k + offset);
        }
    }
    ...
    /**
     * gets offset from the start of this DBCellRecord to the start of the first
     * cell in the next DBCell block.
     *
    ...
    public int serialize(int offset, byte[] data) {
        ...
        LittleEndian.putShort(data, 2 + offset, ((short) (4 + (getNumCellOffsets() * 2))));
    ...
    /**
     * sets offset from the start of this DBCellRecord to the start of the first
     * cell in the next DBCell block.
     *
     * @param offset offset to the start of the first cell in the next DBCell
     *            block
     */
    ...
    public String toString() {
        ...
        for (int k = 0; k < getNumCellOffsets(); k++) {
            buffer.append("    .cell_" + k + "          = ").append(Integer.toHexString(getCellOffsetAt(k))).append(
                    "\n");
        }
        buffer.append("[/DBCELL]\n");
