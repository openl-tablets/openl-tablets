...
public class ContinueRecord extends Record {
    ...
    /**
     * Use to serialize records that are too big for their britches (>8228..why
     * 8228 and not 8192 aka 8k? Those folks in washington don't ususally make
     * sense... or at least to anyone outside fo marketing...
     *
     * @deprecated handle this within the record...this didn't actualyl work out
     */

    @Deprecated
    public static byte[] processContinue(byte[] data) { // could do this
                                                        // recursively but that
                                                        // seems hard to debug

        // how many continue records do we need
        // System.out.println("In ProcessContinue");
        int records = (data.length / 8214); // we've a 1 offset but we're also
                                            // off by one due to rounding...so
                                            // it balances out
        int offset = 8214;

        // System.out.println("we have "+records+" continue records to
        // process");
    ...
    protected void fillFields(byte[] ignored_parm1, short ignored_parm2) {
        field_1_data = ignored_parm1;
        // throw new RecordFormatException("Are you crazy? Don't fill a continue
        // record");
        // do nothing
    }
    ...
    public int serialize(int offset, byte[] data) {
        ...
        // "You're not supposed to serialize Continue records like this
        // directly");
    }
