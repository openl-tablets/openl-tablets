 ...
 * expression is : <expression> ::= <term> [<addop> <term>]* <term> ::=
 * <factor> [ <mulop> <factor> ]* <factor> ::= <number> | (<expression>) |
 * <cellRef> | <function> <function> ::= <functionName> ([expression [,
 * expression]*])
 *
...
public class FormulaParser {
    ...
    /**
     * Using an unsynchronized linkedlist to implement a stack since we're not
     * multi-threaded.
     */
    ...
    /**
     * Static method to convert an array of Ptgs in RPN order to a human
     * readable string format in infix mode.
     *
    ...
    /**
     * create the parser with the string that is to be parsed later call the
     * parse() method to return ptg list in rpn order then call the getRPNPtg()
     * to retrive the parse results This class is recommended only for single
     * threaded use
     */
    ...
    private void addArgumentPointer() {
        if (functionTokens.size() > 0) {
            // no bounds check because this method should not be called unless a
            // token array is setup by function()
            List arguments = (List) functionTokens.get(0);
    ...
    /**
     * Create a tree representation of the RPN token array used to run the
     * class(RVA) change algo
     */
    ...
    private void Expression() {
        ...
        /*
         * This isn't quite right since it would allow multiple comparison
         * operators.
         */
    ...
    private void Factor() {
        if (look == '-') {
            ...
            if (look == '.') {
                ...
                if (IsDigit(look)) {
                    number = number + "." + GetNum(); // this also takes care
                }
                                                        // of someone entering
                                                        // "1234."
                tokens.add(new NumberPtg(number));
            } else {
                tokens.add(new IntPtg(number)); // TODO:what if the number is
                                                // too big to be a short? ..add
                                                // factory to return Int or
                                                // Number!
            }
        }
    }
    ...
    private AbstractFunctionPtg getFunction(String name, byte numArgs) {
        ...
        if (name.equals("IF")) {
            ...
            // simulated pop, no bounds checking because this list better be
            // populated by function()
            List argumentPointers = (List) functionTokens.get(0);
            ...
            // Biffview of an IF formula record indicates the attr ptg goes
            // after the condition ptgs and are
            // tracked in the argument pointers
            // The beginning first argument pointer is the last ptg of the
            // condition
            int ifIndex = tokens.indexOf(argumentPointers.get(0)) + 1;
            tokens.add(ifIndex, ifPtg);

            // we now need a goto ptgAttr to skip to the end of the formula
            // after a true condition
            // the true condition is should be inserted after the last ptg in
            // the first argument
            ...
            if (numArgs > 2) { // only add false jump if there is a false
                                // condition
                ...
                tokens.add(goto2Ptg); // this goes after all the arguments are
                                        // defined
            }

            // data portion of the if ptg points to the false subexpression
            // (Page 472 of MS Excel Developer's kit)
    ...
    /**
     * Get an Identifier AS IS, without stripping white spaces or converting to
     * uppercase; used for literals
     */
    ...
    /***************************************************************************
     * PARSER IMPLEMENTATION ENDS HERE EXCEL SPECIFIC METHODS BELOW
     **************************************************************************/
    ...
    private void StringLiteral() {
        ...
        if (look != '"') {
            ...
            for (;;) {
                if (look == '"') {
                    GetChar();
                    SkipWhite(); // potential white space here since it
                                    // doesnt matter up to the operator
                    if (look == '"') {
                        Token.append("\"");
                    } else {
                        break;
