...
public class TextPieceTable {
    ...
    public TextPieceTable(byte[] documentStream, byte[] tableStream, int offset, int size, int fcMin)
        ...
        for (int x = 0; x < pieces.length; x++) {
            ...
            // multiple will be 2 if there is only one piece and its unicode.
            // Some
            ...
            _textPieces.add(new TextPiece(startFilePosition, startFilePosition + textSize, buf, pieces[x], node
                    .getStart()));
        }
    }
    ...
    public byte[] writeTo(HWPFOutputStream docStream) throws IOException {
        ...
        for (int x = 0; x < size; x++) {
            ...
            textPlex.addProperty(new GenericPropertyNode(nodeStart - bumpDown,
                    ((next.getEnd() - nodeStart) / multiple + nodeStart) - bumpDown, pd.toByteArray()));
