...
public class TableRow extends Paragraph {
    ...
    public TableRow(int startIdx, int endIdx, Table parent, int levelNum) {
        ...
        for (int cellIndex = 0; cellIndex < _cells.length; cellIndex++) {
            ...
            while (!(s.length() > 0 && (s.charAt(s.length() - 1) == TABLE_CELL_MARK) || p.isEmbeddedCellMark()
                    && p.getTableLevel() == levelNum)) {
                end++;
                if (end >= numParagraphs()) {
                    break;
                }
                p = getParagraph(end);
                s = p.text();
            }
            _cells[cellIndex] = new TableCell(start, end, this, levelNum, _tprops.getRgtc()[cellIndex], _tprops
                    .getRgdxaCenter()[cellIndex], _tprops.getRgdxaCenter()[cellIndex + 1]
