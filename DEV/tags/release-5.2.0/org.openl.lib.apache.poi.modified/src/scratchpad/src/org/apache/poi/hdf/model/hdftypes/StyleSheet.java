...
public class StyleSheet implements HDFType {
     ...
     */
    static void doCHPOperation(CharacterProperties oldCHP, CharacterProperties newCHP, int operand, int param,
            byte[] varParam, byte[] grpprl, int offset, StyleSheet styleSheet) {
        switch (operand) {
            case 0:
     ...
     */
    static void doPAPOperation(ParagraphProperties newPAP, int operand, int param, byte[] varParam, byte[] grpprl,
            int offset, int spra) {
        switch (operand) {
            case 0:
            ...
            case 0x1b:
                ...
                /*
                 * byte pcVert = (param & 0x0c) >> 2; byte pcHorz = param &
                 * 0x03; if(pcVert != 3) { newPAP._pcVert = pcVert; } if(pcHorz !=
                 * 3) { newPAP._pcHorz = pcHorz; }
                 */
                break;
            case 0x1c:
    ...
    /**
     * Converts an byte value into a boolean. The byte parameter can be 1,0,
     * 128, or 129. if it is 128, this function returns the same value as
     * oldVal. If it is 129, this function returns !oldVal. This is used for
     * certain sprms
     *
    ...
    public static Object uncompressProperty(byte[] grpprl, Object parent, StyleSheet styleSheet, boolean doIstd) {
        ...
        while (offset < grpprl.length) {
            ...
            switch (propertyType) {
                case PAP_TYPE:
                    if (type == 1)// papx stores TAP sprms along with PAP
                                    // sprms
                    {
                        doPAPOperation((ParagraphProperties) newProperty, operand, param, varParam, grpprl, offset,
                                spra);
                    }
                    break;
                case CHP_TYPE:

                    doCHPOperation((CharacterProperties) parent, (CharacterProperties) newProperty, operand, param,
                            varParam, grpprl, offset, styleSheet);
                    break;
                case SEP_TYPE:
