...
public class CharacterSprmUncompressor {
     ...
     * @param operand The operand defined by the sprm (See Word file format
     *            spec)
     * @param param The parameter defined by the sprm (See Word file format
     *            spec)
    ...
    static void unCompressCHPOperation(CharacterProperties oldCHP, CharacterProperties newCHP, SprmOperation sprm) {
        ...
        switch (sprm.getOperation()) {
            ...
            case 0x47:
                 ...
                 * newCHP.isFBold ()) { newCHP.setFBold (styleCHP.isFBold ()); }
                 * if (genCHP.isFItalic () == newCHP.isFItalic ()) {
                 * newCHP.setFItalic (styleCHP.isFItalic ()); } if
                 * (genCHP.isFSmallCaps () == newCHP.isFSmallCaps ()) {
                 * newCHP.setFSmallCaps (styleCHP.isFSmallCaps ()); } if
                 * (genCHP.isFVanish () == newCHP.isFVanish ()) {
                 * newCHP.setFVanish (styleCHP.isFVanish ()); } if
                 * (genCHP.isFStrike () == newCHP.isFStrike ()) {
                 * newCHP.setFStrike (styleCHP.isFStrike ()); } if
                 * (genCHP.isFCaps () == newCHP.isFCaps ()) { newCHP.setFCaps
                 * (styleCHP.isFCaps ()); } if (genCHP.getFtcAscii () ==
                 * newCHP.getFtcAscii ()) { newCHP.setFtcAscii
                 * (styleCHP.getFtcAscii ()); } if (genCHP.getFtcFE () ==
                 * newCHP.getFtcFE ()) { newCHP.setFtcFE (styleCHP.getFtcFE ()); }
                 * if (genCHP.getFtcOther () == newCHP.getFtcOther ()) {
                 * newCHP.setFtcOther (styleCHP.getFtcOther ()); } if
                 * (genCHP.getHps () == newCHP.getHps ()) { newCHP.setHps
                 * (styleCHP.getHps ()); } if (genCHP.getHpsPos () ==
                 * newCHP.getHpsPos ()) { newCHP.setHpsPos (styleCHP.getHpsPos
                 * ()); } if (genCHP.getKul () == newCHP.getKul ()) {
                 * newCHP.setKul (styleCHP.getKul ()); } if (genCHP.getDxaSpace () ==
                 * newCHP.getDxaSpace ()) { newCHP.setDxaSpace
                 * (styleCHP.getDxaSpace ()); } if (genCHP.getIco () ==
                 * newCHP.getIco ()) { newCHP.setIco (styleCHP.getIco ()); } if
                 * (genCHP.getLidDefault () == newCHP.getLidDefault ()) {
                 * newCHP.setLidDefault (styleCHP.getLidDefault ()); } if
                 * (genCHP.getLidFE () == newCHP.getLidFE ()) { newCHP.setLidFE
                 * (styleCHP.getLidFE ()); }
                 */
                break;
            case 0x48:
            ...
            case 0x4b:
                ...
                break;
            case 0x4c:
                // unCompressCHPOperation (oldCHP, newCHP, 0x47, param,
                // varParam,
                // styleSheet, opSize);
                break;
            case 0x4d:
