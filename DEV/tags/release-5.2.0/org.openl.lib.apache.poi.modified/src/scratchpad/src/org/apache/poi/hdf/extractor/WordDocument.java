...
public class WordDocument {
    ...
    public static void main(String args[]) {
        /*
         * try { WordDocument file = new WordDocument(args[0], "r"); Writer out =
         * new BufferedWriter(new FileWriter(args[1])); file.writeAllText(out);
         * out.flush(); out.close(); } catch(Throwable t) { t.printStackTrace(); }
         */
        try {
            _outName = args[1];
    ...
    public WordDocument(InputStream inputStream) throws IOException {
        ...
        for (int x = 0; x < size; x++) {
            ...
            writeSection(Math.max(_fcMin, start), Math.min(_fcMin + _ccpText, end), sep, _text, _paragraphTable,
                    _characterTable, _styleSheet);
        }
        // finish
        istream.close();
    ...
    private void addBlockContent(int start, int end, BTreeSet text, BTreeSet paragraphTable, BTreeSet characterTable) {
    ...
    private void addCharacterProperties(CHP chp, StringBuffer buf) {
        ...
        // buf.append("letter-spacing=\"" + ((double)chp._dxaSpace)/1440.0f +
        // "in\" ");
    ...
    private void addListParagraphContent(LVL lvl, StringBuffer blockBuffer, PAP pap, PapxNode currentNode, int start,
            int end, StyleDescription std) {
        pap = (PAP) StyleSheet.uncompressProperty(lvl._papx, pap, _styleSheet, false);

        addParagraphProperties(pap, blockBuffer);

        ArrayList charRuns = findProperties(Math.max(currentNode.getStart(), start), Math
                .min(currentNode.getEnd(), end), _characterTable.root);
        ...
        indent = indent - (bulletText.length() * 10) * 20;// (metrics.stringWidth(bulletText)
                                                            // * 20);
    ...
    private void addParagraphContent(StringBuffer blockBuffer, PAP pap, PapxNode currentNode, int start, int end,
    ...
    private void addParagraphProperties(PAP pap, StringBuffer buf) {
        ...
        if (pap._lspd[1] == 0) {
            // buf.append("line-height=\"" + ((float)pap._lspd[0])/1440.0f +
            // "in\"\r\n");
        }
    ...
    private String createEvenOddPageSequence(String titlePage, String evenPage, String oddPage, int counter) {
        ...
        if (titlePage != null) {
            _headerBuffer.append("<fo:conditional-page-master-reference "
                    + "page-position=\"first\" master-reference=\"" + titlePage + "\"/>");
        }
    ...
    private String createPageMaster(SEP sep, String type, int section, String regionBefore, String regionAfter) {
        float height = (sep._yaPage) / 1440.0f;
        float width = (sep._xaPage) / 1440.0f;
        float leftMargin = (sep._dxaLeft) / 1440.0f;
        float rightMargin = (sep._dxaRight) / 1440.0f;
        float topMargin = (sep._dyaTop) / 1440.0f;
        float bottomMargin = (sep._dyaBottom) / 1440.0f;

        // add these to the header
        String thisPage = type + "-page" + section;

        _headerBuffer.append("<fo:simple-page-master master-name=\"" + thisPage + "\"\r\n");
        ...
        _headerBuffer.append("margin=\"" + topMargin + "in " + rightMargin + "in " + bottomMargin + "in " + leftMargin
                + "in\"\r\n");
        ...
        if (sep._ccolM1 > 0) {
            ...
            if (sep._fEvenlySpaced) {
                _headerBuffer.append("column-gap=\"" + ((sep._dxaColumns)) / 1440.0f + "in\"");
            } else {
    ...
    private void createParagraph(int start, int end, PapxNode currentNode, BTreeSet characterTable, BTreeSet text) {
     ...
     * String fontName = _fonts.getFont(chp._ftcAscii); int style = 0;
     * if(chp._bold) { style |= Font.BOLD; } if(chp._italic) { style |=
     * Font.ITALIC; }
     *
    ...
    private String createRegion(boolean before, HeaderFooter header, SEP sep, String name) {
        ...
        return "<fo:region-" + where + " display-align=\"" + align + "\" extent=\"" + extent + "pt\" padding-left=\""
                + marginLeft + "pt\" padding-right=\"" + marginRight + "pt\" padding-top=\"" + marginTop
                + "pt\" padding-bottom=\"" + marginBottom + "pt\" " + region + "/>";
     ...
     */
    private void findFormatting(byte[] tableStream, int charOffset, int charPlcSize, int parOffset, int parPlcSize)
            throws IOException {
        openDoc();
    ...
    private void overrideCellBorder(int row, int col, int height, int width, TC tc, TAP tap) {
        ...
        if (_table != null) {
            ...
            for (int x = 0; x < size; x++) {
                ...
                for (int y = 0; y < tap._itcMac; y++) {
                    ...
                    rowBuffer.append("width=\"" + ((tap._rgdxaCenter[y + 1] - tap._rgdxaCenter[y])) / 1440.0f
                            + "in\" ");
    ...
    /**
     * Extracts the correct Table stream from the POI filesystem then hands off
     * to other functions to process text and formatting info. the name is based
     * on the fact that in Word 8(97) all text (not character or paragraph
     * formatting) is stored in complex format.
     *
     ...
     * @throws IOException
     */
    private void processComplexFile(boolean useTable1, int charTable, int charPlcSize, int parTable, int parPlcSize)
            throws IOException {
    ...
    /**
     * Extracts the main document stream from the POI file then hands off to
     * other functions that parse other areas.
     *
     * @throws IOException
     */
    ...
    public void writeSection(int start, int end, SEP sep, BTreeSet text, BTreeSet paragraphTable,
            BTreeSet characterTable, StyleSheet stylesheet) {
        ...
        /*
         * if(sep._fTitlePage) { titlePage = createPageMaster(sep, "first",
         * _sectionCounter, createRegion("before", "title-header"),
         * createRegion("after", "title-footer"));
         *
         * if(!titleHeader.isEmpty()) { addStaticContent("title-header" +
         * _sectionCounter, titleHeader); } if(!titleFooter.isEmpty()) {
        ...
        if (_docProps._fFacingPages) {
            ...
            /*
             * if(sep._fTitlePage) { String before = createRegion(true,
             * titleHeader, sep); String after = createRegion(false,
             * titleFooter, sep); titlePage = createPageMaster(sep, "first",
             * _sectionCounter, before, after); }
             */
