...
{
	static public String makeSourceLocationURL(
		ILocation location,
...
//			position = location.getStart().getAbsolutePosition(info);
			...
			//      columnFrom = location.getStart().getColumn(info, module.getTabSize());
			//	    lineTo = location.getEnd().getLine(info);
			//	    columnTo = location.getEnd().getColumn(info, module.getTabSize());
...
//		stream.println("    at " + url);
	...
	static public Map parseUrl(String urls)
	{
	...
	static void parseQuery(String query, Map map)
	{
		if (query == null)
			return;
