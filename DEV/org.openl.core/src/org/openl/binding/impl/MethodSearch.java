...
public class MethodSearch
{
	...
	/* (non-Javadoc)
	 * @see org.openl.binding.IMethodFactory#getMethod(java.lang.String, org.openl.types.IOpenClass[], org.openl.binding.ICastFactory)
	 */
	static public IMethodCaller getMethodCaller(
		String name,
		IOpenClass[] params,
		ICastFactory casts,
	...
	static protected IMethodCaller getCastingMethodCaller 
	(
		String name,
		IOpenClass[] params,
		ICastFactory casts,
	...
	throws AmbiguousMethodException
	{
		...
		for (Iterator iter = methods(name, params.length, factory); iter.hasNext();)
		{
		...
		switch(matchingMethods.size())
		{
			case 0:
				return null;
			case 1:
				return  new CastingMethodCaller( (IOpenMethod)matchingMethods.elementAt(0), bestCastHolder);
			default:
	...
	protected static Iterator methods(String name, int nParams, IMethodFactory factory)
	{
		Iterator it = factory.methods();
		return it == null ? OpenIterator.EMPTY : 
		OpenIterator.select(it, new NameAndParSelector(name, nParams));
		
	}

	static class NameAndParSelector extends ASelector
	{
		String name; 
		int nParams;
    ...
    {
    	IOpenMethod method = (IOpenMethod)obj;
      return method.getName().equals(name) && method.getSignature().getParameterTypes().length == nParams;
    }

        /* (non-Javadoc)
         * @see org.openl.util.ASelector#equalsSelector(org.openl.util.ASelector)
         */
