		...
		try
		{
			...
			for (;; pos++)
			{
				...
				if (c == EOF || isDelim(c))
				{
					if (buf != null)
					{
						IdentifierNode node = new IdentifierNode("token", new TextInterval(new AbsolutePosition(startToken), new AbsolutePosition(pos))  ,  buf.toString(),src );
						nodes.add(node);
						buf = null;
					}
					if (c == EOF)
			...
			return (IdentifierNode[]) nodes.toArray(new IdentifierNode[nodes.size()]);
