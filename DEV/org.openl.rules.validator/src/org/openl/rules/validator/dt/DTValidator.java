...
public class DTValidator implements IDTValidator
{
	...
	public static DTValidationResult validateDT(String dtname, Map domains, IOpenClass type)
	{
	...
	{
		return validateDT((IDTValidatedObject) ivo, openl);
	}
	...
	static class ValidationAlgorithm
	{
		...
		public DTValidationResult validateDT()
		{
			...
			if (sel != null)
				cc = (IDTCondition[]) OpenIterator.fromArray(cc).select(sel).asList()
						.toArray(new IDTCondition[0]);
			...
			Vector completeness = dtc.checkCompleteness();
			Vector overlappings = dtc.checkOverlappings();
			...
			return new DTValidationResult(dtvo.getDT(), 
					(Overlapping[])overlappings.toArray(new Overlapping[0]), 
					(Uncovered[])completeness.toArray(new Uncovered[0]),
					dtvo.getTransformer()
					);
		}
		...
		private IntBoolExp makeExpression(int rule, int cnum)
		{
			...
			for (int i = 0; i < nargs; i++)
			{
				...
				else
				{
					...
					args[i] = transformValue(name, cc[cnum], params[rule][i-ndtArgs]);
				}	
		 ...
		 * @return
		 */
		private Object transformValue(String name, IDTCondition condition, Object value)
		{
			return dtvo.getTransformer().transformParameterValue(name, condition, value, C);
		}
		...
		private IntExpArray makeVars()
		{
			...
			for (int i = 0; i < nargs; i++)
			{
				IntVar var = dtvo.getTransformer().makeSignatureVar(dtsign.getParameterName(i), dtsign.getParameterTypes()[i], C);
				if (var != null)
					v.add(var);
			}
				...
				IMethodSignature dtsignature)
		{
			...
			IParameterDeclaration[] pdd = new ParameterDeclaration[ptypes.length + pd.length];
			
			for (int i = 0; i < ptypes.length; i++)
			{
				IOpenClass newType = dtvo.getTransformer().transformSignatureType(ptypes[i], dtsignature.getParameterName(i), dtsignature.getParameterDirection(i) );
				if (newType == null) newType = ptypes[i];
				pdd[i] = new ParameterDeclaration(newType, dtsignature.getParameterName(i), dtsignature.getParameterDirection(i));
			}
			
			for (int i = 0; i < pd.length; i++)
			{
				IOpenClass newType = dtvo.getTransformer().transformParameterType(pd[i]);
				pdd[i + ptypes.length] = newType == null ? pd[i] : new ParameterDeclaration(newType, pd[i].getName(), pd[i].getDirection());
			}
