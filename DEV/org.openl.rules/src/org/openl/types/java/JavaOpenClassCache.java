...
public final class JavaOpenClassCache {
     ...
     */
    private final AtomicReference<Map<Class<?>, JavaOpenClass>> javaClassCache = new AtomicReference<>();
    /**
    ...
    private Map<Class<?>, JavaOpenClass> getJavaClassCache() {
        var classes = javaClassCache.get();
        if (classes == null) {
            synchronized (this) {
                classes = javaClassCache.get();
                if (classes == null) {
                    classes = initializeJavaClassCache();
                    javaClassCache.set(classes);
                }
            }
        }
        return classes;
