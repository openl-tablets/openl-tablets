...
abstract class BaseExport {
    ...
    protected static void setCellComment(Cell cell, String message) {
        var factory = cell.getSheet().getWorkbook().getCreationHelper();
        ...
        var comment = cell.getSheet().createDrawingPatriarch().createCellComment(anchor);
    ...
    protected Cell createCell(Row row, int cellNum, Object value, CellStyle style) {
        ...
        if (simpleValue != null) {
            if (simpleValue instanceof Date date) {
                ...
                var maxTextLength = SpreadsheetVersion.EXCEL2007.getMaxTextLength();
    ...
    protected Object getSimpleValue(Object value) {
        ...
        if (value instanceof ParameterWithValueDeclaration parameter) {
            var simpleValue = parameter.getValue();
