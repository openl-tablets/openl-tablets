...
public class DateRangeParser extends ARangeParser<Long> {
    ...
    private static final String VERBAL_PATTERN = "\\s*((?:\\d{1,2}/){2}\\d+(?: (?:\\d{1,2}:){2}\\d{1,2})?)\\s*(\\+|and\\s+more|or\\s+less)\\s*";
    private static final String MORE_LESS_PATTERN = "\\s*(<|>|>=|<=|less\\s+than|more\\s+than)\\s*((?:\\d{1,2}/){2}\\d+(?: (?:\\d{1,2}:){2}\\d{1,2})?)\\s*";
    ...
    private static final class DateRangeBoundAdapter implements RangeBoundAdapter<Long> {
        ...
        private static Long toInstant(TemporalAccessor t) {
