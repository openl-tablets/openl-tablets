...
public class MoreThanConstraint extends AbstractConstraint {
    ...
    private Object[] parseParams() {
        var mathes = getValue().replaceAll("\\s", "").split(CONSTRAINT_KEY);
        if (mathes.length > 1) {
    ...
    public boolean check(Object... valuesToCheck) {
        if (valuesToCheck.length > 1) {
            var value1 = valuesToCheck[0];
            var value2 = valuesToCheck[1];
            if (value1 instanceof Comparable && value2 instanceof Comparable) {
                var compareResult = ((Comparable<Object>) value1).compareTo(value2);
