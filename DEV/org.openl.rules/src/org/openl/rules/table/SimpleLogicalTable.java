 ...
 * interface.<br>
 * <b>Be careful, as this implementation doesn`t provide that following operations<br>
 * </b> {@link ITable#getColumn(int)}<br>
...
public class SimpleLogicalTable extends ALogicalTable {
    ...
    /**
     * This method consider that this table doesn`t have merged regions inside itself. In general it is not true.
     * Current Openl parsing implementation, correctly handles such cases. If you need the trully ILogicalTable, with
     * correctly calculated logical columns and rows, you may get the source table, by calling
     * {@link ILogicalTable#getSource()} extract the region you need by calling
     * {@link ITable#getSubtable(int, int, int, int)} and create {@link ILogicalTable} by calling
