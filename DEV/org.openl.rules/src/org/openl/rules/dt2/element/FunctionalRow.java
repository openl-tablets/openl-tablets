...
import org.openl.rules.dt2.storage.StorageBuilder;
...
public abstract class FunctionalRow implements IDecisionRow {
    ...
    private void prepareParamValues(CompositeMethod method, OpenlToolAdaptor ota, RuleRow ruleRow)
        throws Exception {
        ...
        StorageBuilder<?>[] builders = initializeStorage(len, paramDecl);
    ...
    private StorageBuilder<?>[] initializeStorage(int len,
			IParameterDeclaration[] paramDecl) {
    	
    	int nparams = paramDecl.length;
    	StorageBuilder<?>[] builders = new StorageBuilder[nparams];
    	for (int i = 0; i < builders.length; i++) {
	...
	private void loadParamsFromColumn(OpenlToolAdaptor ota, RuleRow ruleRow, IParameterDeclaration[] paramDecl,
            boolean[] paramIndexed, ArrayList<SyntaxNodeException> errors, int ruleN, StorageBuilder<?>[] builders) {        
