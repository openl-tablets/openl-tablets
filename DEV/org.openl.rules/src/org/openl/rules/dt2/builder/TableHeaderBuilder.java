package org.openl.rules.dt2.builder;
...
public class TableHeaderBuilder {
     ...
     * @param lastTableColumnNumber the number of the last column of the table. Is needed to merge the 
     * header region.
     ...
     * 
     */
    public void build(IWritableGrid sheet, int lastTableColumnNumber, int columnStartIndex, 
            int rowStartIndex) {
        sheet.setCellValue(columnStartIndex, rowStartIndex, tableHeader);
                        
        sheet.addMergedRegion(new GridRegion(rowStartIndex, columnStartIndex, 
            rowStartIndex, lastTableColumnNumber));
    }
}
