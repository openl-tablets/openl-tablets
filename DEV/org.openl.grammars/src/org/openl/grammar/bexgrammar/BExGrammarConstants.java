/* Generated By:JavaCC: Do not edit this line. BExGrammarConstants.java */
package org.openl.grammar.bexgrammar;


/**
 * Token literal values and constants.
 * Generated by org.javacc.parser.OtherFilesGen#start()
 */
public interface BExGrammarConstants {

  /** End of File. */
  int EOF = 0;
  /** RegularExpression Id. */
  int OR = 6;
  /** RegularExpression Id. */
  int AND = 7;
  /** RegularExpression Id. */
  int NOT = 8;
  /** RegularExpression Id. */
  int ABSTRACT = 9;
  /** RegularExpression Id. */
  int BREAK = 10;
  /** RegularExpression Id. */
  int CALCULATE = 11;
  /** RegularExpression Id. */
  int CASE = 12;
  /** RegularExpression Id. */
  int CATCH = 13;
  /** RegularExpression Id. */
  int CONST = 14;
  /** RegularExpression Id. */
  int CONTINUE = 15;
  /** RegularExpression Id. */
  int _DEFAULT = 16;
  /** RegularExpression Id. */
  int DO = 17;
  /** RegularExpression Id. */
  int ELSE = 18;
  /** RegularExpression Id. */
  int EXTENDS = 19;
  /** RegularExpression Id. */
  int FALSE = 20;
  /** RegularExpression Id. */
  int FINAL = 21;
  /** RegularExpression Id. */
  int FINALLY = 22;
  /** RegularExpression Id. */
  int FOR = 23;
  /** RegularExpression Id. */
  int GOTO = 24;
  /** RegularExpression Id. */
  int IF = 25;
  /** RegularExpression Id. */
  int PLUSSTR = 26;
  /** RegularExpression Id. */
  int IMPLEMENTS = 27;
  /** RegularExpression Id. */
  int IMPORT = 28;
  /** RegularExpression Id. */
  int INSTANCEOF = 29;
  /** RegularExpression Id. */
  int INTERFACE = 30;
  /** RegularExpression Id. */
  int NATIVE = 31;
  /** RegularExpression Id. */
  int NEW = 32;
  /** RegularExpression Id. */
  int NULL = 33;
  /** RegularExpression Id. */
  int PACKAGE = 34;
  /** RegularExpression Id. */
  int PRIVATE = 35;
  /** RegularExpression Id. */
  int PROTECTED = 36;
  /** RegularExpression Id. */
  int PUBLIC = 37;
  /** RegularExpression Id. */
  int RETURN = 38;
  /** RegularExpression Id. */
  int STATIC = 39;
  /** RegularExpression Id. */
  int SUPER = 40;
  /** RegularExpression Id. */
  int SWITCH = 41;
  /** RegularExpression Id. */
  int SYNCHRONIZED = 42;
  /** RegularExpression Id. */
  int THROW = 43;
  /** RegularExpression Id. */
  int THROWS = 44;
  /** RegularExpression Id. */
  int TRANSIENT = 45;
  /** RegularExpression Id. */
  int TRUE = 46;
  /** RegularExpression Id. */
  int TRY = 47;
  /** RegularExpression Id. */
  int VOID = 48;
  /** RegularExpression Id. */
  int VOLATILE = 49;
  /** RegularExpression Id. */
  int WHILE = 50;
  /** RegularExpression Id. */
  int OF = 51;
  /** RegularExpression Id. */
  int THE = 52;
  /** RegularExpression Id. */
  int WHERE = 53;
  /** RegularExpression Id. */
  int IS = 54;
  /** RegularExpression Id. */
  int LESS = 55;
  /** RegularExpression Id. */
  int THAN = 56;
  /** RegularExpression Id. */
  int LPAREN = 57;
  /** RegularExpression Id. */
  int RPAREN = 58;
  /** RegularExpression Id. */
  int LBRACE = 59;
  /** RegularExpression Id. */
  int RBRACE = 60;
  /** RegularExpression Id. */
  int LBRACKET = 61;
  /** RegularExpression Id. */
  int RBRACKET = 62;
  /** RegularExpression Id. */
  int SEMICOLON = 63;
  /** RegularExpression Id. */
  int COMMA = 64;
  /** RegularExpression Id. */
  int DOT = 65;
  /** RegularExpression Id. */
  int DDOT = 66;
  /** RegularExpression Id. */
  int TDOT = 67;
  /** RegularExpression Id. */
  int TDOT_UNICODE = 68;
  /** RegularExpression Id. */
  int ASSIGN = 69;
  /** RegularExpression Id. */
  int GT = 70;
  /** RegularExpression Id. */
  int STRICT_GT = 71;
  /** RegularExpression Id. */
  int LT = 72;
  /** RegularExpression Id. */
  int STRICT_LT = 73;
  /** RegularExpression Id. */
  int BANG = 74;
  /** RegularExpression Id. */
  int TILDE = 75;
  /** RegularExpression Id. */
  int HOOK = 76;
  /** RegularExpression Id. */
  int COLON = 77;
  /** RegularExpression Id. */
  int EQ = 78;
  /** RegularExpression Id. */
  int STRICT_EQ = 79;
  /** RegularExpression Id. */
  int LE = 80;
  /** RegularExpression Id. */
  int STRICT_LE = 81;
  /** RegularExpression Id. */
  int GE = 82;
  /** RegularExpression Id. */
  int STRICT_GE = 83;
  /** RegularExpression Id. */
  int NE = 84;
  /** RegularExpression Id. */
  int NE_ALT = 85;
  /** RegularExpression Id. */
  int STRICT_NE = 86;
  /** RegularExpression Id. */
  int BOOL_OR = 87;
  /** RegularExpression Id. */
  int BOOL_AND = 88;
  /** RegularExpression Id. */
  int INCR = 89;
  /** RegularExpression Id. */
  int DECR = 90;
  /** RegularExpression Id. */
  int PLUS = 91;
  /** RegularExpression Id. */
  int MINUS = 92;
  /** RegularExpression Id. */
  int STAR = 93;
  /** RegularExpression Id. */
  int SLASH = 94;
  /** RegularExpression Id. */
  int BIT_AND = 95;
  /** RegularExpression Id. */
  int BIT_OR = 96;
  /** RegularExpression Id. */
  int BIT_XOR = 97;
  /** RegularExpression Id. */
  int REM = 98;
  /** RegularExpression Id. */
  int LSHIFT = 99;
  /** RegularExpression Id. */
  int RSIGNEDSHIFT = 100;
  /** RegularExpression Id. */
  int RUNSIGNEDSHIFT = 101;
  /** RegularExpression Id. */
  int PLUSASSIGN = 102;
  /** RegularExpression Id. */
  int MINUSASSIGN = 103;
  /** RegularExpression Id. */
  int STARASSIGN = 104;
  /** RegularExpression Id. */
  int SLASHASSIGN = 105;
  /** RegularExpression Id. */
  int ANDASSIGN = 106;
  /** RegularExpression Id. */
  int ORASSIGN = 107;
  /** RegularExpression Id. */
  int XORASSIGN = 108;
  /** RegularExpression Id. */
  int REMASSIGN = 109;
  /** RegularExpression Id. */
  int LSHIFTASSIGN = 110;
  /** RegularExpression Id. */
  int RSIGNEDSHIFTASSIGN = 111;
  /** RegularExpression Id. */
  int RUNSIGNEDSHIFTASSIGN = 112;
  /** RegularExpression Id. */
  int EXP = 113;
  /** RegularExpression Id. */
  int IMPL = 114;
  /** RegularExpression Id. */
  int INTEGER_LITERAL = 115;
  /** RegularExpression Id. */
  int DECIMAL_LITERAL = 116;
  /** RegularExpression Id. */
  int HEX_LITERAL = 117;
  /** RegularExpression Id. */
  int OCTAL_LITERAL = 118;
  /** RegularExpression Id. */
  int FP_LITERAL1 = 119;
  /** RegularExpression Id. */
  int FP_LITERAL2 = 120;
  /** RegularExpression Id. */
  int FLOATING_POINT_LITERAL = 121;
  /** RegularExpression Id. */
  int BUSINESS_INTEGER_LITERAL = 122;
  /** RegularExpression Id. */
  int DATE_FORMAT_LITERAL_4_2_2 = 123;
  /** RegularExpression Id. */
  int TIME_FORMAT_LITERAL = 124;
  /** RegularExpression Id. */
  int PERCENT_LITERAL = 125;
  /** RegularExpression Id. */
  int EXPONENT = 126;
  /** RegularExpression Id. */
  int CHARACTER_LITERAL = 127;
  /** RegularExpression Id. */
  int STRING_LITERAL = 128;
  /** RegularExpression Id. */
  int IDENTIFIER = 129;
  /** RegularExpression Id. */
  int LETTER = 130;
  /** RegularExpression Id. */
  int DIGIT = 131;
  /** RegularExpression Id. */
  int RANGE_VARIABLE = 132;
  /** RegularExpression Id. */
  int SINGLE_LINE_COMMENT = 135;
  /** RegularExpression Id. */
  int FORMAL_COMMENT = 136;
  /** RegularExpression Id. */
  int MULTI_LINE_COMMENT = 137;

  /** Lexical state. */
  int DEFAULT = 0;
  /** Lexical state. */
  int IN_FORMAL_COMMENT = 1;
  /** Lexical state. */
  int IN_MULTI_LINE_COMMENT = 2;

  /** Literal token values. */
  String[] tokenImage = {
    "<EOF>",
    "\" \"",
    "\"\\r\"",
    "\"\\t\"",
    "\"\\n\"",
    "\"\\f\"",
    "\"or\"",
    "\"and\"",
    "\"not\"",
    "\"abstract\"",
    "\"break\"",
    "\"Calculate\"",
    "\"case\"",
    "\"catch\"",
    "\"const\"",
    "\"continue\"",
    "\"default\"",
    "\"do\"",
    "\"else\"",
    "\"extends\"",
    "\"false\"",
    "\"final\"",
    "\"finally\"",
    "\"for\"",
    "\"goto\"",
    "\"if\"",
    "\"plus\"",
    "\"implements\"",
    "\"import\"",
    "\"instanceof\"",
    "\"interface\"",
    "\"native\"",
    "\"new\"",
    "\"null\"",
    "\"package\"",
    "\"private\"",
    "\"protected\"",
    "\"public\"",
    "\"return\"",
    "\"static\"",
    "\"super\"",
    "\"switch\"",
    "\"synchronized\"",
    "\"throw\"",
    "\"throws\"",
    "\"transient\"",
    "\"true\"",
    "\"try\"",
    "\"void\"",
    "\"volatile\"",
    "\"while\"",
    "\"of\"",
    "\"the\"",
    "\"where\"",
    "\"is\"",
    "\"less\"",
    "\"than\"",
    "\"(\"",
    "\")\"",
    "\"{\"",
    "\"}\"",
    "\"[\"",
    "\"]\"",
    "\";\"",
    "\",\"",
    "\".\"",
    "\"..\"",
    "\"...\"",
    "\"\\u2026\"",
    "\"=\"",
    "\">\"",
    "\">==\"",
    "\"<\"",
    "\"<==\"",
    "\"!\"",
    "\"~\"",
    "\"?\"",
    "\":\"",
    "\"==\"",
    "\"====\"",
    "\"<=\"",
    "\"<===\"",
    "\">=\"",
    "\">===\"",
    "\"!=\"",
    "\"<>\"",
    "\"!===\"",
    "\"||\"",
    "\"&&\"",
    "\"++\"",
    "\"--\"",
    "\"+\"",
    "\"-\"",
    "\"*\"",
    "\"/\"",
    "\"&\"",
    "\"|\"",
    "\"^\"",
    "\"%\"",
    "\"<<\"",
    "\">>\"",
    "\">>>\"",
    "\"+=\"",
    "\"-=\"",
    "\"*=\"",
    "\"/=\"",
    "\"&=\"",
    "\"|=\"",
    "\"^=\"",
    "\"%=\"",
    "\"<<=\"",
    "\">>=\"",
    "\">>>=\"",
    "\"**\"",
    "\"->\"",
    "<INTEGER_LITERAL>",
    "<DECIMAL_LITERAL>",
    "<HEX_LITERAL>",
    "<OCTAL_LITERAL>",
    "<FP_LITERAL1>",
    "<FP_LITERAL2>",
    "<FLOATING_POINT_LITERAL>",
    "<BUSINESS_INTEGER_LITERAL>",
    "<DATE_FORMAT_LITERAL_4_2_2>",
    "<TIME_FORMAT_LITERAL>",
    "<PERCENT_LITERAL>",
    "<EXPONENT>",
    "<CHARACTER_LITERAL>",
    "<STRING_LITERAL>",
    "<IDENTIFIER>",
    "<LETTER>",
    "<DIGIT>",
    "<RANGE_VARIABLE>",
    "<token of kind 133>",
    "\"/*\"",
    "<SINGLE_LINE_COMMENT>",
    "\"*/\"",
    "\"*/\"",
    "<token of kind 138>",
    "\"equals to\"",
    "\"is same as\"",
    "\"does not equal to\"",
    "\"is different from\"",
    "\"is less than\"",
    "\"is more than\"",
    "\"is less or equal\"",
    "\"is no more than\"",
    "\"is in\"",
    "\"is more or equal\"",
    "\"is no less than\"",
    "\"@\"",
    "\"select all having\"",
    "\"select all where\"",
    "\"!@\"",
    "\"select first having\"",
    "\"select first where\"",
    "\"^@\"",
    "\"order by\"",
    "\"order increasing by\"",
    "\"v@\"",
    "\"order decreasing by\"",
    "\"~@\"",
    "\"s@\"",
    "\"split by\"",
    "\"*@\"",
    "\"transform to\"",
    "\"*!@\"",
    "\"transform unique to\"",
    "\"less than\"",
    "\"more than\"",
    "\"or less\"",
    "\"and more\"",
  };

}
