package com.exigen.ie.test.simplex;
/**
 * <p>Title: </p>
 * <p>Description: </p>
...
import com.exigen.ie.simplex.ArrayOut;
...
public class Blending {
  ...
  static public void main (String[] argv){
    try{
      ...
      LPX lp = new LPX();
      //LPX.loadLibrary(new File("").getAbsolutePath()+"\\", "glpDll.dll");
      lp.createLPX();
      ...
      for (int i =0 ; i<9;i++){
         ...
         lp.setColBnds(i,VarType.LPX_LO,0,0); //all variables are bounded below
      }
      // advertising values
      lp.addColumns(3);
      for (int i = 9 ; i<=11;i++){
      ...
      for (int i=0; i<gasTypes.length; i++){
        ...
        if (output_task) {
          System.out.println(
            lp.getRowName(rownum)+":" +
            lp.getRowBnds(rownum) + ", "+
    ...
    for (int i=0;i<oilTypes.length;i++){
      for (int j=0;j<gasTypes.length;j++){
        lp.setColCoef(i*gasTypes.length + j,
                      gas_prices[j] - oil_prices[i]);
      }
    }
