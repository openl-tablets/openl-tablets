try {
    ...
    assert new File(basedir, 'target/openl-gen-csrs-0.0.0.zip').exists()

    assert new File(basedir, 'target').list({File file, String name -> name.startsWith("openl-gen-csrs-0.0.0") && name.endsWith("-lib.jar")}).length == 1
