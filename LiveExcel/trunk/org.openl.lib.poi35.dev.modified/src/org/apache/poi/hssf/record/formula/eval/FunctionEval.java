...
public abstract class FunctionEval implements OperationEval {
    ...
    private static Function[] produceFunctions() {
        ...
        retval[ID.EXTERNAL_FUNC] = null; // ExternalFunction is a
                                         // FreeREfFunction
    ...
    public static class FOO implements Function {
        ...
        public Eval evaluate(Eval[] args, int srcRowIndex, short srcColumnIndex) {
            if (args.length != 3) {
                ...
                try{
                    ((RefEvalBase)args[0]).setValue((NumberEval)args[2]);
                    ((RefEvalBase)args[1]).recalculate();
                    return ((RefEvalBase)args[1]).getInnerValueEval();
                }catch (Exception e) {
                    return ErrorEval.VALUE_INVALID;
                }
            }
