...
class ExprTool {
    ...
    private static final IOpenMethod DIV_OP_METHOD = JavaOpenClass.getOpenClass(Operators.class).getMethod("divide", new IOpenClass[]{JavaOpenClass.DOUBLE, JavaOpenClass.DOUBLE})
    ...
    private static Double divide(Double a, Double b) {
        return (Double) DIV_OP_METHOD.invoke(null, new Object[]{a, b}, new SimpleRulesRuntimeEnv())
    }
    ...
    private static String cutZeroEnding(String s) {
        while (s.charAt(s.length() - 1) == '0' as char) {
            s = s.substring(0, s.length() - 1)
        }
        if (s.charAt(s.length() - 1) == '.' as char) {
            s = s.substring(0, s.length() - 1)
        }
        return s
    }
    ...
    private static String doubleToString(Double d) {
        if (Double.isNaN(d)) {
            ...
            return cutZeroEnding(String.valueOf(d))
        }
    }
    ...
    private static String objectToString(Object d) {
        if (d instanceof Double) {
            return doubleToString((Double) d)
        } else if (d instanceof Float) {
            return floatToString((Float) d)
        } else {
    ...
    private static String concat(String str1, String str2, String op) {
        String s1 = str1
        if (s1.startsWith("-")) {
            s1 = s1.substring(1)
        }
    ...
    private static SE[] simplifySumSes(SE[] ses) {
        ...
        for (int i = 0; i < ses.length - 1; i++) {
            if (ses[i] != null && ses[i].determined) {
                ...
                if (f && v1 != ZERO) {
                    SE s1
                    if (value1 != null) {
                        s1 = new SE(concat(objectToString(v1), value1, " * "), true, "*", false)
                        s1.noScalarValue = value1
                    } else {
                        s1 = new SE(objectToString(v1), true, null, true)
                    }
                    s1.terms = x1.terms
                    s1.scalar = v1
                    x.add(s1)
        ...
        if (!stack.isEmpty()) {
            Double v = stack.pop()
            simplifiedTerms.add(0, new SE(objectToString(v), true, null, true))
        }
    ...
    private static Term simplifyTerm(Term term) {
        ...
        if (!leftStack.isEmpty()) {
            ...
            if (v == ZERO) {
                ...
                if (simplifiedLeftTerms.isEmpty()) {
                    additionalLeftTerm = new SE(objectToString(v), true, null, true)
        ...
        if (!rightStack.isEmpty()) {
            ...
            if (v != ONE) {
                ...
                additionalRightTerm = new SE(objectToString(v), true, null, true)
        ...
        if (additionalRightTerm != additionalLeftTerm) {
            if (additionalLeftTerm != null && additionalRightTerm != null && additionalLeftTerm.scalar != null && additionalRightTerm.scalar != null) {
                Double v = divide(additionalLeftTerm.scalar, additionalRightTerm.scalar)
                SE se = new SE(objectToString(v), true, null, true)
                se.scalar = v
                simplifiedLeftTerms.add(0, se)
            } else {
                if (additionalLeftTerm != null) {
                    simplifiedLeftTerms.add(0, additionalLeftTerm)
                }
                if (additionalRightTerm != null) {
                    simplifiedRightTerms.add(0, additionalRightTerm)
                }
            }
        }
        if (simplifiedLeftTerms.isEmpty()) {
    ...
    private static SE buildMulSE(SE[] ses) {
        if (ses.length > 1) {
            ...
            SE se
            if (v != null) {
                se = new SE(concat(sb.toString(), objectToString(v), " * "), determined, "*", false)
                se.scalar = v
                se.noScalarValue = sb.toString()
            } else {
                se = new SE(sb.toString(), determined, "*", false)
            }
            return negative ? negativeSE(se) : se
        } else {
            return ses[0]
        }
    }
                    ...
                    if (method.getDeclaringClass() == JavaOpenClass.getOpenClass(Comparison.class) && method.getSignature().getNumberOfParameters() == 2) {
                        ...
                        if (op == "==" || op == "!=") {
                            if (e0 instanceof Literal && e1 instanceof Literal) {
                                ...
                                return new SE(objectToString(v), true, null, true)
                            }
                            if (e0 instanceof Literal && !(e1 instanceof Literal)) {
                                Literal literal0 = (Literal) e0
                        ...
                        if (isLiteral0 && !isLiteral1 && op != null) {
                            ...
                            if (d.getLeft()) {
                                ...
                                se1.inverted = () -> new SE(se.determined, getOppositeSwapOperation(d.getRight()), se.value, objectToString(getLiteralValue(e0)))
                                return se1
                            }
                            ...
                            if (d1.getLeft()) {
                                SE se = exprToSE(ast, e1, formula)
                                SE se1 = new SE(se.determined, op1, se.value, objectToString(getLiteralValue(e0)))
                        ...
                        if (!isLiteral0 && isLiteral1 && op != null) {
                            ...
                            if (d.getLeft()) {
                                ...
                                SE se1 = new SE(se.determined, d.getRight(), se.value, d.getMiddle())
                                se1.inverted = () -> new SE(se.determined, getOppositeSwapOperation(d.getRight()), se.value, objectToString(getLiteralValue(e1)))
                                return se1
                            }
                            ...
                            if (d1.getLeft()) {
                                SE se = exprToSE(ast, e0, formula)
                                SE se1 = new SE(se.determined, op, se.value, objectToString(getLiteralValue(e1)))
                                se1.inverted = () -> new SE(se.determined, d1.getRight(), se.value, d1.getMiddle())
                                return se1
                            }
                        }
    ...
    private static SE exprToSE(AST ast, Expression<Node> expression, boolean formula) {
        if (expression instanceof And) {
            ...
            return new SE(objectToString(literal), true, null, true)
